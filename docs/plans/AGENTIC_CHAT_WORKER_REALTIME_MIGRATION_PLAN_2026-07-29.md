...
**Status:** **Phases 0–3 are exited; Phase 4 deterministic implementation and all eight differential classes are complete.** Phase 0/1 passed the clean hosted rerun (24/24 scenarios, 30/30 turn assertions); Phase 2A–2D are hosted and closed per `AGENTIC_CHAT_WORKER_PHASE_2_EXIT_GATE_DECISION_PACKET_2026-08-03.md`. The Phase 3 production canary campaign (2026-08-05 → 2026-08-07, eleven attempts, taskers 49/50) peeled seven layered production defects — capacity budgets, RPC cap scoping, worker cohort, a prod-only `tool_category` constraint, a swallowed terminal-finalize error, and two independent timing-validator contract bugs (`text_delta` evidence source; microsecond-vs-millisecond arithmetic) — and closed with **canary 11 (turn `9e54c04b`, 2026-08-07): completed/stop in 83 s, fully executor-written, verifier PASS, timing summary persisted**. The delegated exit-gate run the same day proved live Stop (391/402 ms executor-written cancelled terminals in the round-1 and synthesis windows), disconnect-survival, and reconnect-restore; the full bullet-by-bullet verdict table and GO recommendation are in `AGENTIC_CHAT_WORKER_PHASE_3_EXIT_GATE_PACKET_2026-08-07.md` (honest carried partials: live queued-window Stop, live worker-restart-mid-turn). Phase 4 schema is hosted through `20260813070000`; P1–P5 are complete, and both legacy and worker coverage trackers exercise success, clarification, read-only tools, mutating tools, supervisor checkpoint, cancellation, timeout, and provider error. Production worker/routing/capability/billing gates remain off and no Phase 4 application rollout is implied. **The remaining Phase 4 exit item is the explicitly authorized hosted 24/24 quality battery against the retained Phase 0 baseline; see `tasker/51-worker-behavioral-parity-phase4.md`.**

**Final Phase 2D checkpoint — 2026-08-03:** The inert fixture includes strict execution/input/effect adapters, fake text/read/mutating-tool execution, durable process-interruption recovery, a chat-only stalled sweep with bounded drain, stable logical effect identity and uncertain-commit convergence, saturated chat/general pool isolation, and a measured 100-turn cancellation/publisher/PostgreSQL fixture. The final composed proofs add durable supersede terminal-wait, both provider-start/cancel orderings, abort-ignoring output fencing, immutable source history across a generation-2 retry, prepared-cache cleanup retention/executability, and duplicate/reload/reconnect/sequence-gap/terminal browser convergence. The latest complete-suite local sample recorded 102 logical RPC calls, 300 affected rows, 448,376 WAL bytes / 4,483.76 bytes per turn, a 20.62 ms text flush, and 31.20 ms total; the explicit disposable-fixture ceilings remain 64 KiB WAL per turn, 2 seconds flush, and 5 seconds total and are not hosted-capacity claims. The exact hosted/local/preliminary evidence split is recorded in `AGENTIC_CHAT_WORKER_PHASE_2_EXIT_GATE_DECISION_PACKET_2026-08-03.md`. Production remains legacy-only and inert.
...
**Current continuation handoff:** `tasker/51-worker-behavioral-parity-phase4.md` (Phase 4 parity, starting with the P0 differential harness)

**Phase 3 exit decision:** `docs/plans/AGENTIC_CHAT_WORKER_PHASE_3_EXIT_GATE_PACKET_2026-08-07.md`

**Phase 2 exit decision:** `docs/plans/AGENTIC_CHAT_WORKER_PHASE_2_EXIT_GATE_DECISION_PACKET_2026-08-03.md`
...
- Added pinned contract fixtures in `packages/shared-types/src/agentic-chat-worker-contract.test.ts` and exported the contract through `@buildos/shared-types` (six at first lock; eight contract tests as of revision .4/.5; the full shared-types package suite was two files / 13 tests at that checkpoint and is two files / 16 tests after the Phase 2B Slice 4 remediation).
...
Phase 1 runtime extraction and server-boundary work is complete. Phase 2A Slice 1 adds the hosted trust/schema foundation, regenerated types, and a server-only immutable input-artifact boundary without worker routing; its exact hosted migration receipt is reconciled. Slice 2's isolated irreversible queue enum and Slice 3's create-validate-drop queued-status/active-index replacement are hosted through `20260801030400`. Slice 4's server-only generation-aware stream state, append-only cancellation signals, and protected terminal retention are hosted as `20260801030500`. Slice 5 is hosted as `20260801030600`; it locks generic queue admission/recovery to the service boundary, scopes recovery to registered consumer types, and unconditionally excludes agentic-chat turns from generic replay. Phase 2B Slice 1 is hosted as `20260801041000`; it adds the server-only effect ledger and scoped tool-execution telemetry link without an executable RPC or worker path. Phase 2B Slice 2 is hosted as `20260801041100`; it adds duplicate-first, current-owner-fenced reserve/begin/reconcile effect RPCs and a type-level single-winner invocation result. Phase 2B Slice 3A–3B is hosted as `20260802020000` and `20260802020100`: duplicate-first atomic admission and the queue/domain current-generation claim bridge are live but unused while worker routing remains disabled. Phase 2B Slice 4 is hosted through `20260802030500`: reserved message identity, generation-scoped durable event identity, and service-only cancellation/finalization are live but unused. Phase 2B Slice 5 is hosted as `20260802031000`: it adds the single-winner provider-start fence and database-enforced safe pre-start recovery classifier without a caller or loop. Phase 2C Slices 1–5 are hosted through exact receipt `20260802037000`: durable stream writers, the inert bounded publisher, exact-sequence delivery acknowledgement, batched cancellation observation, exact private per-user Broadcast receive authorization, and the service-only reconciliation transaction are live but unused by a worker consumer. Phase 2D Slices 1–3 now mount and audit the authenticated receive-only channel/reconciliation runtime and add a legacy-only transport lease plus owned-turn discovery/cancellation gateway without changing production routing. Slice 4 has begun with exact runtime parsing for the hosted duplicate-first admission receipt. Current hosted regeneration aligns at 241 tables / 13 views and 223 RPC names. The Phase 1 record is linked from `AGENTIC_CHAT_WORKER_PHASE_1_HANDOFF_2026-07-31.md`; current Phase 2 progress is recorded in `AGENTIC_CHAT_WORKER_PHASE_2_HANDOFF_2026-07-31.md`.
...
Status note (2026-08-02): Phase 2B Slices 1–2 are hosted through `20260801041100`, including service-only `reserve_agentic_chat_effect`, `begin_agentic_chat_effect`, and `reconcile_agentic_chat_effect`. The SQL resolves an existing same-effect/logical-hash receipt before requiring live ownership, but any new reservation or `reserved -> started` winner must match the running turn, current execution generation, queue job, processing token, queue user, identifier envelope, and correlation id. Only `outcome='started'` returns `invokeAdapter=true` and atomically sets `irreversible_boundary_at`; provider tool-call id and the effect's original reservation generation are not idempotency identity, so a later current generation can safely resume the same reserved effect while the stale generation cannot write. The disposable proof includes a genuine two-connection begin race, accepted-cancel and ownership-loss interleavings, uncertain no-retry/reconciliation, security-wrapper denial, and package-only rollback. The isolated dry run named only `20260801041100`, the post-apply dry run is empty, the hosted ledger is aligned, service role sees all three RPCs while anon sees none, and generated types align at 240 tables / 13 views and 206 RPC names. Worker execution is still disabled.

Admission/claim status note (2026-08-02): migrations `20260802020000_agentic_chat_worker_atomic_admission.sql` and `20260802020100_agentic_chat_worker_claim_fencing.sql` implement hosted Phase 2B Slice 3A–3B, with applied SHA-256 values `b46f33e86cf589a56d6c693dbf1dac58e3a05f3981cf354dc898b0fcbafa3eee` and `b8e9e6c3855e19ac124b9b86981824717cee65e5294dbe986f6c1dbeb50c3d8e`. Admission resolves duplicates under the legacy per-user advisory lock before pressure or writes; enforces hard running/queued caps and server-observed pressure; resolves or creates the session after capacity; validates/consumes trusted prepared input; freezes bounded pre-message lineage; and atomically inserts the queued turn, immutable input artifact, exactly-once user message, stable-dedup queue job, and links. A preflighted unique partial daily-brief session key closes the remaining cross-mode session race, and the compatible legacy session service adopts the SQLSTATE `23505` winner. A read-only hosted preflight found 12 active rows across 12 keys and zero duplicates. Claim requires the exact processing queue envelope, atomically transitions a safe queued turn to running, advances exactly one execution generation, and resets stream state for that generation; same-token repeats are idempotent, stale/forged owners are rejected, accepted cancellation returns a non-start result, and any execution/effect/terminal boundary blocks a new generation. Claim intentionally does not set `execution_started_at`. The complete proof is 9 PostgreSQL files / 11 tests, including genuine concurrent admission and claim races plus package rollback. A receipt-isolated dry run named only these two files, application succeeded in timestamp order, the post-apply dry run is empty, and both ledger receipts show local/remote parity. Service-role OpenAPI exposes both RPCs, anon exposes neither, generated types align at 240 tables / 13 views and 208 RPC names, and worker execution remains disabled. The compatible legacy `23505` winner-adoption code remains to be deployed; until then, a rare concurrent legacy daily-brief create can surface the uniqueness error but cannot create a duplicate canonical session. These slices still add no queue consumer, provider call, cancellation/finalizer, or recovery loop.

Terminal-control status note (2026-08-02): Phase 2B Slice 4 is review-hardened, locally proven, and hosted through exact receipt `20260802030500`. Migration `20260802029900` reserves exact worker `chat-turn:<uuid>:user|assistant` message keys from authenticated preemption while preserving legacy keys and trusted admission/finalization. Migrations `20260802030000`–`20260802030400` add generation-scoped durable event identity with legacy generation-zero backfill, an insert-compatibility default consumed by the validator so legacy generated insert types remain valid, two concurrently built and validated replacement unique indexes, current-generation/scope validation, immutable event rows, and removal of the old turn-wide sequence key only after the replacements are safe. Migration `20260802030500` adds service-only `request_agentic_chat_turn_cancel` and `finalize_agentic_chat_turn`: queued cancel owns queue/turn/projection/event atomically without a blank message; running cancel owns the first durable request/signal; and the terminal CAS atomically resolves the outcome-appropriate assistant message, independently optional provider usage, authoritative complete text/projection, deterministic `done` event, and immutable post-lock terminal receipt. The hosted preflight found 10,324 legacy events with zero invalid scopes or duplicate sequence groups, zero reserved worker-message keys, zero worker-mode rows/events, and no target-object conflicts. A receipt-isolated dry run named only `20260802029900`–`20260802030500`; all seven applied in order; the post-apply dry run was empty; every event now has valid generation-zero identity; both replacement indexes are ready/valid/unique; both checks are validated; and the legacy key is absent. Service-role OpenAPI exposes both terminal RPCs while anon exposes neither. Regenerated types preserve optional event insert identity and align at 240 tables / 13 views and 210 RPC names. The complete agentic-chat PostgreSQL set is green at 10 files / 14 tests, and the package remains inert with no caller, consumer, provider/model path, notification/Broadcast, or retry/recovery loop. Worker execution remains disabled. The remediation record is `AGENTIC_CHAT_WORKER_PHASE_2B_SLICE_4_REMEDIATION_PLAN_2026-08-02.md`.

Execution/recovery status note (2026-08-02): Phase 2B Slice 5 is hosted through exact receipt `20260802031000_agentic_chat_worker_execution_recovery.sql`, SHA-256 `0885c69b21f468152c96884028a33fc98ea061b0dbc3cfd5aed47a06714f6736`. `begin_agentic_chat_turn_execution` is the immediate-before-provider CAS: the single winner receives `invoke_provider=true`, while duplicate/lost-response callers receive the committed start receipt without permission to invoke. `recover_agentic_chat_turn` atomically requeues only explicitly retryable pre-start failures with clean provider/mutation boundaries, zero effect rows, remaining attempts, and fresh input; unknown, post-start, stale, exhausted, and effect-bearing turns never inherit generic retry. It also returns typed cancellation/finalization/effect decisions and idempotently reconciles a terminal domain turn's queue row without re-execution. Genuine concurrent start/start and both start/recovery orderings, post-lock timing, signed-wrapper denial, rollback, stale context, effect corruption defense, and terminal queue repair pass in disposable PostgreSQL. The hosted preflight found zero active worker-mode turns and no target RPC conflicts; the receipt-isolated dry run named only `20260802031000`; application succeeded; the post-apply dry run is empty; the ledger is aligned; and `20260802032000` was intentionally excluded and remained unapplied at this Slice 5 gate. It was subsequently applied and verified later on 2026-08-02 as the native-search query-cache receipt. Service-role OpenAPI exposes both RPCs while anonymous access is denied, regenerated types/schema align at 240 tables / 13 views and 212 RPC names at this gate, and RPC drift is clean. The cumulative database gate is 11 files / 15 tests, the complete Agentic Chat suite is 83 files / 736 tests, and shared types pass 19/19 plus typecheck/build. Worker execution remains disabled and the package adds no caller, consumer, provider/model path, publisher, or recovery loop. The implementation record is `AGENTIC_CHAT_WORKER_PHASE_2B_SLICE_5_EXECUTION_RECOVERY_PLAN_2026-08-02.md`.
...
- Status note (2026-08-01): hosted Slice 5 (`20260801030600`) preserves the 2026-07-23 backoff/attempt behavior while adding include/exclude job-type parameters and an unconditional `agentic_chat_turn` exclusion. `SupabaseQueue` sends the exact registered processor set and the explicit chat exclusion. The disposable and hosted privilege/signature probes are green, and post-application types/function snapshots are aligned.
...
**Operator gate status 2026-08-02:** The baseline, parity ledger, correction chain, production preflight, and hosted evidence are complete. The final clean-tree Phase 0 gate passed all 24 registered scenario executions and all 30 turn assertions with no stream/capture errors. Independent acceptance remains unretained, but the user explicitly waived that signature for bounded Phase 1 work. Phase 1 subsequently completed and its remediation rerun passed the same 24/24 and 30/30 quality bar with zero stream/capture errors; see `AGENTIC_CHAT_WORKER_PHASE_1_HANDOFF_2026-07-31.md`. Phase 2A is closed through exact hosted receipt `20260801030600`. Phase 2B Slices 1–2 are hosted through `20260801041100`; generated types report 240 tables / 13 views and RPC drift is aligned at 206 names. The expanded eight-file / nine-test PostgreSQL suite, service/anon RPC probe, shared contract/typecheck/build, and generator regressions are green. Worker execution remains disabled; see `AGENTIC_CHAT_WORKER_PHASE_2_HANDOFF_2026-07-31.md`.
...
Current Phase 2 checkpoint (2026-08-03): Phase 2A, Phase 2B Slices 1–5, the native-search receipt `20260802032000`, and Phase 2C Slices 1–5 are hosted through exact receipt `20260802037000`. Atomic admission, claim-generation fencing, effect ownership, reserved message/event identity, cancellation/terminal compare-and-set, immediate-before-provider start authorization, fail-closed chat recovery, generation-fenced stream persistence/publication/acknowledgement, durable batched cancellation observation, exact private per-user Broadcast receive authorization, and bounded generation-consistent reconciliation are live but still have no production worker consumer. Hosted types/schema align at 241 tables / 13 views and 223 RPC names, RPC drift is clean, and OpenAPI generation passes 3/3. Phase 2D Slices 1–6, the remaining local exit matrix, and the Phase 2 exit-gate decision packet are complete: lease-verified admission/discovery, mounted receive-only Realtime, authoritative UI projection, the inert fake-provider/tool consumer, process-interruption recovery, chat/general pool isolation, and the deterministic 100-turn cancellation/publisher/PostgreSQL fixture all pass. The final composed proofs close durable supersede terminal-wait, both provider-start/cancel orderings, abort-ignoring provider output fencing, immutable history across source edit/delete and generation-2 retry, prepared-cache cleanup retention/executability, and duplicate/reload/reconnect/sequence-gap/terminal browser convergence. The gate is green at 106 complete Agentic Chat files / 863 tests, 78 passing worker files / 641 passing tests with one explicit opt-in skip, worker typecheck, full lint exit 0 with 170 pre-existing warnings, and whole-worktree `svelte-check` at 0 errors / 0 warnings. Every genuinely new lease remains hardcoded to legacy SSE, the browser does not call the new admission gateway, live capacity defaults closed, the fixture consumer/sweeper are absent from production entrypoints, and no real provider exists. Phase 3 remains the first explicitly approved real asynchronous model execution.
...
- Database reservation of exact `chat-turn:<uuid>:user|assistant` message idempotency keys so authenticated session writers cannot preempt worker admission/finalization while legacy message keys remain compatible.
