...
## 9. Follow-up (2026-08-21 02:20–02:35Z): both §7 observations re-verified and fixed

Live re-test after the routing fix deployed (Vercel `build-nkxgefo9q`, first message now negotiates
worker transport directly). Session `887ccdcf`, four worker turns, all `worker_realtime/completed`.

| Observation                         | Still a problem?                                                                                                                                                                                                                      | Cause                                                                                                                                                                                                                                                                                                                                                                                                                         | Fix                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Doubled reply                       | **Yes — 4/4 turns** (`canary complete.canary complete.`, `done.done.`, `ok.ok.ok.`, `yes.yes.yes.` + guard fallback)                                                                                                                  | `readOnlyProvider.streamInitial/streamContinuation` stream prose live unless a _mutation_ gate is armed. In production (no mutation capabilities) the gate is off, so prose emitted in the same pass as `declare_read_only_turn` was published, the independent review ran, and the post-approval pass answered again. Every turn showed the same 2-round shape (`declare_read_only_turn` → `approve_read_only_turn_review`). | Provider passes are already fully buffered upstream, so text is now held until pass end and **withheld when the pass ends in a semantic-disposition control call** (`declare_read_only_turn` / `declare_turn_contract` / `request_turn_clarification`); the post-disposition pass owns the answer. Non-disposition tool passes and plain finishes flush exactly as before. New regression test; provider suite 62/62, worker suite 1087/1087. |
| `/api/agent/v2/prewarm` "503" burst | **Not a 503.** The Chrome extension reports _aborted_ fetches as `statusCode: 503` (verified with an abort probe: aborted → "503", same request un-aborted → 200; a prewarm POST issued by hand _during_ a worker turn returned 200). | Stack capture shows `PrewarmController.orchestrate → prewarmAgentContext`. The orchestrator's `getIsTurnActive` guard reads `stream.isStreaming`, which the legacy SSE handler (`setIsStreaming`) flips while worker semantic events are projected through it, so each reconcile tick re-ran the `$effect`, started a prewarm and the cleanup aborted it (17–20 wasted function invocations per worker turn).                 | `AgentChatModal` now also treats `stream.activeTurnHandle !== null` as turn-active, so no prewarm is issued while a worker turn owns the session. svelte-check clean.                                                                                                                                                                                                                                                                         |

Residual to watch after deploy: the guard's append-only correction (`terminalTextIntegrity`) can still
append a fallback sentence when the final pass is empty (`synthesis_empty`, seen once); and a model
that repeats itself inside one pass is not deduped. Re-run the canary once the dedicated service is on
the new build and expect a single `canary complete.`.

**Deploy gap:** the dedicated `agentic-chat-worker` did **not** auto-deploy the 02:07Z push (legacy and
Vercel did); it is still on `49dcd5a2b`. Its deployment list shows a FAILED deploy at 23:47Z
(`loadAgenticChatPhase3Config` threw — config validation) followed by manual redeploys at the same
commit. The GitHub trigger for that service needs re-enabling/redeploy from `main` for the provider fix
to reach production.
