...
| # | Tranche | Notes |
|---|---|---|
| T1 | Pure payload builders: `activity-log-summary` (674, zero imports) + `start-here-selector` (44, zero) + `overview-helper` (863) + `overview-helper.test` | Zero I/O, zero access surface. Web shims; broad fan-in (project routes, context loaders) covered by shims. |
| T2 | Pure query-shaping leaves (~320 LOC): `stripInternalPayloadFields`, search-term stack `:155-303`, `applyArchivedReadFilter`, `normalizeAgenticSearchTypes`, counted-rows helpers, `buildDetailNotFoundPayload`, `summarizeDocumentForList`, base-executor normalizers `:383-461`. Delete dead outline code. | Pinned by search/payload tests. |
| T3 | **Access port**: `getActorId` / `getVisibleProjectIds` / `assertProjectAccess(projectId, level)` / `assertEntityAccess` / `resolveProjectSummaries`. Web adapter = RLS client + `current_actor_has_project_member_access`; worker adapter = `loadVisibleProjects` + `actor_has_project_member_access(actorId, …)`. | Pinned by the 391 LOC access tests. |
| T4 | The 18 direct-Supabase reads. Includes the `loadAgentDocumentDetails` fetch/check reorder and re-pointing `scopeEntityQueryToReadableProject:127` + `loadCompactProjectDetails:342` at the port. | **Prod `pg_constraint` diff on `chat_tool_executions_tool_category_check` BEFORE T4 ships** — these tools emit `read`/`search`, worker has only ever written `utility`. |
| T5 | Overview/utility reads: `get_field_info`, `get_workspace_overview`, `get_project_overview`, `change_chat_context`. | **Closes Known live divergence 1** — target the ≥3-real-tool golden here. Decide the profile `usage_scope` filter. |
