...
**Follow-up status (updated 2026-07-24):** Per-slot refill is deployed and production-verified: while a regenerated daily brief remained `processing`, an overlapping `send_notification` job was claimed 3.43 seconds after creation and completed successfully. Typed queue failures now distinguish permanent from transient processor errors; shared payload-validation failures are permanent and unknown errors remain transient for backward compatibility. End-to-end queue correlation application changes are implemented locally, and migration `20260724010000` is applied in production: web enqueue helpers originate a trace ID, `add_queue_job` guarantees a database fallback, and the worker exposes the ID on `ProcessingJob`, job-scoped logging, and queue lifecycle logs.
...
14. **Correlation IDs web→queue→worker implemented locally; database migration applied 2026-07-24** — enqueue helpers and Agent Run atomic dispatch originate `metadata.correlationId`; worker HTTP preserves `X-Correlation-ID`; `add_queue_job` supplies a fallback for direct RPC producers; processors and logs receive the persisted ID. Migration `20260724010000` is applied in production; the application changes still need to be committed and deployed.
15. Retention/cleanup expansion (N18): cover failed/cancelled queue rows plus `agent_run_events`, `notification_deliveries`, and `chat_turn_runs`, using bounded batches and explicit retention windows.
16. Durable chat-turn execution (assessment P2): keep as the eventual direction; the 285s in-process bound + reconcile path is acceptable at current scale.
