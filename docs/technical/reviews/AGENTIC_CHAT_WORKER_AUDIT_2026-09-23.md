...
| 8dda88170            | Regex judges of language removed from the turn path: the final-answer gate is replaced by Jev's structured write scores (a factual "No changes were saved in this turn." instead of 1–2 extra passes); ledger receipts replace claim/disclosure regexes; the stated-future auto-task is retired; scratchpad stripping and web keyword prompt triggers are off the hot path.                                                                                |
| 7224171e2            | Turn leases (`docs/architecture/AGENTIC_CHAT_TURN_LEASES_2026-09-23.md`, migration `20260924000100`): a crashed turn recovers in ~90 s from the worker or the web cron, Stop finalizes a turn silent for 45 s, in-flight writes close as "may already be saved", poison turns back off and park.                                                                                                                                                           |

Pending: paid gate with both flags on; apply migrations `20260924000000` then `20260924000100` to production before the worker and web deploy (patch needles verified against production 2026-09-23), then `pnpm gen:all`; push; flip flags after the gate; remove `CHAT_STALLED_TIMEOUT_MS` from Railway after the worker deploy.

Still open from this audit: one commented lexical check (source-preservation review routing; a `preserve_user_text` tool flag would replace it); a timed-out turn shows no error after a reload; payload growth of the semantic projection; `pass tools with tool_choice none` on synthesis (needs cache data from `llm_usage_logs`).
