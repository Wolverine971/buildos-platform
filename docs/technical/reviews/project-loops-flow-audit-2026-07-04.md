...
BURST LIGHT LOOP
  ...
  → queueProjectLoopBurstAsync                    [skipped if X-Skip-Project-Loop-Burst header]
...
SUGGESTION ACCEPT
  ...
  → decideProjectSuggestion: claim → scoped freshness guard when operations mutate concrete entities
    ...
    (with burst-skip header) → applied/failed → refresh audit counts → sync inbox
...
| Feedback memory        | ✅ server write + read path       | Targeted server/worker coverage; no live post-fix prod run | n/a                                                                             | Pre-fix: **0 of 5 suggestions had `user_feedback`**                                                                   | Dismiss reason UI still thin, but bare dismissals now get implicit feedback and prior decisions load without the old `IS NOT NULL` filter |
...
1. **No input control existed. ✅ Fixed 2026-07-05.** Project Inbox and Dashboard Inbox now render shared dismissal reason/note fields, preserve their values before optimistic card removal, and submit only non-empty fields.
2. **Dashboard modal sent nothing. ✅ Fixed 2026-07-05.** Dashboard dismissals now use the same reason/note state as the Project Inbox path.
...
- **Recursion guard works but is header-scoped.** Suggestion application wraps fetch with `X-Skip-Project-Loop-Burst` (`project-suggestion-actions.service.ts:321-335`), and the 5 burst-wired routes honor it. Any future mutation surface must remember the header — easy future leak, no test enforcing it.
...
1. `ENABLE_PROJECT_LOOPS` in prod worker: **unverifiable from this machine** (no Railway CLI); no scheduled rows since 6/25 is consistent with off. 2. `PUBLIC_ENABLE_PROJECT_LOOPS` in prod web: **confirmed absent** (0 loop/audit vars across all 58 Vercel env entries). 3. Effectively yes — loops are off pending validation, but note dev mode force-enables web-side (`project-loops.ts:18-19` — `dev ||` ignores the flag). 4. Archive the 97 failed rows as historical. 5. Dedup keys: light loop ✅ per project/day; complete audit ✅ per project/day; audit-trigger-eval ✅ project/reason/minute; all parent rows now verify returned queue metadata ownership and fail local losers. 6. Stalled-job reclaim is safe against duplicate suggestions (status-fenced claim) and no longer depends only on future enqueue. 7. Cost caps improved but still soft per LLM call. 8. Telemetry: audit lifecycle + project suggestion lifecycle now exists, but prod signal is unvalidated. 9. Recursion exclusion: yes on the wired paths, header-based, untested. 10. Inbox transitions repairable: yes for inbox↔source drift; no repair for stuck _source_ states or for suggestions stuck `approved` mid-apply.
