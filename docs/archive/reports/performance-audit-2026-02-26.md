<!-- docs/archive/reports/performance-audit-2026-02-26.md -->
...
- Additional `/projects/[id]` payload pass delivered: **6 lazy-load moves complete**
- P1 batch scope delivered: **4/4 complete (100%)**
- Index/RPC migration authored: **complete in repo** (`supabase/migrations/20260426000014_dashboard_projects_perf_phase1.sql`)
...
- ✅ P1 dashboard/projects server-load optimizations:
    - RPC-first project summaries path with legacy fallback (`get_onto_project_summaries_v1`)
    - RPC-first dashboard analytics path with legacy fallback (`get_user_dashboard_analytics_v1`)
    - Route/freshness-gated and TTL-cached `+layout.server.ts` invite/onboarding/billing loads
    - Webhook registration throttle in layout load path
- ✅ P1 index + DB function migration added:
    - Composite partial indexes for `(project_id, updated_at DESC)` scans on tasks/goals/plans/documents
    - Chat-session recency index for dashboard activity queries
    - New SQL functions:
        - `get_onto_project_summaries_v1(uuid)`
        - `get_user_dashboard_analytics_v1(uuid, uuid, integer)`

### Verification Gates (Current)

- ✅ `pnpm --filter @buildos/web test -- 'src/routes/projects/[id]/page.server.test.ts'`
- ✅ `pnpm --filter @buildos/web run build`
- ✅ `pnpm --filter @buildos/web run guardrails:onto-project-columns`
- ✅ Local housekeeping check: no listeners on ports `4174` / `4175`

### Remaining Operational Step

- DB migration application is still required in the target Supabase environment so the new RPC/index path becomes active at runtime.
