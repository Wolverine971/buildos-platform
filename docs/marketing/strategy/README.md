<!-- docs/marketing/strategy/README.md -->

# Marketing Strategy Hub

Use this folder for current positioning, audience, content doctrine, and lifecycle strategy.

## Start Here

1. [BuildOS Marketing Strategy 2026](./buildos-marketing-strategy-2026.md)
2. [BuildOS Welcome Sequence](./buildos-welcome-sequence.md)
3. [BuildOS Guerrilla Content Doctrine](./buildos-guerrilla-content-doctrine.md)
4. [Anti-AI Show-Don't-Tell Strategy](./anti-ai-show-dont-tell-strategy.md)

The four docs above are the fastest path to the current strategy stack.

## Document Map

### Current Operating Docs

| Document                                                                      | Use It For                                                  |
| ----------------------------------------------------------------------------- | ----------------------------------------------------------- |
| [BuildOS Marketing Strategy 2026](./buildos-marketing-strategy-2026.md)       | Current master positioning, audience order, channel roles   |
| [BuildOS Welcome Sequence](./buildos-welcome-sequence.md)                     | Signup-to-activation lifecycle messaging and email sequence |
| [BuildOS Guerrilla Content Doctrine](./buildos-guerrilla-content-doctrine.md) | Why the current content motion works                        |
| [Anti-AI Show-Don't-Tell Strategy](./anti-ai-show-dont-tell-strategy.md)      | Relief-first positioning and anti-hype framing              |

### Supporting Strategy Docs

| Document                                                                            | Use It For                                                             |
| ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| [Thinking Environment Creator Strategy](./thinking-environment-creator-strategy.md) | Creator-facing framing and adjacent strategy thinking                  |
| [ADHD Productivity OS Strategy](./adhd-productivity-os-strategy.md)                 | ADHD wedge, earlier product positioning, and segment-specific strategy |

## Related Docs Outside This Folder

- [Brand Documentation Hub](../brand/README.md)
- [Marketing Documentation Index](../INDEX.md)
- [Social Media Documentation Hub](../social-media/README.md)
- [User Segment Documentation Hub](../user-segments/README.md)
- [Why I Built BuildOS](../content/drafts/why-i-built-buildos.md)
- [DJ's Founder Context](../social-media/FOUNDER_CONTEXT.md)
- [ADHD Customer Language Decoder](../customer-lingo-adhd.md)
- [User Segments: ADHD](../user-segments/users-adhd.md)
- [User Segments: Professionals](../user-segments/users-professionals.md)

## Organizational Rules

- Put current strategy and lifecycle docs here.
- Put brand-system docs in `../brand/`.
- Put reusable narrative drafts in `../content/drafts/`.
- If a new doc replaces an older one, update this hub and the main marketing index the same day.
