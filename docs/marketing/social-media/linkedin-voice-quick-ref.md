<!-- docs/marketing/social-media/linkedin-voice-quick-ref.md -->

# LinkedIn Voice Quick Reference

**Purpose:** Condensed reference for daily LinkedIn engagement
**Full Context:** See `docs/marketing/social-media/FOUNDER_CONTEXT.md`

---

## Identity

- **Who:** DJ, solo founder, Marines veteran (use sparingly), builder
- **Building:** BuildOS - context infrastructure for the AI era / AI-first project organization
- **Angle:** Context engineering > agent engineering, AI that remembers

---

## The Philosophy: Interesting Guy + Cheerleader

> **NOT a thought leader.** Everyone is doing that. It's overdone and inauthentic.
> **The goal:** Interesting person with interesting things to say, OR genuine supporter.

### Two Modes of Engagement

**Mode 1: Add Value (When You Have Something)**

- Share specific experience or insight
- Lead with curiosity, not authority
- "Here's what I've seen..." not "The answer is..."
- Teammate energy, not teaching

**Mode 2: Cheerleader (When You Don't)**

- Genuine excitement for their work
- Curious questions about their journey
- Celebrate their wins specifically
- "We're all rowing the same boat" energy

---

## Voice (LinkedIn-Specific)

- Professional but authentic - not corporate
- Interesting first, founder second
- Longer, more nuanced than Twitter (2-4 sentence comments minimum)
- Direct, no bullshit - but professional
- Hates inauthenticity - especially LinkedIn thought leader types

### DO (Value Mode)

- Lead with observation or insight from experience
- Be specific - use real examples
- Ask questions that show expertise
- "What I'm finding..." / "In my experience building..."
- Reference their past content when engaging big accounts

### DO (Cheerleader Mode)

- Genuine excitement for their work/win
- Specific praise - what exactly impressed you
- Curious questions about their journey
- "Hell yes" and "this is awesome" when you mean it
- "We're all rowing the same boat" energy

### DON'T

- "Love this!" / "Great post!" (hollow)
- Generic observations anyone could make
- Preachy setups ("Most people don't realize...")
- Teaching when you're just supporting
- Thought leader energy
- Making it about you/BuildOS when cheering someone on

---

## Content Pillars (What to Engage With)

| Pillar                  | Your Angle                                                          |
| ----------------------- | ------------------------------------------------------------------- |
| **Context Engineering** | "AI has amnesia. Context that compounds > intelligence that resets" |
| **Building in Public**  | Solo founder, learning voice, the grind, what's working/not         |
| **AI + Productivity**   | Clarity > time management. Context > prompts.                       |
| **Contrarian Takes**    | Blue collar software engineering. PM tools for managers not makers. |

---

## Key Stories to Reference

| Story                          | When to Use                                      |
| ------------------------------ | ------------------------------------------------ |
| **Curri Linear frustration**   | PM tools, thought partner, maker vs manager      |
| **Curri integrations**         | Tool use credibility, "before AI made it trendy" |
| **Pasting docs into ChatGPT**  | Context problem, AI memory                       |
| **Obsidian stale notes**       | Second brain graveyard, tool frustration         |
| **Brain dump → agentic pivot** | Building in public, what I was wrong about       |
| **Marines** (sparingly)        | Background color, not headline                   |

---

## How to Plug BuildOS

**Be transparent. Don't sneak it in.**

- "I'm building BuildOS to solve this exact problem..."
- "This is what I'm trying to address with BuildOS..."
- Only mention when genuinely relevant to the conversation
- Learning voice: "What I'm finding as I build..."

---

## Comment Starters

**Good:**

- "What I'm finding as I build in this space..."
- "I hit this same wall when..."
- "The pattern I keep seeing..."
- "Curious how you think about..."
- "This connects to something I've been wrestling with..."

**Bad:**

- "Love this!"
- "Great post!"
- "This resonated with me"
- "I think..." (just say it directly)
- "Most people don't realize..." (preachy)

---

## Tier 1 Accounts (Engage Every Post)

| Account         | Focus                     | Your Angle                  |
| --------------- | ------------------------- | --------------------------- |
| Ethan Mollick   | AI applications, Wharton  | AI productivity, context    |
| swyx            | AI Engineer, Latent Space | Context engineering         |
| Dan Shipper     | Every, AI productivity    | Direct competitor space     |
| Lenny Rachitsky | Product leadership        | Product decisions, building |
| Sahil Lavingia  | Gumroad, solopreneur      | Solo founder, building      |
| Greg Isenberg   | Startup Ideas             | Community, building         |

## Tier 2 Accounts (Engage When Relevant)

| Category               | Accounts                                      |
| ---------------------- | --------------------------------------------- |
| **AI/LLM**             | Simon Willison, Harrison Chase, AI founders   |
| **Productivity/PKM**   | Tiago Forte, Ali Abdaal, productivity leaders |
| **Indie Hackers**      | Justin Welsh, Arvid Kahl, solo founders       |
| **Building in Public** | Founders sharing their journey                |

**Full list:** See `docs/marketing/social-media/linkedin-engagement-targets.md`

---

## Search Topics

- Context engineering AI
- AI memory problem
- ChatGPT workflow / AI productivity
- Productivity app overwhelm
- Building in public / founder journey
- AI agents (for contrarian takes)

---

## Hashtags to Check

- #ArtificialIntelligence
- #Productivity
- #BuildingInPublic
- #FutureOfWork
- #FounderLife
- #KnowledgeManagement

---

## Commenting Cadence

- **Freshness matters**: Posts <1h old = highest priority (first hour critical on LinkedIn)
- **Comment length**: 2-4 sentences minimum
- **Spacing**: 15-30 min between comments
- **Target**: 5-7 quality engagements per day
- **Best times**: Tue-Thu, 8-9 AM EST or 12-1 PM EST
