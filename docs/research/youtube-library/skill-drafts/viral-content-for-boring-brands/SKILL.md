---
skill_id: viral-content-for-boring-brands
name: Viral Content For Boring Brands
description: Design or audit brand-account content (B2B SaaS, commodity products, internal tools) against the six pre-conscious filters that decide whether a viewer scrolls or stays in the first two seconds. Use when a brand or founder account is producing content for a product that is not inherently exciting (instant ramen, dev tools, cell-phone plans, project-management software) and needs the framework that turns commodity products into 100M+ view accounts. Holds the line between honest attention engineering (curiosity gaps, identity layers, story skeletons) and slop-format theft / performative credentials.
skill_type: combo
categories:
...
The framework is built on a single thesis from Tuan Le, an agency operator who generated 3B+ views for brands like Bulldock (Korean instant ramen, 300K → 1.8M followers, 900M views in 12 months), Stan (creator platform, 20M views for the CEO's personal brand), and a Japanese restaurant (200 views → 1.8M+ per video, 300K followers in three months). The product never changed in any of those cases. The mechanics did.
...
- Creator-led content (use `hook-craft-short-form`, `story-driven-content-craft`, `viral-video-script-structure` — those skills assume a personal-brand voice).
...
**The brain decides before the viewer does.** Six filters fire in <2 seconds. The agent's job is to align with the filters, not bypass them.

**Format steal is a strategic choice with a moral edge.** Borrow formats already aligned with your voice; refuse formats whose audience-acquisition mechanism is rage-bait or fake-confession. Brand morality must be imposed on the format-steal principle — the principle itself is brand-amoral.

**Identity layer beats feature layer every time.** Every product has three layers (attribute / functional / psychological value). Brand content lives at layer 3 or it dies. Features become evidence for the identity claim, not the lead.

**Credentials matter; performance is a tax.** The credential shortcut is real. The honest version is receipts (founder using the product, real customer outcomes, audited numbers). The dishonest version is performative authority (lab coats, fake offices, borrowed environments). Use receipts; reject performance.

**Sharers protect their reputation, not yours.** Content gets shared when sharing makes the _sharer_ look smart, funny, surprising, or ahead-of-the-curve. Design for the sharer's identity payoff, not the viewer's enjoyment.

**Stories suspend skepticism.** Once a viewer is inside Hook → Problem → Story → Payoff, critical processing slows. This is the most influence-dense state your message can occupy.

## Skill Composition

This is a single-source skill distilled from Tuan Le's analysis. Each primitive is an executable check the agent runs in order.

| Primitive ID                        | Job                                                                                                  | Source layer                   |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------ |
...
- The product / feature / message the content is supposed to communicate.
...
- **Tuan Le** (founder of [Shortscut](https://shortscut.com/)) — [What Getting 3 Billion Views Taught Me About Human Psychology](https://www.youtube.com/watch?v=KkK-Y7GiQ2o). The six pre-conscious filters; format-steal mechanics; means-end-chain identity layers; credential shortcut; share-as-status framing; Hook → Problem → Story → Payoff skeleton. Receipts: Bulldock 900M views, Stan 20M views, Japanese restaurant 300K followers in 3 months.

The BuildOS overlay (slop-format rejection, performative-credential rejection, anti-feed alignment) is original synthesis and is documented in the deep-read analysis.

### Deep-read source analysis

- [Tuan Le — What Getting 3 Billion Views Taught Me About Human Psychology (analysis)](../../analyses/2026-05-10_tuan-le_3-billion-views-psychology_analysis.md) — operating logic behind every check, with quotes, anti-patterns, cross-source notes, and the BuildOS use tally per principle.

### Related sibling skills

- `hook-craft-short-form` — sentence-level hook polish (creator-side perspective).
- `story-driven-content-craft` — narrative-arc construction.
...
- **Means-end chain theory** (consumer research literature) — the academic backbone of Principle 3. Worth supplementing the video's three-line summary with the original theory when laddering ambiguous products.
- **Mere exposure effect** (Robert Zajonc, 1968) — the academic backbone of Principle 1.
