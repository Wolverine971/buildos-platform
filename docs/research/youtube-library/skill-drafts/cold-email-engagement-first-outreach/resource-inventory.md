...
Archive summary: the source corpus now has 36 active web-source Markdown cards and 9 active PDF-source Markdown cards after the 2026-05-16 tactical gap-fill pass. 28 low-value cards were removed from the active corpus; their URLs and pruning reasons remain in `source-materials/metadata/sources.json`. Raw HTML snapshots and raw PDF binaries were removed after conversion. Combined with the earlier 2026-05-15 transcript pulls already listed here, this inventory now tracks 7 newly pulled transcripts for the v2 cold email buildout. Use the manifest, cleaned corpus index, cleaned synthesis, and provider requirement matrix as the operational entry points for acquired files, manual book extractions, and next analysis batches.
