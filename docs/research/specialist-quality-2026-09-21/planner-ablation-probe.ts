// docs/research/specialist-quality-2026-09-21/planner-ablation-probe.ts
...
} from '../../../apps/worker/src/workers/agentic-chat/workflow/prototype-provider.ts';
import {
...
async function main() {
	...
	const flush = () =>
		writeFileSync(
			`${directory}/planner-ablation-results.json`,
			JSON.stringify(
				{
					version: 'specialist_planner_ablation_v1',
					createdAt: new Date().toISOString(),
					inputSha256: createHash('sha256').update(inputBytes).digest('hex'),
					note: 'Synthetic sequential model-only comparison. One sample per arm per case, alternating arm order. Same v2 evidence and specialist definitions; vary compiled versus model-authored assignments. No production graph/recovery/dispatch accounting. No retries. Not independently human-scored.',
					spentUsd: spent,
					results
				},
				null,
	...
	async function call(system: string, user: string, maxTokens: number, stage: string, run: any) {
		...
		const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
			...
			headers: {
				...
				'X-Title': 'BuildOS synthetic planner ablation'
			},
	...
	for (const [index, fixture] of fixtures.entries()) {
		const arms = index % 2 ? ['compiled', 'model_planner'] : ['model_planner', 'compiled'];
		for (const arm of arms) {
			...
			try {
				if (arm === 'model_planner') {
					...
					const parsed = parseWorkflowAssignments(text);
					run.plannerParsed = parsed;
					if (parsed) assignments = parsed;
					else run.plannerFallback = true;
				}
