...
function calculateDailyRunTime(now: Date, hours: number, minutes: number, seconds: number, timezone: string): Date {
...
function testScheduler() {
  ...
  testCases.forEach(({ now, desc }) => {
    ...
    console.log(`  ${desc}:`);
    console.log(`    Current time: ${format(nowDate, 'yyyy-MM-dd HH:mm:ss')} UTC`);
    console.log(`    Next run: ${format(nextRun, 'yyyy-MM-dd HH:mm:ss')} UTC`);
    console.log(`    Hours until: ${hoursDiff.toFixed(2)}`);
    console.log(`    ✅ ${hoursDiff >= 0 && hoursDiff < 25 ? 'PASS' : 'FAIL'}\n`);
  });
  ...
  timezones.forEach(tz => {
    ...
    console.log(`    Next run: ${format(nextRunInTz, 'yyyy-MM-dd HH:mm:ss')} ${tz}`);
    console.log(`    Next run UTC: ${format(nextRun, 'yyyy-MM-dd HH:mm:ss')} UTC`);
    console.log(`    ✅ Time is 09:00 in ${tz}: ${format(nextRunInTz, 'HH:mm') === '09:00' ? 'PASS' : 'FAIL'}\n`);
  });
  ...
  for (let i = 0; i < 5; i++) {
    ...
    console.log(`  Day ${i + 1}:`);
    console.log(`    Current: ${format(currentTime, 'yyyy-MM-dd HH:mm:ss')} UTC`);
    console.log(`    Next run: ${format(nextRun, 'yyyy-MM-dd HH:mm:ss')} UTC`);
    ...
    console.log(`    ✅ ${Math.abs(hoursDiff - expectedHours) < 0.1 ? 'PASS' : 'FAIL'}`);
  ...
  console.log(`    Current: ${format(leapYearTest, 'yyyy-MM-dd HH:mm:ss')} UTC`);
  console.log(`    Next run: ${format(leapNextRun, 'yyyy-MM-dd HH:mm:ss')} UTC`);
  console.log(`    ✅ Next day: ${format(leapNextRun, 'MM-dd') === '03-01' ? 'PASS' : 'FAIL'}\n`);
  
  // Year boundary
  const yearBoundaryTest = new Date('2023-12-31T10:00:00Z');
  const yearBoundaryNextRun = calculateDailyRunTime(yearBoundaryTest, 9, 0, 0, 'UTC');
  console.log(`  Year boundary (Dec 31):`);
  console.log(`    Current: ${format(yearBoundaryTest, 'yyyy-MM-dd HH:mm:ss')} UTC`);
  console.log(`    Next run: ${format(yearBoundaryNextRun, 'yyyy-MM-dd HH:mm:ss')} UTC`);
  console.log(`    ✅ Next year: ${format(yearBoundaryNextRun, 'yyyy') === '2024' ? 'PASS' : 'FAIL'}\n`);
