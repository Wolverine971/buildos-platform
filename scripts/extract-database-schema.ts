...
const databaseTypesPath = path.join(process.cwd(), 'packages/shared-types/src/database.types.ts');
const content = fs.readFileSync(databaseTypesPath, 'utf-8');

// Extract the Tables section - handle both old and new format
let tablesMatch = content.match(/public:\s*{\s*Tables:\s*{([\s\S]*?)}\s*Views:/);
if (!tablesMatch) {
	// Try old format
...
for (const section of tableSections) {
	...
	const tableMatch = section.match(/^(\w+):\s*{\s*Row:\s*{([\s\S]*?)}\s*Insert/);
