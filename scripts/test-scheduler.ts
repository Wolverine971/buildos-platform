...
function runSchedulerDiagnostics() {
  ...
  if (nextRunDaily) {
    const hoursUntilNext = (nextRunDaily.getTime() - now.getTime()) / (1000 * 60 * 60);
    ...
    const expectedTime = now < todayAt9UTC ? todayAt9UTC : addDays(todayAt9UTC, 1);
    const isCorrect = nextRunDaily.getTime() === expectedTime.getTime();
    
    console.log(`Expected: ${expectedTime.toISOString()}`);
    console.log(`✅ Daily scheduling: ${isCorrect ? 'CORRECT' : '❌ INCORRECT'}`);
  }
  console.log('');
  ...
  timezones.forEach(timezone => {
    ...
    if (nextRun) {
      ...
      console.log(`  Current time in TZ: ${format(userCurrentTime, 'yyyy-MM-dd HH:mm:ss zzz')}`);
      console.log(`  Next run in TZ: ${format(userNextRun, 'yyyy-MM-dd HH:mm:ss zzz')}`);
      console.log(`  Next run UTC: ${nextRun.toISOString()}`);
      
      // Verify the time is 9:00 AM in the user's timezone
      const isCorrectHour = userNextRun.getHours() === 9 && userNextRun.getMinutes() === 0;
      console.log(`  ✅ Correct local time: ${isCorrectHour ? 'YES' : '❌ NO'}`);
    }
    console.log('');
  });
  ...
  for (let dayOfWeek = 0; dayOfWeek <= 6; dayOfWeek++) {
    ...
    if (nextRun) {
      ...
      const isCorrectTime = nextRun.getUTCHours() === 10 && nextRun.getUTCMinutes() === 0;
  ...
  console.log(`Invalid time (25:00:00): ${invalidResult ? '❌ SHOULD BE NULL' : '✅ CORRECTLY NULL'}`);
  ...
  if (nullResult) {
    const isDefaultTime = nullResult.getUTCHours() === 9 && nullResult.getUTCMinutes() === 0;
  ...
  const validationTests = [
    { desc: 'Valid daily preference', pref: { frequency: 'daily', time_of_day: '09:00:00' }, expectErrors: 0 },
    { desc: 'Invalid frequency', pref: { frequency: 'invalid' }, expectErrors: 1 },
    { desc: 'Invalid time format', pref: { time_of_day: 'not-a-time' }, expectErrors: 1 },
    { desc: 'Invalid hours', pref: { time_of_day: '25:00:00' }, expectErrors: 1 },
  ...
  validationTests.forEach(test => {
    ...
    console.log(`${test.desc}: ${passed ? '✅ PASS' : '❌ FAIL'} (expected ${test.expectErrors} errors, got ${errors.length})`);
  ...
  console.log('================================');
  console.log('The scheduler appears to be working correctly based on these tests.');
...
function checkForSchedulingIssues() {
  ...
  const nextRunDuringDST = calculateNextRunTime(beforeDST as any, dstTransition);
  ...
  console.log('   - This means jobs scheduled 30-60 minutes ahead might not be caught as duplicates');
  
  // This could potentially cause duplicate jobs in edge cases
  console.log('   ⚠️ POTENTIAL ISSUE: Scheduling window (1 hour) vs duplicate window (±30 min) mismatch');
  issues.push('Potential duplicate jobs if scheduling window > duplicate detection window');
  console.log('');

  // Issue 4: Check for missing error handling
  console.log('4. Error Handling Analysis:');
  console.log('   ✅ calculateNextRunTime has try/catch with null return on error');
  console.log('   ✅ Individual user preference processing has error isolation');
...
if (require.main === module) {
  ...
  if (issues.length > 0) {
    console.log('\n🔧 RECOMMENDED FIXES:');
    console.log('1. Consider aligning scheduling window with duplicate detection window');
    console.log('2. Add more comprehensive DST transition testing');
    console.log('3. Consider reducing scheduling window to 45 minutes to prevent edge cases');
  }
}