#!/usr/bin/env node
// scripts/security/export-phase2-retired-data.mjs
// Archive the non-empty Phase 2 retirement bundle before its guarded removal.
...
const suggestedOutput = resolve(homedir(), 'Documents', 'BuildOS Database Archives', 'phase2');
if (!requestedOutput) {
...
async function readTable(table) {
	...
	for (let offset = 0; ; offset += pageSize) {
		let response;
		try {
			response = await fetch(`${supabaseUrl}/rest/v1/${table}?select=*`, {
				headers: {
...
const packageDirectory = resolve(outputRoot, `retired-schema-${timestamp}`);
...
let brainDumpLinks;
try {
	[decisions, edges, notes, brainDumps, brainDumpLinks] = await Promise.all([
		readTable('onto_decisions'),
		readTable('onto_edges'),
		readTable('notes'),
		readTable('brain_dumps'),
		readTable('brain_dump_links')
	]);
} catch (error) {
...
const decisionEdges = edges.filter(
	(row) =>
		decisionIds.has(row.src_id) ||
		decisionIds.has(row.dst_id) ||
		row.src_kind === 'decision' ||
		row.dst_kind === 'decision'
);

const datasets = [
	{ name: 'onto_decisions', rows: decisions, source: 'public.onto_decisions' },
	{
		name: 'onto_decision_edges',
		rows: decisionEdges,
		source: 'public.onto_edges',
		filter: 'src_id/dst_id in exported decision ids OR src_kind/dst_kind = decision'
	},
	{ name: 'notes', rows: notes, source: 'public.notes' },
	{ name: 'brain_dumps', rows: brainDumps, source: 'public.brain_dumps' },
	{ name: 'brain_dump_links', rows: brainDumpLinks, source: 'public.brain_dump_links' }
];
