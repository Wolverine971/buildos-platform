...
import {
	uploadProcessingContract,
	verifyUploadProcessingRaces
} from './libri-upload-processing-races.mjs';
import {
...
import { uploadQuotaContract, verifyUploadQuotaRaces } from './libri-upload-quota-races.mjs';
...
async function runContract(filename, index) {
	...
	try {
		...
		if (filename === uploadProcessingContract)
			await verifyUploadProcessingRaces([
				'-h',
				socketDirectory,
				'-U',
				'postgres',
		...
		if (filename === uploadDownloadContract)
			await verifyUploadDownloadRaces([
				'-h',
				socketDirectory,
				'-U',
				'postgres',
		...
		if (filename === uploadRetirementContract)
			await verifyUploadRetirementRaces([
				'-h',
				socketDirectory,
				'-U',
				'postgres',
		...
		if (filename === uploadQuotaContract)
			await verifyUploadQuotaRaces([
				'-h',
				socketDirectory,
				'-U',
				'postgres',
...
async function runContractAgainstExternalServer(filename, index) {
	...
	try {
		...
		if (filename === uploadQuotaContract)
			await verifyUploadQuotaRaces(['-d', testDatabaseUrl.toString()]);
		console.log(`✓ ${filename}`);
	} catch (error) {
		const output = `${error.stdout ?? ''}${error.stderr ?? ''}`.trim();
		console.error(`✗ ${filename}`);
