// apps/web/src/lib/utils/activityLogger.ts
...
export class ActivityLogger {
	...
	async logActivity(
		...
		try {
			...
			const { error } = await this.supabase.from('user_activity_logs').insert(logEntry);
			...
			await this.errorLogger.logError(error, {
				...
				metadata: {
					activityType,
					activityData,
					errorContext: 'Failed to log user activity'
				}
			});
		}
	}
		...
		try {
			...
			await this.errorLogger.logError(error, {
				metadata: {
					metricName,
					metricValue,
					errorContext: 'Failed to log system metric'
				}
			});
		}
	}
	...
	async logActivitiesBatch(
		...
		try {
			...
			const { error } = await this.supabase.from('user_activity_logs').insert(logEntries);
	...
	async getUserActivitySummary(
		...
		try {
			...
			const mostActiveDay =
				Object.entries(dailyActivity).sort(([, a], [, b]) => b - a)[0]?.[0] || null;
			...
			const metrics = [
				{
					...
					metric_value: new Set(activeUsers.data?.map((log) => log.user_id)).size,
