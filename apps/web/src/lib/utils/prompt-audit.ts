...
export async function savePromptForAudit({
	...
	try {
		...
		const scenarioMap: Record<string, string> = {
			...
			// Calendar analysis (legacy single-prompt)
			'calendar-analysis': 'calendar-analysis/legacy/calendar-analysis-single-prompt.md',

			// Calendar analysis (2-part flow)
		...
		// Detect if this is part of a 2-part flow
		const is2PartFlow = scenarioType.includes('part1') || scenarioType.includes('part2');
		const flowPart = scenarioType.includes('part1')
			? '1'
			: scenarioType.includes('part2')
				? '2'
				: null;

		let flowContextSection = '';
		if (is2PartFlow && flowPart) {
			flowContextSection = `
## Flow Context
...
${is2PartFlow ? `## Flow Notes

${flowPart === '1' ? `This is Part 1 of a 2-part flow. The output (event groups) will be passed to Part 2.
Part 1 uses a lightweight event format to reduce token usage and focus on pattern recognition.` : `This is Part 2 of a 2-part flow. It receives event groups from Part 1 and creates structured projects.
