		...
		<div class="flex gap-1 px-2 py-2 overflow-x-auto border-b border-border/60">
			{#each visibleColumns as col (col.key)}
				...
				<button
					...
					class="inline-flex items-center gap-1 px-2 py-1 rounded-md border text-2xs font-medium transition-colors shrink-0 {isActive
				...
				>
					...
					<span>{col.label}</span>
					<span class="text-2xs text-muted-foreground/70">{count}</span>
				</button>
			{/each}
		</div>
				...
				{#each visibleColumns as col (col.key)}
					...
					>
						...
						>
							...
							<span class="text-2xs text-muted-foreground/80 shrink-0">
								{bucket.length}
							</span>
						</div>
						<div class="p-2 space-y-1.5 max-h-[320px] overflow-y-auto">
							{#if bucket.length === 0}
								<p
									class="text-2xs text-muted-foreground italic text-center px-2 py-3"
								>
									Empty
								</p>
							{:else}
								...
								{#if bucket.length > 8}
									<p
										class="text-2xs text-muted-foreground italic px-2 text-center"
									>
