...
<div class="flex h-full flex-col bg-muted">
	<div class="flex-1 overflow-auto p-3 sm:p-6">
		...
		<div class="flex flex-col gap-3 sm:grid sm:grid-cols-3 sm:gap-3">
			{#each actions as action (action.id)}
				{@const Icon = action.Icon}
				<button
					type="button"
					onclick={() => onSelectAction(action.id)}
					class={`group flex flex-col rounded-lg border border-border bg-card p-3 text-left shadow-ink ${action.texture} transition-all duration-200 hover:border-accent hover:shadow-ink-strong active:scale-[0.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:h-full sm:rounded-xl sm:p-4 lg:p-5 sm:hover:-translate-y-0.5`}
				>
					<!-- Mobile: Icon + Title + Chevron in one row -->
					<div class="flex items-center gap-2 sm:gap-3">
						<div
							class={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${action.iconBg} sm:h-10 sm:w-10`}
						>
					...
					<p
						class="mt-2 text-xs leading-snug text-muted-foreground pl-10 sm:pl-0 sm:mt-3"
					>
						{action.subtitle}
					</p>
					<!-- Desktop footer -->
		...
		<div
			class="mt-3 flex flex-col gap-2 rounded-lg border border-border bg-card p-3 shadow-ink tx tx-bloom tx-weak sm:mt-5 sm:gap-3 sm:p-4"
		>
			<div class="flex items-center gap-2">
				<div
					class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-accent/10 text-accent sm:h-8 sm:w-8"
				>
			...
			<div class="flex flex-wrap items-center gap-1.5 pl-9 sm:gap-2 sm:pl-0">
				...
				<button
					...
					class="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-xs font-semibold text-accent shadow-ink transition hover:border-accent hover:bg-accent/5 pressable focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
				>
					Open focus selector
				</button>
				<button
					...
					class="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-xs font-semibold text-muted-foreground shadow-ink transition hover:border-accent hover:text-foreground pressable focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
				>
					Switch to workspace
				</button>
			</div>
