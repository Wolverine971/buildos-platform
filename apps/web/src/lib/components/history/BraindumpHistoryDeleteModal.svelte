...
<script lang="ts">
	...
	let totalLinkedItems = $derived(
		Object.values(linkedCounts).reduce((sum, count) => sum + count, 0)
	);
</script>
		...
		<div class="space-y-4">
						...
						<div class="flex-1">
							...
							<ul class="text-xs text-amber-700 dark:text-amber-400 space-y-1">
								{#if linkedCounts.projects > 0}
									<li>
										• {linkedCounts.projects} project{linkedCounts.projects !==
										1
											? 's'
											: ''}
									</li>
								...
								{#if linkedCounts.tasks > 0}
									<li>
										• {linkedCounts.tasks} task{linkedCounts.tasks !== 1
											? 's'
											: ''}
									</li>
								{/if}
								{#if linkedCounts.notes > 0}
									<li>
										• {linkedCounts.notes} note{linkedCounts.notes !== 1
											? 's'
											: ''}
									</li>
								{/if}
			...
			>
				<p class="text-sm text-red-700 dark:text-red-300">
					<strong>This action is permanent.</strong> The braindump content will be deleted
