								...
								<div class="mt-4 space-y-2">
									<label class="flex items-start gap-3">
										...
										<div>
											...
											>
												Keep completed and current phases. Move completed
												future tasks to historical phases.
											</p>
										</div>
									</label>
									...
									<label class="flex items-start gap-3">
										...
										<div>
											...
											<p
												class="text-xs text-amber-700 dark:text-amber-300 mt-0.5"
											>
												Delete all existing phases and create completely new
												ones.
											</p>
										</div>
									</label>
										...
										{#if selectedSchedulingMethod === method.value}
											...
											>
												{#if method.value === 'phases_only'}
													Tasks will be organized into phases without
													specific start dates.
												{:else if method.value === 'schedule_in_phases'}
													Tasks will be scheduled within their assigned
													phase durations. Existing task dates may change.
												{:else if method.value === 'calendar_optimized'}
													Tasks will be scheduled around your existing
													calendar events while respecting phase
													boundaries.
												{/if}
											</div>
										{/if}
							...
							<label class="flex items-start gap-2 cursor-pointer">
								...
								<div>
									...
									<p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
										When checked, tasks with existing dates will keep their
										current scheduling. When unchecked (default), all tasks will
										be rescheduled optimally within phases.
									</p>
								</div>
							</label>
								...
								>
									<div class="flex items-start gap-2">
										...
										<p class="text-xs text-blue-700 dark:text-blue-300">
											Tasks will be rescheduled to optimal dates within their
											assigned phases. This ensures better workflow and
											timeline alignment.
										</p>
									</div>
								</div>
								...
								>
									<div class="flex items-start gap-2">
										...
										<p class="text-xs text-amber-700 dark:text-amber-300">
											Tasks will maintain their current dates. This may result
											in less optimal phase organization if task dates
											conflict with phase timelines.
										</p>
									</div>
								</div>
							{/if}
					...
					<div class="flex flex-wrap gap-2">
						{#each statusOptions as option}
							<Button
								...
								variant={selectedStatuses.includes(option.value)
									? 'primary'
									: 'outline'}
								size="sm"
								class="rounded-full"
												...
												>
													...
													<div>
														<span
															class="text-sm font-medium text-gray-900 dark:text-white"
														>
															Allow AI to suggest rescheduling
															recurring tasks
														</span>
														<p
															class="text-xs text-gray-500 dark:text-gray-400 mt-1"
													...
													>
														<div class="flex items-start gap-2">
															...
															<p
																class="text-xs text-amber-700 dark:text-amber-300"
															>
																When enabled, the AI may suggest
																moving weekly meetings to phase
																start dates or adjusting review
																cycles to align with phase
																completions. You'll review all
					...
					<div class="mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
						...
						<div class="space-y-1 text-sm text-gray-600 dark:text-gray-300">
							...
							{#if previewData.estimated_phases}
								<p>
									Estimated <strong>{previewData.estimated_phases}</strong> phases
									will be created
								</p>
							{/if}
						</div>
								...
								>
									...
									{#if previewData.conflicts.past_incomplete_tasks?.length > 0}
										<div class="mb-4">
											...
											>
												...
												{:else}
													These tasks have past dates and will be
													rescheduled:
												{/if}
											</p>
									...
									{#if previewData.conflicts.outside_timeline_tasks?.length > 0}
										<div>
											...
											>
												{#if selectedSchedulingMethod === 'phases_only'}
													These tasks have dates outside the project
													timeline and will be moved to phases without
													dates:
												{:else}
													These tasks have dates outside the project
													timeline and will be rescheduled:
												{/if}
											</p>
											<div class="space-y-1 max-h-32 overflow-y-auto">
