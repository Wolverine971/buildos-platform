...
{#if isOpen}
	...
	>
		<!-- Modal -->
		<div
			class="bg-white dark:bg-gray-800 w-full sm:max-w-lg sm:rounded-xl shadow-xl max-h-[90vh] sm:max-h-[85vh] flex flex-col animate-slide-up sm:animate-fade-in"
		...
		>
			...
			>
				<div class="flex items-center gap-3">
					...
					<div>
						...
						<p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
							{#if clearDates && !moveToBacklog}
								{affectedTasks.length} task{affectedTasks.length !== 1 ? 's' : ''} will
								have dates cleared
							{:else if moveToBacklog}
								{affectedTasks.length} task{affectedTasks.length !== 1 ? 's' : ''} will
								be moved to backlog
							{:else}
								{scheduledTaskCount} scheduled task{scheduledTaskCount !== 1
									? 's'
									: ''} will be affected
							{/if}
						</p>
				...
				<Button
					...
					class="p-2 min-h-0 min-w-0 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
			...
			<div class="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
				...
				>
					<div class="flex gap-2">
						<AlertTriangle
							class="w-4 h-4 text-orange-600 dark:text-orange-400 flex-shrink-0 mt-0.5"
						/>
						<div class="space-y-2">
				...
				<div class="space-y-3">
					...
					>
						...
						<div class="flex-1">
							...
							<p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
								Remove start and due dates from tasks (tasks will become
								unscheduled)
							</p>
						</div>
					</label>
					...
					{#if totalTasksInPhases > 0}
						<label
							class="flex items-start gap-3 p-3 rounded-lg cursor-pointer transition-colors {moveToBacklog
				...
				>
					<div class="flex gap-2">
						...
						<div class="text-sm text-blue-700 dark:text-blue-300">
							{#if moveToBacklog}
								Tasks will be moved to the backlog, removed from their phases, and
								all calendar events will be deleted.
								{#if clearDates}
							...
							{:else if clearDates}
								...
								will be removed.
							{:else}
								Tasks will keep their dates but calendar events will be removed. You
								can sync them back to calendar later.
							{/if}
						</div>
					</div>
				</div>
