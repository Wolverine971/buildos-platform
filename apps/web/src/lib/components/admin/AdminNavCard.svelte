...
>
	...
	>
		<div class={`flex h-full flex-col ${compact ? 'gap-4' : 'gap-6'}`}>
			<div class="flex items-start justify-between gap-4">
				<div class={`flex items-center ${compact ? 'gap-3' : 'gap-4'}`}>
					{#if icon}
						{@const Icon = icon}
						<span
							class={`flex ${compact ? 'h-10 w-10' : 'h-12 w-12'} items-center justify-center rounded-2xl bg-white/70 text-blue-600 shadow-sm dark:bg-slate-900/40 dark:text-blue-200`}
						>
			...
			<div class="mt-auto flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
				...
				<span class="font-semibold tracking-[0.2em]">
					{active ? 'ACTIVE' : 'OPEN'}
				</span>
			</div>
		</div>
