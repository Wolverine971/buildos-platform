...
<script lang="ts">
	...
	// Process brain dumps data into monthly buckets
	$: processedData = (() => {
	...
	$: maxCount = Math.max(...processedData.map((d) => d.count), 1);
	$: pointSpacing = processedData.length > 1 ? 350 / (processedData.length - 1) : 0;
		...
		<div class="relative">
			<svg class="w-full h-60" viewBox="0 0 450 240">
				...
				{#each Array(6) as _, i}
					...
					/>
					<text x="55" y={45 + i * 28} text-anchor="end" font-size="10" fill="#6b7280">
				...
				>
					...
					{#if hoveredPoint.point.dumps?.length}
						<p class="text-xs text-gray-600 dark:text-gray-400 mt-1">
							Latest: {hoveredPoint.point.dumps[hoveredPoint.point.dumps.length - 1]
								.title || 'Untitled'}
						</p>
					{/if}
				</div>
...
{:else}
	<div class="flex flex-col items-center justify-center h-60 text-gray-500 dark:text-gray-400">
