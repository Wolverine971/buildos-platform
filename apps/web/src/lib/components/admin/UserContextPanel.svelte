...
<script lang="ts">
	...
	function computeUserHealth(): {
		...
		if (userContext.basic.last_visit) {
			const daysSinceVisit = Math.floor(
				(Date.now() - new Date(userContext.basic.last_visit).getTime()) / (1000 * 60 * 60 * 24)
			);
		...
		// Beta engagement
		if (userContext.beta) {
			if (userContext.beta.total_feedback_submitted && userContext.beta.total_feedback_submitted > 0)
				score += 10;
	...
	function getSubscriptionBadge(status: string | null): { class: string; label: string } {
		switch (status) {
			case 'active':
				return { class: 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-500/30', label: 'Active' };
			case 'trialing':
				return { class: 'bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-500/30', label: 'Trial' };
			case 'past_due':
				return { class: 'bg-red-500/10 text-red-700 dark:text-red-300 border-red-500/30', label: 'Past Due' };
			case 'beta':
				return { class: 'bg-violet-500/10 text-violet-700 dark:text-violet-300 border-violet-500/30', label: 'Beta' };
			default:
	...
	function formatUserContextAsMarkdown(): string {
		const lines: string[] = [];
		lines.push(`# User Context: ${userContext.beta?.full_name || userContext.basic.name || userContext.basic.email}`);
		lines.push('');
		...
		if (userContext.beta) {
			...
			if (userContext.beta.company_name) lines.push(`- **Company**: ${userContext.beta.company_name}`);
		...
		if (userContext.onboarding) {
			...
			if (userContext.onboarding.projects) lines.push(`- **Projects**: ${userContext.onboarding.projects}`);
			if (userContext.onboarding.challenges) lines.push(`- **Challenges**: ${userContext.onboarding.challenges}`);
			if (userContext.onboarding.helpFocus) lines.push(`- **Focus**: ${userContext.onboarding.helpFocus}`);
		}
	...
	function getEmailStatusBadge(status: string): { class: string; label: string } {
		switch (status) {
			case 'sent':
				return { class: 'bg-sky-500/10 text-sky-700 dark:text-sky-300 border-sky-500/30', label: 'Sent' };
			case 'delivered':
				return { class: 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-500/30', label: 'Delivered' };
			case 'failed':
				return { class: 'bg-red-500/10 text-red-700 dark:text-red-300 border-red-500/30', label: 'Failed' };
			case 'bounced':
				return { class: 'bg-orange-500/10 text-orange-700 dark:text-orange-300 border-orange-500/30', label: 'Bounced' };
			case 'complaint':
				return { class: 'bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-500/30', label: 'Complaint' };
			default:
				return { class: 'bg-muted text-muted-foreground border-border', label: status || 'Pending' };
		}
	}
	...
	<div class="px-3 py-2 border-b border-border bg-muted/30">
		<div class="flex items-center justify-between gap-2">
			...
			<div class="flex items-center gap-2 shrink-0">
				<div
					class="px-2 py-1 rounded-md text-xs font-medium border {getHealthBg(userHealth.status)} {getHealthColor(userHealth.status)}"
				>
	...
	{#if expanded}
		...
		<div class="px-3 py-2 border-b border-border grid grid-cols-4 gap-2">
			<div class="text-center">
				<div class="text-lg font-bold text-foreground">{userContext.activity.project_count}</div>
				<div class="text-[0.65rem] uppercase tracking-wide text-muted-foreground">Projects</div>
			</div>
			<div class="text-center">
				<div class="text-lg font-bold text-foreground">{userContext.activity.tasks_created}</div>
				<div class="text-[0.65rem] uppercase tracking-wide text-muted-foreground">Tasks</div>
			</div>
			<div class="text-center">
				<div class="text-lg font-bold text-foreground">{userContext.activity.agentic_sessions_count}</div>
				<div class="text-[0.65rem] uppercase tracking-wide text-muted-foreground">Agentic</div>
			</div>
			<div class="text-center">
				<div class="text-lg font-bold {userContext.activity.tasks_completed > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-foreground'}">
		...
		{#if showActions}
			<div class="px-3 py-2 border-b border-border">
				<div class="text-[0.65rem] uppercase tracking-wide text-muted-foreground mb-1.5">Quick Email</div>
			...
			<div>
				...
				>
					...
					{#if true}
						...
						<span class="ml-auto px-1.5 py-0.5 text-[0.65rem] rounded border {badge.class}">{badge.label}</span>
					{/if}
				</button>
				...
				{#if expandedSections.has('basic')}
					<div class="px-3 pb-2 grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
						<div class="flex justify-between">
							...
							<span class="text-foreground">{formatDate(userContext.basic.created_at)}</span>
						</div>
						<div class="flex justify-between">
							<span class="text-muted-foreground">Last visit</span>
							<span class="text-foreground">{formatRelativeDate(userContext.basic.last_visit)}</span>
						</div>
				...
				<div>
					...
					>
						...
						{#if userContext.beta.beta_tier}
							<span class="ml-auto px-1.5 py-0.5 text-[0.65rem] rounded bg-violet-500/10 text-violet-700 dark:text-violet-300 border border-violet-500/30">
						...
						<div class="px-3 pb-2 space-y-2">
							...
							<div class="grid grid-cols-3 gap-2">
								<div class="text-center p-1.5 rounded-md bg-muted/50">
									<div class="text-sm font-semibold text-foreground">{userContext.beta.total_feedback_submitted || 0}</div>
									<div class="text-[0.6rem] text-muted-foreground">Feedback</div>
								</div>
								<div class="text-center p-1.5 rounded-md bg-muted/50">
									<div class="text-sm font-semibold text-foreground">{userContext.beta.total_calls_attended || 0}</div>
									<div class="text-[0.6rem] text-muted-foreground">Calls</div>
								</div>
								<div class="text-center p-1.5 rounded-md bg-muted/50">
									<div class="text-sm font-semibold text-foreground">{userContext.beta.total_features_requested || 0}</div>
							...
							<div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
								{#if userContext.beta.company_name}
									<div class="flex justify-between">
										...
										<span class="text-foreground truncate ml-2">{userContext.beta.company_name}</span>
									</div>
								{/if}
								{#if userContext.beta.job_title}
									<div class="flex justify-between">
										<span class="text-muted-foreground">Role</span>
										<span class="text-foreground truncate ml-2">{userContext.beta.job_title}</span>
									</div>
								{/if}
								{#if userContext.beta.discount_percentage}
									<div class="flex justify-between col-span-2">
										<span class="text-muted-foreground">Discount</span>
										<span class="text-emerald-600 dark:text-emerald-400 font-medium">{userContext.beta.discount_percentage}% lifetime</span>
									</div>
								{/if}
							</div>
							...
							<div class="flex flex-wrap gap-1">
								{#if userContext.beta.wants_community_access}
									<span class="px-1.5 py-0.5 text-[0.6rem] rounded bg-muted text-muted-foreground">Community</span>
								{/if}
								{#if userContext.beta.wants_weekly_calls}
									<span class="px-1.5 py-0.5 text-[0.6rem] rounded bg-muted text-muted-foreground">Weekly Calls</span>
								{/if}
								{#if userContext.beta.wants_feature_updates}
									<span class="px-1.5 py-0.5 text-[0.6rem] rounded bg-muted text-muted-foreground">Updates</span>
								{/if}
							</div>

							<!-- Challenge -->
							{#if userContext.beta.biggest_challenge}
								<div class="p-2 rounded-md bg-amber-500/5 border border-amber-500/20 tx tx-static tx-weak">
									<div class="text-[0.65rem] uppercase tracking-wide text-amber-600 dark:text-amber-400 mb-0.5">Challenge</div>
									<p class="text-xs text-foreground line-clamp-2">{userContext.beta.biggest_challenge}</p>
								</div>
							{/if}
						</div>
					{/if}
				...
				{#if expandedSections.has('activity')}
					<div class="px-3 pb-2 space-y-2">
							...
							<div class="flex items-center gap-2">
								<div class="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
									<div
										...
										style="width: {Math.round((userContext.activity.tasks_completed / userContext.activity.tasks_created) * 100)}%"
									></div>
								</div>
								<span class="text-xs text-muted-foreground">
									{Math.round((userContext.activity.tasks_completed / userContext.activity.tasks_created) * 100)}% done
								</span>
							</div>
						{/if}
						...
						{#if userContext.activity.recent_projects.length > 0}
							<div>
								<div class="text-[0.65rem] uppercase tracking-wide text-muted-foreground mb-1">Recent Projects</div>
								<div class="space-y-1">
									{#each userContext.activity.recent_projects.slice(0, 3) as project}
										<div class="flex items-center justify-between text-xs p-1.5 rounded-md bg-muted/50">
											<span class="text-foreground truncate flex-1">{project.title}</span>
											<span class="text-muted-foreground ml-2 shrink-0">{formatRelativeDate(project.updated_at)}</span>
										</div>
									{/each}
								</div>
							</div>
						...
						<div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
							<div class="flex justify-between">
								...
								<span class="text-foreground">{userContext.activity.daily_briefs_count}</span>
							</div>
							<div class="flex justify-between">
								<span class="text-muted-foreground">Calendar</span>
								<span class="text-foreground">{userContext.activity.calendar_connected ? 'Connected' : 'No'}</span>
							</div>
						</div>
					</div>
				{/if}
				...
				<div>
					...
					>
						...
						{#if userContext.onboarding.completedAt}
							<span class="ml-auto text-[0.65rem] text-emerald-600 dark:text-emerald-400">
						...
						<div class="px-3 pb-2 space-y-2">
							{#if userContext.onboarding.projects}
								<div class="p-2 rounded-md bg-blue-500/5 border border-blue-500/20">
									<div class="text-[0.65rem] uppercase tracking-wide text-blue-600 dark:text-blue-400 mb-0.5">Projects</div>
									<p class="text-xs text-foreground line-clamp-2">{userContext.onboarding.projects}</p>
								</div>
							{/if}
							{#if userContext.onboarding.challenges}
								<div class="p-2 rounded-md bg-amber-500/5 border border-amber-500/20">
									<div class="text-[0.65rem] uppercase tracking-wide text-amber-600 dark:text-amber-400 mb-0.5">Challenges</div>
									<p class="text-xs text-foreground line-clamp-2">{userContext.onboarding.challenges}</p>
								</div>
							{/if}
							{#if userContext.onboarding.helpFocus}
								<div class="p-2 rounded-md bg-violet-500/5 border border-violet-500/20">
									<div class="text-[0.65rem] uppercase tracking-wide text-violet-600 dark:text-violet-400 mb-0.5">Focus</div>
									<p class="text-xs text-foreground line-clamp-2">{userContext.onboarding.helpFocus}</p>
								</div>
							{/if}
						</div>
					{/if}
			...
			{#if userContext.emailHistory && userContext.emailHistory.length > 0}
				<div>
					...
					>
						...
						<span class="ml-auto px-1.5 py-0.5 text-[0.65rem] rounded bg-muted text-muted-foreground">
								...
								<div class="rounded-md border border-border overflow-hidden">
									...
									>
										...
										<div class="min-w-0 flex-1">
											<div class="flex items-center gap-2 mb-0.5">
												<p class="text-xs font-medium text-foreground truncate flex-1">{email.subject}</p>
												{#if true}
													{@const badge = getEmailStatusBadge(email.recipient_status)}
													<span class="px-1.5 py-0.5 text-[0.6rem] rounded border shrink-0 {badge.class}">
														{badge.label}
													</span>
												{/if}
											</div>
											<div class="flex items-center gap-2 text-[0.65rem] text-muted-foreground">
												<span>{formatRelativeDate(email.sent_at || email.created_at)}</span>
												{#if email.opened_at}
									...
									{#if expandedEmails.has(email.id)}
										<div class="border-t border-border bg-card">
											<!-- Email Metadata -->
											<div class="px-3 py-2 border-b border-border text-xs grid grid-cols-2 gap-x-4 gap-y-1">
												<div class="flex justify-between">
													<span class="text-muted-foreground">From</span>
													<span class="text-foreground">{email.from_name}</span>
												</div>
												<div class="flex justify-between">
													<span class="text-muted-foreground">To</span>
													<span class="text-foreground font-mono text-[0.65rem]">{email.recipient_email}</span>
												</div>
												<div class="flex justify-between">
													<span class="text-muted-foreground">Sent</span>
													<span class="text-foreground">{formatDate(email.sent_at || email.created_at)}</span>
												</div>
												{#if email.delivered_at}
													<div class="flex justify-between">
														<span class="text-muted-foreground">Delivered</span>
														<span class="text-foreground">{formatDate(email.delivered_at)}</span>
													</div>
												{/if}
												{#if email.opened_at}
													<div class="flex justify-between col-span-2">
														<span class="text-muted-foreground">First opened</span>
														<span class="text-emerald-600 dark:text-emerald-400">{formatDate(email.opened_at)}</span>
													</div>
												{/if}
												{#if email.category}
													<div class="flex justify-between col-span-2">
														<span class="text-muted-foreground">Category</span>
														<span class="text-foreground">{email.category}</span>
													</div>
												{/if}
											</div>
											...
											<div class="p-3">
												<div class="flex items-center justify-between mb-2">
													<span class="text-[0.65rem] uppercase tracking-wide text-muted-foreground">Content</span>
													<button
														onclick={() => copyEmailContent(email.content)}
												...
												<div class="p-3 rounded-md bg-muted/50 border border-border text-xs text-foreground whitespace-pre-wrap leading-relaxed max-h-64 overflow-y-auto">
											...
											{#if email.error_message}
												<div class="px-3 pb-3">
													<div class="p-2 rounded-md bg-red-500/10 border border-red-500/30 text-xs text-red-700 dark:text-red-300 flex items-start gap-2">
