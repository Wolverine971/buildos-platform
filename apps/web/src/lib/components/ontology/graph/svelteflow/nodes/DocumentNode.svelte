<!-- apps/web/src/lib/components/ontology/graph/svelteflow/nodes/DocumentNode.svelte -->
<!-- DocumentNode.svelte - Svelte Flow custom node for documents -->
<script lang="ts">
...
>
	<Handle type="target" position={Position.Top} class="!bg-info !w-2 !h-2" />

	<div class="flex items-center gap-1.5">
		<FileText class="w-3.5 h-3.5 flex-shrink-0 text-info" />
		<span class="text-xs font-medium truncate text-info">
			{data.label}
		</span>
	</div>
	...
	<Handle type="source" position={Position.Bottom} class="!bg-info !w-2 !h-2" />
</div>
