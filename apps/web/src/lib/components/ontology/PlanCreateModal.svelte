...
<!--
	...
	Creates plans within the BuildOS ontology system using a two-step flow:
	1. Template selection
	2. Plan details entry with date range support
	...
	- Task Creation: /apps/web/src/lib/components/ontology/TaskCreateModal.svelte
-->
<script lang="ts">
	import { ChevronRight, Loader, Calendar } from 'lucide-svelte';
	...
	// Form configuration for plan details
	...
	async function loadTemplates() {
		try {
			...
			templateError = 'Failed to load plan templates';
		} finally {
...
<FormModal
	title={showTemplateSelection ? 'Select Plan Template' : 'Create New Plan'}
									...
									>
										...
										{#if template.metadata?.typical_scope}
											<div class="mt-2 flex items-center gap-2">
												...
												>
													{template.metadata.typical_scope}
												</span>
											</div>
										{/if}
									</button>
