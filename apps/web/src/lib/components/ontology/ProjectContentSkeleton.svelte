...
<div class="min-w-0 space-y-2 sm:space-y-4">
	...
	>
			...
			<div class="flex items-center gap-2 sm:gap-3 flex-1">
				...
				<div>
					...
					<p class="text-[10px] sm:text-xs text-muted-foreground hidden sm:block">
						Loading {outputCount === 1 ? 'deliverable' : 'deliverables'}
					</p>
				</div>
			</div>
		...
		<div class="border-t border-border">
			{#if outputCount === 0}
				<div class="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-3 sm:py-4 bg-muted/30 tx tx-bloom tx-weak">
					<div class="w-6 h-6 sm:w-8 sm:h-8 rounded-md sm:rounded-lg bg-accent/10 flex items-center justify-center">
						<Sparkles class="w-3 h-3 sm:w-4 sm:h-4 text-accent" />
					</div>
					<div>
						<p class="text-xs sm:text-sm text-foreground">No outputs yet</p>
						<p class="text-[10px] sm:text-xs text-muted-foreground hidden sm:block">Create one to start delivering</p>
					</div>
				</div>
			{:else}
					...
					{#each Array(Math.min(outputCount, 5)) as _, i}
						<li class="animate-pulse" style="animation-delay: {i * 75}ms">
							<div class="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-3">
								<div class="w-6 h-6 sm:w-8 sm:h-8 rounded-md sm:rounded-lg bg-muted flex-shrink-0"></div>
								<div class="flex-1 space-y-1.5 sm:space-y-2">
									<div class="h-3 sm:h-4 rounded bg-muted w-2/3"></div>
									<div class="h-2.5 sm:h-3 rounded bg-muted/70 w-1/4 hidden sm:block"></div>
								</div>
								<div class="h-4 sm:h-5 w-10 sm:w-14 rounded-full bg-muted"></div>
							</div>
						</li>
					{/each}
					{#if outputCount > 5}
						<li class="px-3 sm:px-4 py-1.5 sm:py-2 text-[10px] sm:text-xs text-muted-foreground text-center">
							+{outputCount - 5} more...
						</li>
					{/if}
				</ul>
			...
			{#if documentCount === 0}
				<div class="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-3 sm:py-4 bg-muted/30 tx tx-bloom tx-weak">
					...
					<div>
						...
						<p class="text-[10px] sm:text-xs text-muted-foreground hidden sm:block">Add research or drafts</p>
					</div>
				</div>
			{:else}
				<ul class="divide-y divide-border/80">
