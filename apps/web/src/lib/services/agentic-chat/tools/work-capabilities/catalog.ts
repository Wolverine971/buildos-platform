...
import type { WorkCapabilityDefinition } from './types';

const WORK_CAPABILITY_CATALOG: WorkCapabilityDefinition[] = [
	{
	...
	},
	{
		...
		coverageStatus: 'strong',
		notes: ['BuildOS-native work capability; it does not require an active subject domain.']
	}
];

export function listWorkCapabilities(): WorkCapabilityDefinition[] {
	return [...WORK_CAPABILITY_CATALOG];
}

export function getWorkCapabilityById(id: string): WorkCapabilityDefinition | undefined {
	const normalized = id.trim().toLowerCase();
	return WORK_CAPABILITY_CATALOG.find((capability) => capability.id === normalized);
}

export function listWorkCapabilitiesForDomain(domainId: string): WorkCapabilityDefinition[] {
	const normalized = domainId.trim().toLowerCase();
	if (!normalized) return [];
	return WORK_CAPABILITY_CATALOG.filter((capability) =>
