...
<!--
  ...
  This file is skill_type: procedure (secondary flavor: reference — it carries the atomic-design taxonomy as
  declarative Knowledge, per the inventory's procedure-vs-reference note). It is a child review pass under
  build_quality_ui_ux and loads its OWN reference modules (token taxonomy, adoption diagnosis) rather than
  delegating to sibling skills, so there is no Routing block.
-->
...
1. Map the system layers using the **atomic-design taxonomy** (see **Knowledge**): atoms/tokens, molecules, organisms, templates, pages, documentation, release channels, adoption surfaces. Treat it as a mental model, not a waterfall (Frost: "create the whole and the parts simultaneously").
...
3. Run the **adoption check** (see **Judgment**). If the bottleneck is adoption, the system feels isolated, or the question is ROI, load `adoption-diagnosis` for the distance/virtuous-circle/interface-inventory protocol.
...
6. Classify the migration path — keep template, customize, parity rebuild, flow-by-flow migration, or rebuild — then return findings per the **Contract**.
...
_Source: Brad Frost, Atomic Design (2015), reaffirmed 2024 — **PRIMARY**._
...
- Primary sources (**PRIMARY**): Brad Frost, Nathan Curtis, Una Kravets, Steve Schoger, and Karri Saarinen.
