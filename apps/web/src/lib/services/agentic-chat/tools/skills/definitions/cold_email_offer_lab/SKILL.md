...
<!--
  ...
  This file is skill_type: strategy, so Judgment carries the weight — the Governing Source Cards, the
  Core-vs-Front-End distinction, the Artifact Test / Rewrite Patterns / Disqualifiers, and the Trust/Ask
  Ratio Rubric (T0-T3 ladder, exception register, production-cost + false-positive checks) all live there.
  Knowledge holds the artifact catalogs (families, mode library, CTA-fits-product). Routing is omitted:
  this is a leaf child with no sibling dependencies. All reference content is folded inline by design
  (see Provenance) — no reference modules.
-->

## Identity

Child skill for designing or repairing the cold outreach offer before copy is written — it separates the core offer from a front-end artifact, picks mode-appropriate artifacts, sizes the smallest useful yes against a T0-T3 trust ladder, and runs false-positive checks on the result. This is a **strategy** skill at **domain** altitude, a child of `cold_email_engagement_first_outreach`.

The north star is qualified conversations started per unit of market trust consumed: the offer is the largest lever on both halves of that ratio.
...
The decision spine — the rubrics, thresholds, and boundaries you reason with when constructing or repairing an offer.
...
Internal calibration note (internal-default): the trust-level ladder itself is an internal construction; the level behaviors are sourced as cited. Treat the ceilings as defaults to tune, not industry standards.
...
2. Apply the six-test artifact filter from the Offer Design Rubric (see Judgment → Artifact Test) to anything already on the table; use its rewrite patterns when the existing offer is meeting-first or feature-first.
...
5. Generate 2-3 artifact hypotheses keyed to the mode using the Offer Artifact Library by Mode (see Knowledge). Match the artifact to what is actually being sold (data product → snapshot; service → teardown; SaaS → 3 signals; advisory → note).
6. Using the Trust/Ask Ratio Rubric (see Judgment → Trust/Ask Ratio), assign the trust tier (T0-T3) and choose the smallest useful yes within that tier's ask ceiling. If a meeting ask is proposed, allow it only if it appears in the exception register.
7. Run the false-positive checks (see Judgment → False-Positive Checks): production cost (>30 min per accepted reply disqualifies volume use), Mom Test evidence standard, Dunford buyer-choice check, Moesta struggling-moment check.
...
The candidate catalogs the rubric filters. These fire on every offer construction, so they are folded inline rather than extracted to reference modules (see Provenance).
