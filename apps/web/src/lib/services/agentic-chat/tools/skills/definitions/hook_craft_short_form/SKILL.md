...
<!--
  ...
  This file is skill_type: procedure (was combo). Two ordered workflows (generate-from-scratch, audit-and-rewrite)
  carry the weight; the hook-craft frameworks (Four Pillars, Visual Layer, BuildOS Voice) are stable declarative
  grounding in Knowledge, and the editorial spine (Core Principles) is Judgment. No Routing block: this is a leaf
  skill with no in-workflow deferrals — sibling boundaries are noted in Activation.
-->
...
This is a **procedure** skill at **task** altitude: two ordered workflows (generate-from-scratch, audit-and-rewrite) do the work, invoking the hook-craft frameworks held in **Knowledge** and the editorial spine held in **Judgment**. Its secondary character is strategy/reference (the Core Principles and Four Pillars), but the dominant verb is _do_.
...
The editorial spine — the heuristics the two workflows reason with when they branch.
...
Two ordered workflows. Both invoke the Four Pillars and the Visual Layer held in **Knowledge**; every step is owned by this skill (no cross-skill deferrals).
...
3. **Pick the archetype** from the available visual + the strength of the contrast (archetype table in **Knowledge** below).
...
12. **Output** all four signals as a labelled bundle (see **Contract**).
...
Provenance: the Four Pillars and the Visual Layer below are **practitioner**-sourced (distilled from Kane Kallaway's hook videos — full lineage in **Provenance**); the BuildOS Voice Translation defaults are **internal-default** (BuildOS's own reasoned register, no external source).
...
Source tags: Kane Kallaway's four videos are the **PRIMARY** creator source (dated analyses linked below); Brendan Kane (_Hook Point_) is a corroborating **practitioner** source; the BuildOS Voice Translation register is **internal-default**.
