...
<!--
  ...
  This file is skill_type: strategy, so Judgment carries the weight (the nine core principles + Pillar 0 content
  games). Procedure is the secondary layer (plan / score / diagnose mode dispatch). No Routing block: the
  procedure routes only to this skill's own reference modules, not to sibling skills — sibling escalation lives
  in Activation. Voluminous, volatile declarative grounding — the matching-loop model, the cross-platform
  signal-weight table, the Keep / Modify / Reject catalog, the dual-audit checklists — lives in the three
  reference modules, each self-provenanced.
-->

<!--
  skill_type: strategy (was: combo). Secondary type: procedure (the plan / score / diagnose mode workflows).
  Dominant verb is DECIDE — the nine core principles, the five content games, the Trust Formula, the
  Keep/Modify/Reject filter, and the dual audit are the spine. NOT orchestration: going_viral is the
  orchestrator that composes this skill.
-->
...
This is a **strategy** skill at **domain** altitude — its spine is decision content (the nine core principles, the five content games, the Trust Formula, the Keep / Modify / Reject filter, and the dual audit), with a secondary procedural layer for the plan / score / diagnose modes. The skill body holds the discipline, the mode dispatch, Pillar 0 (the content-game decision, which applies in every mode), the output contract, and the guardrails. The mode-specific deep rules live in reference modules; load the matching reference before producing decisions.
...
The decision spine. These are the criteria you reason with when the mode dispatch branches.
...
Ordered sequence and intent for the three modes. The reference modules named below are this skill's own
(declared in `reference_modules`), not sibling skills — so ownership resolves in frontmatter, not a Routing block.
...
Deliberately thin. This is a strategy skill; the only stable declarative grounding held here is Pillar 0, stated inline under **Judgment**. Pillar 0 (the five content games, Trust Formula, Off-Target Cost Heuristic, BuildOS #4+#5 hybrid) lives inline in this shell — it loads in every mode, so it is not a reference module. Everything voluminous or volatile is out of this file by design and lives in the three reference modules (see **Provenance**), each self-provenanced.
...
- Full source attribution — Kallaway videos + email [PRIMARY], BuildOS internal essays [internal-default], Galloway / Welsh / DHH sharpeners [practitioner], underlying analyses, combo index — lives in `algorithm_aware_publishing.buildos_dual_audit`.
