...
<!--
  ...
  This file is skill_type: strategy (retyped from the invalid `combo`). The dominant verb is *decide* — a craft
  playbook of principles, six craft moves, and a seven-mistake QA filter. Judgment carries the weight; the two
  workflows are a secondary Procedure, and the three pillars are the declarative Knowledge the decisions reason over.
-->
...
This is a **strategy** skill at **domain** altitude. It owns the structural-craft decision layer between idea and ship. It carries a secondary **procedure** flavor (two ordered workflows) and a substantial **knowledge** base (the three pillars); neither is a genuine split.
...
The Kallaway register skews toward shock and creator-economy hype ("0 to 100K subs," "the algorithm hates you"). BuildOS positions anti-AI, anti-feed, anti-hype. The mechanics hold; the register changes. _[internal-default — BuildOS's own reasoned register calibration.]_
...
Two ordered workflows — one to generate a piece, one to audit an existing draft. Sequence and intent only; routed steps carry a `→ <id>` marker resolved in **Routing**, never re-taught here.

### Workflow: Generate A Piece From Scratch

Given a thesis, target reader, and content type:

1. **Source the inquiry first.** If the founder hasn't done the upstream work, extract the lived material, contradiction, or hard-won opinion this piece will explore. → `nonfiction-writing-from-lived-conviction`
...
11. **Write the hook last.** Once the body and last line exist, write the opener. First line names the topic in 6–8 words. For the full slot grammar / archetype / four-mistake diagnostic on the hook itself → `hook-craft-short-form`.
...
Ownership map for the siblings this skill defers to. One concept, one owner — this skill routes rather than re-teaches.

| Concern (what)                                                        | Owner (who)                                | This skill retains                                              |
| --------------------------------------------------------------------- | ------------------------------------------ | --------------------------------------------------------------- |
| Upstream inquiry — lived material, contradiction, hard-won opinion    | `nonfiction-writing-from-lived-conviction` | using the extracted material as this piece's thesis             |
| Final hook polish — slot grammar, archetypes, four-mistake diagnostic | `hook-craft-short-form`                    | the first-line-names-topic-in-6–8-words rule and hook placement |
| Intent → format decision (the layer above structural craft)           | `content-strategy-beyond-blogging`         | the structural craft layer between idea and ship                |
...
The stable declarative frameworks this skill reasons over: the three pillars and the long-form translation table. _[PRIMARY — all three pillars are distilled from the three Kallaway videos listed in Provenance; the inline `(… via Kallaway)` attributions are preserved verbatim.]_
