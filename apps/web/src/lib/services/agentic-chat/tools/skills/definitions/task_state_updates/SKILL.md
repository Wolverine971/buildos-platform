...
Use this child skill when the user reports progress, completion, or blocking on an existing task and the main risk is choosing the correct `state_key` and update payload.

Do not use this child skill for general task creation, task organization, or broad task planning. Use the `task_management` root skill for those.
...
6. After the write succeeds, summarize the state change and any note that was added.
