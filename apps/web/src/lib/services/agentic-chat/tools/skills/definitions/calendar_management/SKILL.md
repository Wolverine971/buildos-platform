...
Stop conditions before replying: scope was chosen explicitly before the write; start/end times are timezone-safe; update/delete used an exact `onto_event_id` or the inseparable Google pair `{ event_id, calendar_source_id }` discovered from a read rather than guessed; exhaustive requests were verified with the same full lookup after mutation; you have not claimed an event was created, moved, or cancelled until the local mutation and any claimed provider sync have each reached a confirmed terminal success state.

## Policy

- Prefer onto_event_id when available for update/delete.
- For Google events, preserve `calendar_source_id` from the selected list result even when several sources contribute the same provider event. Do not guess from title, account label, `calendar_id`, or a contributing source.
