...
1. Classify the surface and outcome: app screen, dashboard, onboarding flow, marketing page, component library, research plan, design-system work, or implementation verification.
2. Choose one primary child skill and at most two secondary child skills. Load the root source map only when provenance, gap analysis, or child-skill buildout is part of the task.
3. Sequence the review from structure to polish: information architecture first, usability evidence next, accessibility floor, visual craft, context-specific posture, then implementation verification.
4. State the evidence level. Separate source-backed rules, repo/design-system constraints, direct observation, and open gaps.
5. Return the artifact the user needs: audit findings, redesign plan, child-skill routing plan, source gap analysis, or implementation checklist.
