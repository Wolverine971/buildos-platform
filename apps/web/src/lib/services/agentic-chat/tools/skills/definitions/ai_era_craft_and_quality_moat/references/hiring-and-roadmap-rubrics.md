...
- **"Surprise users" — the no-A/B extension.** (Saarinen.) Neither A/B tests _nor_ user research can specify the differentiating move; research tells you what users tolerate, not what makes them say "I didn't know I wanted that." The team's trained intuition generates the surprise. Check: if a roadmap item's entire justification is "research/data says," it is a baseline-meeting item, not a differentiating one — name what the team's judgment adds beyond the data.
...
- **Quality-posture rubric: owner + plan + rationale, or it's aspiration.** (Saarinen, 10 Rules — Rule 1.) A stated commitment to quality is _strategy_ only if it names (1) a **leadership owner** who personally holds the bar, (2) an **implementation plan** for how craft propagates (hiring, design review cadence, who can pull a release), and (3) a **business rationale** for why quality pays here. Missing any one → it's aspiration, log it as such. Saarinen's worked example: Coinbase's design lifting crypto from niche to mainstream approachability is the owner+plan+rationale shape; "we value quality" with no owner is the failure shape.
- **Organic-mentions qualitative north star.** (Saarinen.) The positive replacement for the banned per-feature-WAU metric: success = **the product showing up in organic, unprompted conversations about quality** ("the product just feels right"). It is qualitative and lagging by design — that's the point. If a team demands a leading per-feature number to call quality "working," it has already reintroduced the corrupting metric. Track organic mentions; do not convert them into a per-feature dashboard.
- **Principles-over-process; design review as cadence, not a queue.** (Saarinen, 10 Rules — Rule 9.) Establish values + judgment freedom within a defined standard, not rigid checkpoint stages. Design review runs **weekly + before release** as the project warrants — a recurring cadence, never a gated approval queue that work must wait in. Red flag: a fixed checkpoint queue features must clear (a sign-off gate) — that is process substituting for principle; recommend the weekly + pre-release cadence instead.
