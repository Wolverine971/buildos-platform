...
- Example payload when project context is already known:
  `buildos_call({ op: "onto.task.create", args: { project_id: "4cfdbed1-840a-4fe4-9751-77c7884daa70", title: "Revise chapter 2 dialogue between Elena and Master Thorne", description: "Strengthen the dialogue beats in the Elena and Master Thorne scenes from chapter 2.", type_key: "task.refine" } })`
...
- Example:
  `buildos_call({ op: "onto.task.create", args: { project_id: "4cfdbed1-840a-4fe4-9751-77c7884daa70", title: "Draft chapter 3: Elena's first magical forging attempt", description: "Include Elena's first forging attempt, introduce the Shadow King's herald, and foreshadow the prophecy.", type_key: "task.create" } })`
...
- Example when the task is already in structured context:
  `buildos_call({ op: "onto.task.update", args: { task_id: "440c2639-9000-4111-aeea-ee374f8fb925", state_key: "done" } })`
- Example when the task needs a richer update:
  `buildos_call({ op: "onto.task.update", args: { task_id: "440c2639-9000-4111-aeea-ee374f8fb925", description: "Chapter 1 and chapter 2 are complete. Update the outline to reflect the revised chapter 3 beats and continuity fixes.", update_strategy: "append" } })`
