// apps/web/src/lib/services/agentic-chat/legacy-execution/tool-execution/tool-execution-architecture.test.ts
...
const executionDirectory = resolve(
	process.cwd(),
	'src/lib/services/agentic-chat/legacy-execution/tool-execution'
);
const facadePath = resolve(
	process.cwd(),
	'src/lib/services/agentic-chat/legacy-execution/tool-execution-service.ts'
);
const retiredGenericExecutionDirectory = resolve(
	process.cwd(),
...
describe('tool execution architecture ratchets', () => {
	it('keeps the synchronous host isolated under the explicit legacy namespace', () => {
		expect(existsSync(retiredGenericExecutionDirectory)).toBe(false);
	});
