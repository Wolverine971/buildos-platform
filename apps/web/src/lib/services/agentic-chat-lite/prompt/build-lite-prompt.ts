...
function buildOperatingStrategySection(): LitePromptSection {
	...
	return makeSection({
		...
		content: [
			...
			'- For current or external information the workspace cannot answer (news, prices, competitors, product docs, anything recent or outside BuildOS), call web_search to find sources, then web_visit the most promising URLs to read the actual pages. Prefer primary sources — official sites, vendor pricing pages, documentation — over aggregator or SEO listicle blogs, and verify specific claims (prices, dates, quotes) by visiting the page rather than trusting a search snippet.',
			'- Web research parallelizes: issue several web_search or web_visit calls together in one response and they execute concurrently. When you answer from web results, cite the source URLs; when you save findings into a document, include a Sources section listing the URLs used.',
