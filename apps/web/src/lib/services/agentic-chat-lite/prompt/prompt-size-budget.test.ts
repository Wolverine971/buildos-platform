...
describe('total assembled prompt size budget', () => {
	it('keeps a canonical project turn under the prompt-size budget', () => {
		...
		// Budgets set 2026-07-09 from measured canonical values with ~10%
		// headroom: system_prompt 27,169 chars (~6,793 est tokens), tool
		// definitions 10,571 chars, provider payload 37,797 chars (~9,450 est
		// tokens). Largest static section: the skill catalog at 8,451 chars —
		// the Tier-0 domain diet (fix-plan follow-up) targets it. Real prod
		// turns add live project data and history on top — these budgets guard
		// the template + tool schemas, which is the part that drifts silently.
		expect(breakdown.system_prompt.chars).toBeLessThanOrEqual(30_000);
		expect(breakdown.provider_payload_estimate.chars).toBeLessThanOrEqual(41_500);
		expect(breakdown.provider_payload_estimate.est_tokens).toBeLessThanOrEqual(10_400);
	});
});
