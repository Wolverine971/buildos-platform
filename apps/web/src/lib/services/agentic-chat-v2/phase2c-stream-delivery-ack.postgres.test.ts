...
describePostgres('agentic-chat worker Phase 2C stream-delivery acknowledgement contract', () => {
	let tempDir = '';
	let dataDir = '';
	let socketDir = '';
	let port = 0;
	...
	beforeAll(async () => {
		tempDir = mkdtempSync('/tmp/buildos-phase2c-stream-ack-pg-');
	...
	it('passes exact-sequence fencing, ownership, security, replay, and rollback checks', () => {
		expect(proofOutput).toContain('phase2c_stream_delivery_ack_ok');
	});
});
