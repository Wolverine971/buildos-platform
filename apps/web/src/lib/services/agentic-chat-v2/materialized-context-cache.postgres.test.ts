// apps/web/src/lib/services/agentic-chat-v2/materialized-context-cache.postgres.test.ts
...
describePostgres('agentic-chat materialized context cache database contract', () => {
	let tempDir = '';
	let dataDir = '';
	let socketDir = '';
	let port = 0;
	...
	beforeAll(async () => {
		tempDir = mkdtempSync('/tmp/buildos-agentic-materialized-context-pg-');
		...
		applySqlFile(
			sqlPath('supabase/tests/fixtures/agentic_chat_materialized_context_cache_base.sql')
		);
	...
	it('passes generation, shared-project fan-out, removal, and ACL checks', () => {
		expect(proofOutput).toContain('agentic_chat_materialized_context_cache_ok');
	});
});
