...
describePostgres('agentic-chat P4 supervisor-question checkpoint PostgreSQL contract', () => {
	let tempDir = '';
	let dataDir = '';
	let socketDir = '';
	let port = 0;
	...
	beforeAll(async () => {
		tempDir = mkdtempSync('/tmp/buildos-p4-supervisor-checkpoint-pg-');
		...
		applySqlFile(
			sqlPath('supabase/tests/fixtures/agentic_chat_supervisor_checkpoint_base.sql')
		);
		applySqlFile(
			sqlPath(
	...
	it('passes exact replay, legacy compatibility, security, and turn/job fence checks', () => {
		expect(output).toContain('agentic_chat_supervisor_question_checkpoint_ok');
	});
});
