...
describePostgres('agentic-chat worker Phase 2A trust foundation PostgreSQL contract', () => {
	let tempDir = '';
	let dataDir = '';
	let socketDir = '';
	let port = 0;
	...
	beforeAll(async () => {
		tempDir = mkdtempSync('/tmp/buildos-phase2a-pg-');
	...
	it('passes legacy compatibility, immutability, scope, retention, and privilege checks', () => {
		expect(fixtureOutput).toContain('phase2a_trust_foundation_ok');
	});
});
