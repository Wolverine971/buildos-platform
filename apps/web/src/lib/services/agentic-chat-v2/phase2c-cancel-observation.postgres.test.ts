...
describePostgres('agentic-chat worker Phase 2C batched cancellation observation contract', () => {
	let tempDir = '';
	let dataDir = '';
	let socketDir = '';
	let port = 0;
	...
	beforeAll(async () => {
		tempDir = mkdtempSync('/tmp/buildos-phase2c-cancel-observer-pg-');
		...
		proofOutput = applySqlFile(
			sqlPath('supabase/tests/20260802035000_agentic_chat_worker_cancel_observation.test.sql')
		);
	}, 60_000);
	...
	it('passes batching, generation, security, replay, concurrency, and rollback checks', () => {
		expect(proofOutput).toContain('phase2c_cancel_observation_ok');
	});
});
