...
describePostgres('agentic-chat worker Phase 2C reconciliation contract', () => {
	let tempDir = '';
	let dataDir = '';
	let socketDir = '';
	let port = 0;
	...
	beforeAll(async () => {
		tempDir = mkdtempSync('/tmp/buildos-phase2c-reconciliation-pg-');
		...
		proofOutput = applySqlFile(
			sqlPath('supabase/tests/20260802037000_agentic_chat_worker_reconciliation.test.sql')
		);
	}, 60_000);
	...
	it('passes ownership, generation, boundedness, terminal, security, race, and rollback checks', () => {
		expect(proofOutput).toContain('phase2c_reconciliation_ok');
	});
});
