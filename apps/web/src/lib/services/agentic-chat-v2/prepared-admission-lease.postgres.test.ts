// apps/web/src/lib/services/agentic-chat-v2/prepared-admission-lease.postgres.test.ts
...
describePostgres('agentic-chat prepared-admission lease database contract', () => {
	let tempDir = '';
	let dataDir = '';
	let socketDir = '';
	let port = 0;
	...
	beforeAll(async () => {
		tempDir = mkdtempSync('/tmp/buildos-agentic-prepared-admission-pg-');
	...
	it('passes hit, nonce, history, checkpoint, privilege, and idempotence checks', () => {
		expect(proofOutput).toContain('agentic_chat_prepared_admission_lease_ok');
	});
});
