...
describePostgres('agentic-chat worker Phase 2B effect RPC PostgreSQL contract', () => {
	let tempDir = '';
	let dataDir = '';
	let socketDir = '';
	let port = 0;
	...
	beforeAll(async () => {
		tempDir = mkdtempSync('/tmp/buildos-phase2b-effect-rpcs-pg-');
		...
		fixtureOutput = applySqlFile(
			sqlPath('supabase/tests/20260801041100_agentic_chat_worker_effect_rpcs.test.sql')
		);
	}, 30_000);
	...
	it('passes security, idempotency, fencing, cancellation, concurrency, reconciliation, and rollback checks', () => {
		expect(fixtureOutput).toContain('phase2b_effect_rpcs_ok');
	});
});
