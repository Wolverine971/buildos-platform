...
describePostgres('agentic-chat P4 immutable checkpoint-resume PostgreSQL contract', () => {
	let tempDir = '';
	let dataDir = '';
	let socketDir = '';
	let port = 0;
	...
	beforeAll(async () => {
		tempDir = mkdtempSync('/tmp/buildos-p4-checkpoint-resume-pg-');
		...
		applySqlFile(sqlPath('supabase/tests/fixtures/agentic_chat_checkpoint_resume_base.sql'));
		applySqlFile(
			sqlPath(
	...
	it('passes atomic claim, drift, terminal, recovery, and security checks', () => {
		expect(output).toContain('agentic_chat_checkpoint_resume_lifecycle_ok');
	});
});
