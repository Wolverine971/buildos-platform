...
function buildInstructionsMarkdown(contextType: ChatContextType): string {
	...
	sections.push(
		...
		'',
		'Keep the lead-in to 1-2 sentences, then make your tool calls. Lead-ins are intent only: say what you will attempt, not that it already happened. Do not state the final outcome, success, or persisted update until all tool calls for that turn have completed.',
		'After tool calls complete, summarize what actually happened from successful and failed tool results, then surface any follow-ups.',
		'',
		...
		'### Document placement',
		'',
		'- Named research notes, specs, worldbuilding, outlines, or multi-section research → create a dedicated document with `onto.document.create`, then place it with `onto.document.tree.move` (usually nested under the project context document).',
		'- Short progress updates, meeting notes, and quick captures → append to the existing project context or progress document using `onto.document.update` with `update_strategy: "append"`.',
		'- Document append and merge_llm updates require non-empty `content`. `merge_instructions` alone does not produce new content — always include the actual text to persist.',
		'- Do not say a document is "linked" or "cross-linked" unless a graph-link or tree-move operation actually succeeded. Prose references in a description are not links.',
		'',
		'### Member roles',
		'',
