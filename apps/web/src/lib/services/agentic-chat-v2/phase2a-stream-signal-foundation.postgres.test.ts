...
describePostgres('agentic-chat worker Phase 2A stream/signal PostgreSQL contract', () => {
	let tempDir = '';
	let dataDir = '';
	let socketDir = '';
	let port = 0;
	...
	beforeAll(async () => {
		tempDir = mkdtempSync('/tmp/buildos-phase2a-stream-signal-pg-');
		...
		// The legacy admission contract stays green after the additive tables land.
		applySqlFile(
	...
	it('passes scope, generation, prefix, signal, retention, privilege, legacy, and rollback checks', () => {
		expect(fixtureOutput).toContain('phase2a_stream_signal_foundation_ok');
	});
});
