// apps/web/src/lib/services/agentic-chat-v2/input-v3-lifecycle-snapshots.postgres.test.ts
...
describePostgres('agentic-chat input v3 lifecycle snapshot PostgreSQL contract', () => {
	let tempDir = '';
	let dataDir = '';
	let socketDir = '';
	let port = 0;
	...
	beforeAll(async () => {
		tempDir = mkdtempSync('/tmp/buildos-input-v3-lifecycle-pg-');
	...
	it('preserves rolling v2 rows and upgrades only complete immutable snapshot inputs', () => {
		expect(output).toContain('phase4_slice4_input_v3_lifecycle_snapshots_ok');
	});
});
