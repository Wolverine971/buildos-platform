...
export class GoogleCalendarConnectionService {
	...
	async registerCreatedSource(params: {
		userId: string;
		connectionId: string;
		...
		const source = firstRow(data as SourceRow | SourceRow[] | null);
		if (error || !source) {
			throw new GoogleCalendarConnectionError(
				'database_error',
		...
		return this.toSourceSummary(source, null);
	}
	...
	async reconcileDefaultWriteSource(userId: string): Promise<string | null> {
		...
		if (connectionIds.length > 0) {
			...
			eligibleSources = (sourceData ?? []) as SourceRow[];
		}
