...
AUDIT 2026-04-29
QUALITY: 8/10
RECOMMENDATION: KEEP_AS_IS (already published) — but resolve the agent-skills folder strategy question
PURPOSE: A portable Google Calendar SKILL.md for AI agents. Operating rules: choose scope, search before create, use exact IDs, treat recurrence as high-risk. Distill BuildOS's own calendar-integration learnings into a reusable skill spec.
READER VALUE: High for engineers wiring up calendar tools. Concrete failure modes (duplicate events, wrong calendar, mangled recurrence), three good worked examples, optional buildos.yaml marketplace metadata. The "search before create" rule is genuinely the right primitive.
VOICE FIT: Authoritative engineering voice. Less literary than the philosophy posts but appropriate for the genre. No anti-AI tension because this is a pure how-to-build-with-AI post — readers showed up wanting that.
PLACEMENT: Right folder for the agent-skill content type, but the folder still needs an editorial thesis and a hub. Cross-link with the harness-debug skill — they're stack-mates per their own metadata but neither links to the other.
ISSUES:
- No cross-link to its sibling (debug-the-harness-not-the-model.md).
- Same custom-frontmatter sprawl as the sibling (skillId, providers, compatibleAgents, stackWith) — fine if there's a route consuming it; otherwise dead metadata.
- The post veers between two readers (engineer copying a SKILL.md and a thoughtful founder reading about safe calendar agents). Worth picking one and writing toward it.
- "How BuildOS uses this pattern" section is short — could expand with a concrete BuildOS calendar bug we hit and fixed.
- The closing "What to build next" reads like a stub of a future content series; either commit to the series (Gmail, Drive, Docs skills) or remove the tease.
GAPS:
- No mention of consent/auth scope minimization, which matters for product-trust positioning.
- No companion BuildOS-specific blog explaining how BuildOS uses calendar context as part of the daily brief — that's the bridge to the rest of the catalog.
DUPLICATES/OVERLAP: None in current catalog. Sole authoritative voice on calendar-agent design.
NOTES: This is the strongest agent-skills file. If the agent-skills folder is going to exist, this post should anchor it and the harness-debug post should follow. Need 3-5 more (Gmail safe sending, Drive context retrieval, OAuth for agents, Founder Assistant Stack) to make the folder feel complete.
-->
