---
title: 'The 6 Hook Archetypes: Lessons from Kane Kallaway'
description: 'A deep read of Kallaway''s "I Studied 100 Viral Hooks, These 6 Will Make You Go Viral" — the six archetypes (Fortune Teller, Experimenter, Teacher, Magician, Investigator, Contrarian), the four-component alignment, and the visual → audio → visual sandwich.'
author: 'DJ Wayne'
date: '2026-05-04'
lastmod: '2026-05-04'
changefreq: 'monthly'
...
sourceUrl: 'https://www.youtube.com/watch?v=xnOe8aA9Pmw'
sourceChannelUrl: 'https://www.youtube.com/@kallawaymarketing'
lineagePeople:
    - 'Kane Kallaway'
lineageSources:
...
path: apps/web/src/content/blogs/source-analyses/kallaway-100-viral-hooks.md
---

# The 6 Hook Archetypes: Lessons from Kane Kallaway
...
**Mechanism:** Peer-to-peer demo. One friend showing another what they tried and what happened. The visual _is_ the demo.
...
- "Does anything look weird about this shot to you? I actually don't have a camera in my hands. I'm recording with these glasses on my face." (Colin & Samir Ray-Ban Meta — 50M views; the glasses' camera is _the subject being filmed with_, so visual and verbal hook self-prove.)
...
**Mechanism:** Same contrast spine as Experimenter, but framed teacher-to-student instead of peer-to-peer. The proof is _extracted lessons_, not a live experiment.
...
**Critical:** Magician is a _modifier_, not a standalone choice. Without a contrast layer it is empty spectacle.

### 5. The Investigator

**Mechanism:** Contrast against an _unknown secret_. Curiosity loop = "today you don't know X; I show you X; now you know."
...
1. **Spoken hook** — what you say in the voiceover.
2. **Visual hook** — what is on screen.
3. **Text hook** — text overlay on the screen.
4. **Audio hook** — music / SFX in the background.
...
1. Eyes process visual + read text overlay.
2. Ears catch up and parse the spoken hook.
3. Eyes return to the visual seeking confirmation of what they just heard.
...
1. **Inventory the visual.** What b-roll, demo footage, motion graphics, or stock do you actually have? If the visual is weak, decide whether to manufacture one or **kill the video idea entirely**.
2. **Find the biggest contrast.** Of all the angles available, which produces the largest A → B gap? That contrast picks the archetype.
3. **Write the spoken hook.** Two to four lines following the **context-lean → contrast → contrarian snapback** pattern.
4. **Layer the text overlay.** Reinforce the visual on the way in and on the visual-confirmation rebound.
5. **Gut-check max comprehension.** Watch silent → watch with audio → ask: when the viewer goes visual → audio → visual, do all four hook components agree? If not, **stop and rewrite, or kill the idea**.
...
- **Inherent contrast:** never been seen before — the visual alone _is_ the contrast.
...
- **Why it failed:**
    - "Future of storytelling" is too abstract — the visual (rooms) doesn't visually _mean_ "storytelling."
...
- **Making the video when the visual is weak.** If you don't have a strong key visual, _kill the video_.
