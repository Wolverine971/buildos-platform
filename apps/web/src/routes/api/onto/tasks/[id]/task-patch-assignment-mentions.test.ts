// apps/web/src/routes/api/onto/tasks/[id]/task-patch-assignment-mentions.test.ts
		...
		if (this.table === 'onto_tasks' && this.action === 'update') {
			return {
				data: {
					id: 'task-1',
					project_id: 'project-1',
					title: this.updatePayload?.title ?? 'Old title',
					description: this.updatePayload?.description ?? 'Old description',
					type_key: 'task.default',
					state_key: this.updatePayload?.state_key ?? 'todo',
					props: {}
				},
				error: null
			};
...
describe('PATCH /api/onto/tasks/[id] assignment + mention coalescing', () => {
	...
	it('coalesces mention notifications when the same user is newly assigned', async () => {
		...
		const response = await PATCH({
			...
			request: new Request('http://localhost/api/onto/tasks/task-1', {
				...
				body: JSON.stringify({
					description: 'Updated [[user:user-assignee|Sam]] [[user:user-mentioned|Jo]]'
				})
			}),
			locals: {
