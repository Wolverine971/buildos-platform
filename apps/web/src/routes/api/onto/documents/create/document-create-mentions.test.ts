// apps/web/src/routes/api/onto/documents/create/document-create-mentions.test.ts
...
describe('POST /api/onto/documents/create mention notifications', () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	it('notifies mentions found in title/description/content', async () => {
		const { POST } = await import('./+server');
		const response = await POST({
			request: new Request('http://localhost/api/onto/documents/create', {
				method: 'POST',
				...
				body: JSON.stringify({
					...
					content: 'Body [[user:user-mentioned|Jo]]'
				})
			}),
			locals: {
		...
		expect(resolveEntityMentionUserIdsMock).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				nextTextValues: ['Doc title', 'Overview', 'Body [[user:user-mentioned|Jo]]']
			})
		);
