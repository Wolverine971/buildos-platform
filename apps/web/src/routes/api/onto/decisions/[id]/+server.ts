...
export const GET: RequestHandler = async ({ params, locals }) => {
	...
	try {
		...
		const { data: hasAccess, error: accessError } = await supabase.rpc(
			...
			{
				p_project_id: decision.project.id,
				p_required_access: 'read'
			}
		);

		if (accessError) {
			console.error('[Decision GET] Failed to check access:', accessError);
			await logOntologyApiError({
				supabase,
				error: accessError,
				endpoint: `/api/onto/decisions/${params.id}`,
...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
	...
	try {
		...
		const { data: hasAccess, error: accessError } = await supabase.rpc(
			'current_actor_has_project_access',
			{
				p_project_id: existingDecision.project.id,
				p_required_access: 'write'
			}
		);

		if (accessError) {
			console.error('[Decision PATCH] Failed to check access:', accessError);
			await logOntologyApiError({
				supabase,
				error: accessError,
				endpoint: `/api/onto/decisions/${params.id}`,
...
export const DELETE: RequestHandler = async ({ params, request, locals }) => {
	...
	try {
		...
		if (accessError) {
			console.error('[Decision DELETE] Failed to check access:', accessError);
			await logOntologyApiError({
				supabase,
				error: accessError,
				endpoint: `/api/onto/decisions/${params.id}`,
