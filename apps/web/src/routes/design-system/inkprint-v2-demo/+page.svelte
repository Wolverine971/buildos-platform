			...
			<div class="max-w-6xl mx-auto px-4 py-2 flex items-center justify-between">
				...
				<div class="flex items-center gap-3">
					<label class="flex items-center gap-2 text-sm cursor-pointer">
						<input type="checkbox" bind:checked={darkMode} class="rounded border-border" />
		...
		<div class="max-w-6xl mx-auto px-4 py-6">
			...
			<header class="mb-6">
				<div class="flex items-start justify-between gap-4">
					<div class="min-w-0">
						<div class="flex items-center gap-3 mb-1">
							<h1 class="text-2xl font-bold text-foreground truncate">{project.name}</h1>
							<span class="px-2 py-0.5 text-xs font-medium rounded-full bg-emerald-500/10 text-emerald-600 capitalize">
								{project.status}
							</span>
						</div>
				...
				<div class="mt-4 p-3 rounded-lg bg-accent/5 border border-accent/20">
					<div class="flex items-center gap-3">
						...
						<div>
							<p class="text-[10px] uppercase tracking-wider text-accent font-medium">Next Step</p>
			...
			<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
				...
				<div class="lg:col-span-2 space-y-6">
					...
					<!-- ============================================ -->
					<div class="relative overflow-hidden rounded-xl border border-dashed border-accent/40 bg-accent/5 p-4">
						...
						<div class="relative z-10 flex items-start gap-3">
							...
							<div class="flex-1">
								<p class="text-xs font-medium text-accent uppercase tracking-wider">AI Suggestion</p>
								<p class="text-sm text-foreground mt-1">
									Based on your progress, the "Write documentation" task could be broken into smaller chunks. Want me to help?
								</p>
								<div class="flex gap-2 mt-3">
									<button class="px-3 py-1.5 text-xs font-medium bg-accent text-accent-foreground rounded-lg pressable">
										Yes, help me
									</button>
									<button class="px-3 py-1.5 text-xs font-medium border border-border rounded-lg hover:bg-muted transition-colors">
										Dismiss
									</button>
								</div>
							</div>
					...
					<!-- ============================================ -->
					<div class="relative overflow-hidden rounded-xl border-2 border-amber-500/50 bg-amber-500/5 p-4">
						...
						<div class="relative z-10 flex items-center gap-3">
							...
							<div class="flex-1">
								...
								<p class="text-sm font-medium text-foreground mt-0.5">{urgentDeadline.title}</p>
								<p class="text-xs text-muted-foreground">Deadline: {urgentDeadline.dueTime}</p>
							</div>
							<button class="px-4 py-2 text-sm font-medium bg-amber-500 text-white rounded-lg pressable">
								Start Now
							</button>
						</div>
					</div>
					...
					<!-- ============================================ -->
					<section class="bg-card border border-border rounded-xl shadow-ink overflow-hidden">
						<!-- Section Header -->
						<div
							role="button"
							tabindex="0"
							onclick={() => toggleSection('tasks')}
							onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleSection('tasks'); } }}
						...
						>
							<div class="flex items-center gap-3">
								...
								<div class="text-left">
									...
									<p class="text-xs text-muted-foreground">{tasks.length} items</p>
								</div>
							</div>
							<div class="flex items-center gap-2">
								<button
									onclick={(e) => { e.stopPropagation(); showCreateModal = true; }}
									class="p-1.5 rounded-md hover:bg-muted transition-colors"
								>
									<Plus class="w-4 h-4 text-muted-foreground" />
								</button>
								<ChevronDown class="w-4 h-4 text-muted-foreground transition-transform {expandedSections.tasks ? '' : '-rotate-90'}" />
							</div>
						</div>
							...
							<div class="border-t border-border">
								<ul class="divide-y divide-border">
									{#each tasks as task}
										...
										<li>
											<button class="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-muted/30 transition-colors">
												<svelte:component this={style.icon} class="w-4 h-4 {style.color} shrink-0" />
												<span class="flex-1 text-sm text-foreground truncate">{task.title}</span>
												<span class="text-xs text-muted-foreground capitalize px-2 py-0.5 rounded bg-muted">
													{task.priority}
												</span>
											</button>
										</li>
					...
					<!-- ============================================ -->
					<div class="relative overflow-hidden rounded-xl border border-destructive/30 bg-destructive/5 p-4">
						...
						<div class="relative z-10 flex items-start gap-3">
							...
							<div>
								<p class="text-sm font-medium text-destructive">Calendar Sync Failed</p>
								<p class="text-xs text-muted-foreground mt-1">
									Unable to sync with Google Calendar. Your recent changes may not be saved.
								</p>
								<div class="flex gap-2 mt-3">
									<button class="px-3 py-1.5 text-xs font-medium bg-destructive text-destructive-foreground rounded-lg pressable">
										Retry
									</button>
					...
					<section class="bg-card border border-border rounded-xl shadow-ink overflow-hidden">
						...
						<div class="flex items-center justify-between gap-3 px-4 py-3">
							<div
								...
								onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleSection('documents'); } }}
							...
							>
								...
								<div class="text-left">
									...
									<p class="text-xs text-muted-foreground">{documents.length} files</p>
								</div>
							</div>
							<div class="flex items-center gap-2">
								<button
								...
								<button
									...
									class="p-1.5 rounded-md hover:bg-muted transition-colors"
								>
									<ChevronDown class="w-4 h-4 text-muted-foreground transition-transform {expandedSections.documents ? '' : '-rotate-90'}" />
								</button>
							</div>
						</div>
									...
									{#each documents as doc}
										<li>
											<button class="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-muted/30 transition-colors">
												...
												<span class="flex-1 text-sm text-foreground truncate">{doc.title}</span>
												<span class="text-xs text-muted-foreground capitalize">{doc.type}</span>
											</button>
										</li>
									{/each}
								</ul>
				...
				<aside class="space-y-4">
					...
					<section class="bg-card border border-border rounded-xl shadow-ink overflow-hidden">
						...
						<div
							...
							onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleSection('goals'); } }}
							class="w-full flex items-center justify-between gap-3 px-4 py-3 hover:bg-muted/50 transition-colors cursor-pointer"
						>
							<div class="flex items-center gap-3">
								...
								<div class="text-left">
									...
									<p class="text-xs text-muted-foreground">{goals.length} active</p>
								</div>
							</div>
							<ChevronDown class="w-4 h-4 text-muted-foreground transition-transform {expandedSections.goals ? '' : '-rotate-90'}" />
						</div>
					...
					<div class="bg-card border border-border rounded-xl shadow-ink p-4">
						<h3 class="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-3">Quick Actions</h3>
						<div class="space-y-2">
							<button
							...
							<button class="w-full flex items-center gap-3 px-3 py-2 text-sm border border-border rounded-lg hover:bg-muted transition-colors">
					...
					<div class="p-4 rounded-xl bg-muted/50 border border-border">
						...
						<div class="space-y-3 text-xs text-muted-foreground">
							...
							<div class="pt-2 border-t border-border">
								<p class="italic">Everything else: <span class="text-foreground font-medium">CLEAN</span></p>
							</div>
						</div>
					</div>
				</aside>
...
{#if showCreateModal}
	<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
		...
		<!-- Modal - CLEAN design -->
		<div class="relative w-full max-w-md bg-card border border-border rounded-xl shadow-ink-strong">
			<!-- Header -->
			...
			<div class="p-4 space-y-4">
				...
				<div class="grid grid-cols-2 gap-3">
					<div class="space-y-1.5">
						...
						<select class="w-full px-3 py-2 text-sm bg-background border border-border rounded-lg shadow-ink-inner text-foreground outline-none">
							<option>Low</option>
			...
			<!-- Footer -->
			<div class="flex items-center justify-end gap-2 px-4 py-3 border-t border-border bg-muted/30 rounded-b-xl">
				<button
					onclick={() => (showCreateModal = false)}
...
{#if showDeleteModal}
	<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
		...
		<div class="relative w-full max-w-sm bg-card border border-destructive/30 rounded-xl shadow-ink-strong overflow-hidden">
			<div
				class="absolute inset-0 pointer-events-none opacity-[0.15]"
				style="background-image: url('/textures/noisy.png'); background-repeat: repeat;"
			></div>
			...
			<div class="relative z-10 p-4">
				<div class="flex items-start gap-3">
					<div class="w-10 h-10 rounded-lg bg-destructive/10 flex items-center justify-center shrink-0">
