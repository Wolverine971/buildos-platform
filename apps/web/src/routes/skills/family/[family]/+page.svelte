...
<script lang="ts">
	...
	let filteredTrees = $derived.by(() => {
		if (!normalizedQuery) return data.trees;
		...
		return JSON.stringify(
			{
				...
				description: `Browse ${family.name} workflows, public skills, and child skill paths.`,
				url: familyUrl,
				isPartOf: {
				...
				mainEntity: {
					'@type': 'ItemList',
					numberOfItems: family.skills.length,
					itemListElement: family.skills.map((skill, index) => ({
						'@type': 'ListItem',
						position: index + 1,
						item: {
							'@type': 'CreativeWork',
...
<svelte:head>
	...
	<meta
		name="description"
		content={`Browse ${family.name} workflows, public skills, and child skill paths.`}
	/>
			...
			<div class="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_24rem] lg:items-end">
				<div class="max-w-4xl">
					...
					<p class="mt-4 max-w-3xl text-base leading-7 text-muted-foreground sm:text-lg">
						Follow the public workflows in this family, then move into child skills when
						the job needs a narrower operating procedure.
					</p>
					{#if startSkill}
				...
				<div class="grid grid-cols-2 gap-2">
					...
					<div
						class="rounded-lg border border-border bg-background/85 p-3 shadow-ink-inner"
					>
						<p class="text-xs text-muted-foreground">Child skills</p>
						<p class="mt-1 text-2xl font-semibold">{childCount}</p>
					</div>
					<div
						class="rounded-lg border border-border bg-background/85 p-3 shadow-ink-inner"
					>
			...
			<div class="space-y-10">
				<section aria-labelledby="family-skills">
					...
					{:else}
						...
						>
							No public skill title or description matches this search. Child-skill
							matches still appear below.
						</p>
					{/if}
				</section>
