...
class RecurringTaskCalendarMigration {
	...
	private async getPendingMigrations(): Promise<MigrationTask[]> {
		...
		return (data || []).map((item: any) => {
			// Handle task_calendar_events which could be array or single object
			let calendarEventId: string | undefined;
			if (item.task_calendar_events) {
				if (Array.isArray(item.task_calendar_events)) {
					calendarEventId = item.task_calendar_events[0]?.calendar_event_id;
				} else {
					calendarEventId = item.task_calendar_events.calendar_event_id;
				}
			}
	...
	private async migrateTask(task: MigrationTask): Promise<void> {
		try {
			...
			const auth = await this.oAuthService.getAuthenticatedClient(task.user_id);
			if (!auth) {
