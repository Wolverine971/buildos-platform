...
- add write tools only on explicit user intent
- avoid heavy tool catalogs by default
...
**Tier 2 — Write (explicit action only)**
...
### 6.3 Write Tools (FastChat Action Mode)

Only attach these when the user explicitly requests creation or updates:
...
| project_audit | 84 | 3752 | 27 |
| project_forecast | 85 | 3752 | 27 |
| calendar | 93 | 596 | 3 |
...
- Tool schema cost dominates; keep tool pools minimal.
...
2. **Defer write tools** until explicit action intent is detected.
