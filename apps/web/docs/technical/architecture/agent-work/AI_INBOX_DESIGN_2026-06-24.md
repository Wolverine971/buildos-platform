...
Make Project Review, Calendar Analysis, and signal review producers emit `agent_runs` review runs so every item uses one payload shape and one commit path.
...
### Project Review And Audit Alignment Decision

Project Reviews need two levels of product artifact:
...
- Project Reviews should not be forced wholesale into true `ChangeSet`s. Mutating child suggestions can converge toward `ChangeSet`s later; audit/report packets should remain report artifacts.
...
3. **First mutation convergence target:** should Calendar Analysis migrate to true `ChangeSet`s before mutating Project Review suggestions, since calendar accept/reject is a narrower create-project/tasks flow?
...
- **Phase 6 - Mutation convergence:** migrate narrow mutating producers toward true `ChangeSet`s where per-change review is valuable. Calendar Analysis is the likely first candidate; mutating Project Review suggestions can follow. Audit packets should not become `ChangeSet`s.
...
3. **Chat about inbox items.**
    ...
    - 2026-06-30 update: proposal context now has two carriers. The visible assistant seed keeps the user oriented in the restored chat, and the stream endpoint injects `chat_sessions.agent_metadata.proposal_context.llm_text` as a transient system history message for `source:'ai_inbox'` sessions so the model is anchored to the active proposal even when ordinary project/calendar context is sparse. Calendar-suggestion seeds keep raw `calendar_event_ids` in hidden prompt metadata for evidence follow-up, while the visible seed shows readable event titles when `calendar_analysis_events` rows are available.
    - 2026-06-30 prompt-audit update: calendar suggestion chats now use a shorter user-facing seed that states what accepting does, strips generated markdown heading artifacts from visible/hidden context, normalizes display titles such as `9takes Project` to `9 Takes`, and keeps internal event IDs out of the chat body. Reopening an existing inbox chat refreshes the stored seed message and session metadata so older sessions can pick up the cleaned prompt.
...
6. **Design the Complete Project Audit artifact.**
    - Implemented 2026-07-01 design spec: see `COMPLETE_PROJECT_AUDIT_TRACKER_SPEC_2026-07-01.md`.
...
8. **Choose the first true `ChangeSet` migration.**
    ...
    - Revisit mutating Project Review suggestions after audit packets and parent run context are clean.
