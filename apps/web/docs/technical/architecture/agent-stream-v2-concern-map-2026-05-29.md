...
A from-scratch spine can own **A + the seams for D**, but it must *call into* B and C rather than re-deriving them. `djtryserver.ts` looked clean only because it implemented one-third of A and none of C.
