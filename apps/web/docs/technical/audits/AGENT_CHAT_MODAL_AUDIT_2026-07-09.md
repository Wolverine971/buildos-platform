...
**Wave 4 — cleanup: ✅ DONE 2026-07-09** (uncommitted; 256/256 agent tests + typecheck 0 errors/0 warnings). Every "grep-verified" dead-code claim was re-verified against the current tree before deletion — three audit claims did **not** hold and were corrected rather than executed blindly (see below).

- **4.1 camelCase deletion** → removed the dead camelCase fallbacks in `agent-chat-sse-handler.ts` (`computeToolResultInfo` / `sanitizeToolResultForActivityMetadata`) and `AgentChatModal.svelte` (`updateActivityStatus`, 9 ladders + the `.data`/`.tool_result` legacy aliases). **Correction:** the audit's third target `agent-chat-timeline.ts` was NOT dead — that code reads client-built `activity.metadata`, which is camelCase (the audit conflated the DB-row snake path with the thinking-block metadata path). Left intact. Net ~40 lines, not ~100. 3 stale test fixtures (camelCase `toolName` in `tool_result` events) migrated to the real wire format `tool_name`.
- **4.3 dead SSE handlers** → deleted the client cases for `text`, `clarifying_questions`, `focus_active`, `focus_changed`, `operation` (server-emit-verified: none reachable; `operation` in the type union + phase classifier were phantoms, not emitters), plus `addClarifyingQuestionsMessage`, `upsertOperationActivity`, `computeClarifyingQuestionsCount`, `formatOperationEvent` + orphans (`OPERATION_VERBS`, `capitalizeWord`), and the whole `agent-chat-operation-activity.ts` (+ test). **Correction:** `logFocusActivity` the _function_ was KEPT — it has live non-SSE callers in the shell router (`'Focus updated'` / `'Focus reset'`); only the SSE-side plumbing was removed. The shared-types `AgentSSEMessage` / unused `FastAgentStreamEvent` **union narrowing is deferred** — cross-package churn (worker-shared) for harmless type surface the runtime `default` case already absorbs.
- **4.4 dead state/exports** → removed `ontologyLoaded` (write-only-false; ONTO badge unreachable) across modal + header; the entire hidden-tool machinery (`normalizeToolDisplayPayload` hardcoded `hidden:false` → `hiddenToolCallIds` never populated; narrowed `ToolCallActivityBuildResult.activity` to non-null); fully-dead exports (`CONTEXT_BADGE_CLASSES`, `DEFAULT_CONTEXT_BADGE_CLASS`, `formatCompressionTimestamp`, `formatListPreview`, `hasActiveTransport`); dropped `export` on 10 internal-only symbols; deduped `ActivityUpdateResult` (import from sse-handler), `PREPARED_PROMPT_SEND_WAIT_MS` (→ shared `agent-chat.constants.ts`), and the voice-note comparator (→ `compareVoiceNotesInGroup` in `voice-notes.ts`). **Corrections/skips:** the `detachActiveStream` reconcile branch + `hasMultipleAgentHelpers` cluster are a turned-off _feature_ (multi-agent helpers), tested, and `researchAgentId` is actually **live** — left intact, not deleted. ThinkingBlock `prefix`/inert-color was already fixed in Wave 3. The "UUID regex ×3" claim doesn't exist in the agent dir. `isRecord/stringValue/numberValue` NOT merged (`numberValue` diverges: `parseFloat` vs `Number`).
- **4.2 formatter-table merge** → `agent-chat-tool-presenter.ts` is now the single source of tool-display truth via a `describeToolDisplay(toolName, args, opts?)` factory method; `formatToolMessage` delegates to it with no opts so the **live path is byte-identical**. The restored path (`agent-chat-session.ts`) creates a cache-free presenter and calls `describeToolDisplay` with `{ resultRecord, genericPrefix, gatewayOp }`, applying tense via exported `toPastTenseAction`/`toFailureAction`. The ~430-line restored fork was deleted (**session.ts 1281 → 886 lines**). Fixes the 5 tools the fork mislabeled on restore (`search_project` scoping, `get_field_info`, 3 contact tools) — new restored-path tests pin them. No pinned test changed.
- **4.5 structural simplification** → **4.5f done** (the verbatim desktop/mobile export-menu rows in `AgentChatHeader.svelte` collapsed into one parameterized `{#snippet exportMenuRows(closeMenu)}`). The rest of 4.5 (stream-controller teardown `#finishRun` consolidation, `StreamControllerDeps`/`SSEHandlerDeps` narrowing, shellRouter wrapper/derived inlining, `AgentComposer` bindable relay, modal extractions) is **deferred**: these are behaviour-adjacent refactors of _working, tested_ flagship code with subtle reactive/teardown-ordering/referential-stability implications (e.g. inlining `resolvedProjectFocus` reintroduces the instability its `$derived` wrapper prevents; the router-side fix is coupled) — best done with live verification, and the audit's own "don't churn" list warns against exactly this.

Test surface: controllers all have sibling tests; every Wave-4 change landed inside tested modules and extended those tests. `agent-chat-operation-activity.ts` + test deleted.
