...
| Admin chat user performance          | [`ADMIN_CHAT_USER_PERFORMANCE_AUDIT_2026-07-05.md`](./ADMIN_CHAT_USER_PERFORMANCE_AUDIT_2026-07-05.md) | 2026-07-05 | 🔶 Phase 1 partial shipped: icon wrapper import, `.micro-label` sweep, reduced-motion skeletons, encoded fetch URLs, stale request guards, redacted-session slow-threshold propagation + tests. **Open:** split oversized page/service, drawer a11y, mobile table fallback, filters panel, raw command controls, shared DTOs, page-level UI tests | ⬜ pending                                             |
...
2. **Admin chat user performance extraction** — split `/admin/chat/users` page + backend analytics
   service; remaining work is drawer a11y, mobile fallback, filters panel, raw command controls,
   shared DTOs, and page-level UI tests. Slow-threshold contract shipped in Phase 1.
