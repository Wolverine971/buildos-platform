...
**Why:** Vercel bills GB-Hours = `memory × duration × invocations`. Default Pro is 1024MB / 300s. Halving both halves the cost ceiling per invocation. Even when a request finishes in 200ms, the *allocation* affects scheduling.
