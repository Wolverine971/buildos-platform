-- =====================================================
-- FIX QUEUE TYPE COMPARISON
-- =====================================================
-- Fixes type mismatch error in claim_pending_jobs function
-- Issue: job_type is a queue_type ENUM, but we're comparing with TEXT[]
-- Solution: Cast the enum to text for comparison
-- =====================================================
...
BEGIN
  ...
  RETURNING
    ...
    queue_jobs.user_id,
    queue_jobs.job_type::TEXT,  -- FIXED: Cast enum to text in return
    queue_jobs.metadata,
    queue_jobs.status,
    queue_jobs.priority,
    queue_jobs.attempts,
...
COMMENT ON FUNCTION claim_pending_jobs IS 'Atomically claims a batch of pending jobs for processing (FIXED: proper enum to text casting)';
