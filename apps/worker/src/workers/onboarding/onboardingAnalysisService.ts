  ...
  async generateOnboardingQuestions(
    ...
    if (!options?.forceRegenerate) {
      ...
      if (existingQuestions && existingQuestions.length > 0) {
        console.log(`User ${userId} already has ${existingQuestions.length} active onboarding questions`);
    ...
    if (userContext.input_projects) updateData.last_parsed_input_projects = userContext.input_projects;
    if (userContext.input_work_style) updateData.last_parsed_input_work_style = userContext.input_work_style;
    if (userContext.input_challenges) updateData.last_parsed_input_challenges = userContext.input_challenges;
    if (userContext.input_help_focus) updateData.last_parsed_input_help_focus = userContext.input_help_focus;
