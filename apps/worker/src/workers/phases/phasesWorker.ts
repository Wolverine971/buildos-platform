...
export async function processPhasesJob(job: LegacyJob<PhasesJobData>) {
  console.log(
    `🏃 Processing phases job ${job.id} for project ${job.data.projectId}`,
  );
  ...
  try {
    ...
    console.log(
      `✅ Completed phases generation for project ${job.data.projectId}`,
    );
  } catch (error) {
    const errorMessage =
      error instanceof Error ? error.message : "Unknown error";
    console.error(
      `❌ Failed to generate phases for project ${job.data.projectId}:`,
      errorMessage,
    );
