...
| `turn/`      | One turn's lifecycle: `turn-executor` (claim → provider loop → tool batch), with `executor-contracts` (public types and defaults), `turn-run` (per-turn context), `turn-run-services` (semantic publish, session handoff, read fence), `read-tool-runner`, `mutation-tool-runner`, `turn-finalizer` (finalize, recover, partial answers), and `executor-failures`/`executor-helpers`. Also execution input and control RPCs, the generation `write-fence`, the `turn-lease` keeper (renews the claimed generation and aborts the turn when the lease is lost), cancellation, session handoff, terminal text integrity. |
...
| `effects/`   | Never-fatal side effects: observations, prompt snapshots and local dumps, research capture, billing, persistence traces.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
Dependency direction, enforced by `no-restricted-imports` in `apps/worker/eslint.config.mjs`: `host/` composes everything and only the process entrypoints import it; `provider/` never imports `host/`, `stream/`, or `workflow/` (it reaches delivery and side effects through ports in `provider/contracts.ts`); `shared/` imports no other folder. File names are kebab-case; the `V1` suffix is for wire contracts and persisted shapes that carry a version, not for new internal symbols.
...
- `turn-state.ts` owns one invocation's turn state (`ProviderTurnState`) and tool-round completion: it decides which lane runs next; the coordinator streams it.
- `forced-synthesis.ts` owns the tool-free closing answer.
- `openrouter-client.ts` owns the OpenRouter client and `stream()`; `openrouter/` holds its route opening and health, SSE parsing, usage and telemetry, watchdog, validation, and retry classification.
...
- `repair-policy.ts` owns bounded unavailable-skill, reviewer-mimicry, and surface repair policy. A work tool called from a reduced surface is repaired onto the surface its phase owns (`surfaceFor(phase, admitted, { repair: true })`), never killed. Read saturation has one ladder, the runtime `ContextGatheringLedger`.
...
- `review/lanes.ts` owns the reviewer lanes (mutation-batch review, contract review, review exhaustion) and the unapplied-write wording.
...
The provider runs one acting-pass loop (`streamActingPass`) for the opening pass and every continuation; `options.phase` selects the few behaviours that differ (live-vision resolution and the pre-gate prose hold on the opening pass; reviewer-mimicry repair, the required-pass prose fallback, and contract-driven prose holding on continuations). There is no single-read `synthesize` bridge and no turn supervisor: the context-gathering ledger, validation repair, the reviewer, and `request_turn_clarification` cover every job the supervisor had.
