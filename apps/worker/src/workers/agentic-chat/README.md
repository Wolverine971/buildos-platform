...
| `turn/`      | One turn's lifecycle: `turn-executor` (claim → provider loop → tool batch), with `executor-contracts` (public types and defaults), `turn-run` (per-turn context), `turn-run-services` (semantic publish, session handoff, read fence), `read-tool-runner`, `mutation-tool-runner`, `turn-finalizer` (finalize, recover, partial answers), and `executor-failures`/`executor-helpers`. Also execution input and control RPCs, the generation `write-fence`, cancellation, session handoff, terminal text integrity. |
