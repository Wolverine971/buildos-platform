...
export async function processDailySMS(job: LegacyJob<DailySMSJobData>) {
	...
	try {
		...
		if (prefsError || !smsPrefs) {
			console.error(`❌ [DailySMS] No SMS preferences found for user ${userId}`);
		...
		if (eventsError) {
			console.error('❌ [DailySMS] Error fetching calendar events:', eventsError);
			throw eventsError;
		}

		if (!calendarEvents || calendarEvents.length === 0) {
			console.log(`📝 [DailySMS] No calendar events found for user ${userId} on ${date}`);
			return { success: true, message: 'No calendar events found' };
		}

		console.log(`📋 [DailySMS] Found ${calendarEvents.length} calendar event(s)`);
		...
		for (const event of calendarEvents) {
			...
			scheduledMessages.push({
				...
				llm_model: generatedMessage.model,
		...
		if (insertError) {
			console.error('❌ [DailySMS] Error inserting scheduled messages:', insertError);
			throw insertError;
		}

		console.log(`✅ [DailySMS] Created ${insertedMessages?.length} scheduled SMS message(s)`);
