...
export async function processSMSJob(job: LegacyJob<SMSJobData>) {
	...
	const { message_id, phone_number, message, priority, scheduled_sms_id, user_id } =
		validatedData;
	...
	try {
		...
		if (scheduled_sms_id) {
			console.log(`[SMS Worker] Pre-send validation for scheduled SMS ${scheduled_sms_id}`);
		...
		if (scheduled_sms_id) {
			...
			// BUG FIX #2: Fetch both fields at once (ONLY FETCH ONCE!)
			const { data: currentScheduledSms } = await supabase
				.from('scheduled_sms_messages')
				.select('send_attempts, max_send_attempts')
				.eq('id', scheduled_sms_id)
