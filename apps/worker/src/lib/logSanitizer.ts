// apps/worker/src/lib/logSanitizer.ts
/**
 * Worker port of apps/web/src/lib/utils/logging-helpers.ts (redaction and truncation for
 * persisted error logs). The worker cannot import web code, and moving it into
 * @buildos/shared-utils would make both apps depend on a rebuilt package dist.
 * apps/worker/tests/logSanitizerParity.test.ts fails if the two copies drift.
 */
