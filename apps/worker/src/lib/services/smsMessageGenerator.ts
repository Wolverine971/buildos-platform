...
export class SMSMessageGenerator {
	...
	private buildEventContext(event: EventContext, timeUntil: string): EventPromptContext {
		// Extract meeting link if present
		const meetingLink = event.link || this.extractMeetingLink(event.description);
