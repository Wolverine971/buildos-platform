...
async function queueBriefGeneration(
  ...
  const jobData = {
    userId,
    briefDate,
    options: options ? { ...options, requestedBriefDate: undefined } : undefined,
      ...
      try {
        ...
        if (isAfter(nextRunTime, now) && isBefore(nextRunTime, oneHourFromNow)) {
          ...
          if (!existingJobs || existingJobs.length === 0) {
            console.log(`⏰ Scheduling brief for user ${preference.user_id} at ${nextRunTime.toISOString()}`);
        ...
        console.error(`Error processing preference for user ${preference.user_id}:`, error);
      }
    }
  } catch (error) {
 ...
 */
export function calculateNextRunTime(preference: UserBriefPreference, now: Date): Date | null {
  try {
    ...
    switch (frequency) {
      ...
      case 'weekly':
        ...
        targetDate = calculateWeeklyRunTime(now, dayOfWeek, hours, minutes, seconds, timezone);
        break;
