 ...
 */
export function expectPartial<T extends object>(actual: T, expected: Partial<T>) {
...
export const SMSAssertions = {
	...
	assertGeneratedBy(message: any, type: 'llm' | 'template') {
		if (message.generated_via !== type) {
			throw new Error(`Expected message generated by ${type}, got ${message.generated_via}`);
		}
	},
	...
	assertContainsEventDetails(content: string, eventTitle: string) {
		if (!content.includes(eventTitle)) {
			throw new Error(`SMS message does not contain event title "${eventTitle}": ${content}`);
		}
	}
};
