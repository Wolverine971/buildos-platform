...
export class TestSetup {
	...
	constructor() {
		...
		const supabaseUrl = process.env.TEST_SUPABASE_URL || process.env.PUBLIC_SUPABASE_URL;
		const supabaseKey =
			process.env.TEST_SUPABASE_SERVICE_KEY || process.env.PRIVATE_SUPABASE_SERVICE_KEY;
	...
	async createTestUser(
		...
		if (prefsError) {
			throw new Error(`Failed to create SMS preferences: ${prefsError.message}`);
		}
	...
	async createCalendarEvent(
		...
		const calendarEventId = options.calendarEventId || `test-event-${Date.now()}`;

		const eventStart = startTime.toISOString();
		const eventEnd = new Date(startTime.getTime() + durationMinutes * 60000).toISOString();
	...
	async cleanup() {
		...
		if (this.createdUsers.length > 0) {
			...
			await this.supabase.from('sms_messages').delete().in('user_id', this.createdUsers);
		}

		// Delete calendar events
		if (this.createdEvents.length > 0) {
			await this.supabase.from('task_calendar_events').delete().in('id', this.createdEvents);
		}
