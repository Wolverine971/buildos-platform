// worker-queue/tests/test-email-tracking.ts
...
const generateTrackingId = (emailSender as any).generateTrackingId.bind(emailSender);
...
View phase: /phases/phase-789`,
	...
	priority_actions: ['Review project milestone', 'Update [documentation](/docs/readme)']
};
...
const htmlWithTracking = formatEmail(mockBrief, mockBrief.brief_date, trackingId);
const hasTrackingPixel = htmlWithTracking.includes(`<img src="https://build-os.com/api/email-tracking/${trackingId}"`);
console.log(`  Has tracking pixel: ${hasTrackingPixel ? '✅' : '❌'}`);

if (hasTrackingPixel) {
	const pixelMatch = htmlWithTracking.match(/<img src="https:\/\/build-os\.com\/api\/email-tracking\/[^"]+"/);
	console.log(`  Tracking pixel URL: ${pixelMatch ? pixelMatch[0].slice(10, -1) : 'Not found'}`);
}
...
const urlTests = [
	{ pattern: 'https://build-os.com/projects/6ba7b810-9dad-11d1-80b4-00c04fd430c8', exists: htmlWithTracking.includes('https://build-os.com/projects/6ba7b810-9dad-11d1-80b4-00c04fd430c8') },
	{ pattern: 'https://build-os.com/tasks/review-123', exists: htmlWithTracking.includes('https://build-os.com/tasks/review-123') },
	{ pattern: 'https://build-os.com/tasks/complete-456', exists: htmlWithTracking.includes('https://build-os.com/tasks/complete-456') },
	{ pattern: 'https://build-os.com/phases/phase-789', exists: htmlWithTracking.includes('https://build-os.com/phases/phase-789') }
];

urlTests.forEach(test => {
	console.log(`  URL transformed: ${test.pattern.slice(20)} - ${test.exists ? '✅' : '❌'}`);
});
...
console.log('  - Tracking pixel is added to email HTML when tracking is enabled');
console.log('  - Pixel uses 1x1 transparent PNG served from /api/email-tracking/:trackingId');
