...
describe('Libri OCR admission reconciler', () => {
	...
	it('classifies a stale confirmed admission as blocked without repairing it', async () => {
		const reconciler = createLibriAdmissionReconciler(
		...
		await expect(
			...
			healthy: false,
			issues: ['confirmed_run_not_dispatchable', 'confirmed_has_queue_receipts']
		});
	});
	...
	it('classifies an enqueued admission with a missing receipt as incomplete', async () => {
		const context = admissionContext({ admission_status: 'enqueued' });
		const reconciler = createLibriAdmissionReconciler(
			auditHarness(context, manifest, [
				{
					id: '80000000-0000-4000-8000-000000000001',
					queue_job_id: 'only-one',
					job_type: 'libri_ingest',
					metadata: libriOcrQueueMetadata(context, manifest[0]!),
					status: 'complete'
				}
			]).pool
		);
