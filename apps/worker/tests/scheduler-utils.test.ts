// worker-queue/tests/scheduler-utils.test.ts
...
function calculateWeeklyRunTime(
  ...
  if (daysUntilTarget === 0) {
    const targetTimeToday = setSeconds(
      setMinutes(setHours(nowInTz, hours), minutes),
      seconds,
    );
  ...
  const targetInTz = setSeconds(
    setMinutes(setHours(targetDate, hours), minutes),
    seconds,
  );
...
describe("Scheduler Logic Tests", () => {
  describe("Daily Brief Scheduling", () => {
    ...
    it("should handle different timezones correctly", () => {
      ...
      const targetTime = calculateDailyRunTime(
        now,
        9,
        0,
        0,
