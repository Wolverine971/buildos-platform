// apps/worker/tests/agenticChatConsumerFactory.test.ts
...
describe('Agentic Chat consumer factory isolation', () => {
	it('constructs one inert chat-only queue with independent fixture policy', () => {
		...
		const consumer = createAgenticChatConsumerFactory(executor as never);

		expect(consumer.config).toEqual(DEFAULT_AGENTIC_CHAT_CONSUMER_FACTORY_CONFIG);
	...
	it('validates independent concurrency and timeout relationships', () => {
		const executor = { execute: vi.fn() };
		expect(() =>
			createAgenticChatConsumerFactory(executor as never, { concurrency: 9 })
		).toThrow('concurrency cannot exceed 8');
		expect(() =>
			createAgenticChatConsumerFactory(executor as never, {
				workerTimeoutMs: 1_000,
				stalledTimeoutMs: 1_000
			})
		).toThrow('stalled timeout must exceed');
		expect(() =>
			createAgenticChatConsumerFactory(executor as never, { pollIntervalMs: 100 })
	...
	it('keeps saturated general slots independent from bounded chat slots', async () => {
		...
		const chat = createAgenticChatConsumerFactory(
			{
				async execute(job) {
