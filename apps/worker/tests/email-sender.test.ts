...
describe('DailyBriefEmailSender', () => {
	...
	describe('shouldSendEmail', () => {
		it("should query user_notification_preferences with event_type='user'", async () => {
			...
			vi.spyOn(mockSupabase, 'from').mockImplementation((table: string): any => {
				const chain: any = {
					select: vi.fn(() => chain),
					eq: vi.fn(() => chain),
					single: vi.fn(() => {
	...
	describe('formatBriefForEmail', () => {
		...
		it('should transform relative URLs to absolute URLs', () => {
			...
			const brief = {
				...
				summary_content: 'Test summary',
				brief_date: '2025-10-13',
				llm_analysis: '[View project](/projects/my-project)\n\nSee [task](/tasks/task-123)'
			};

			const result = emailSender.formatBriefForEmail(brief, '2025-10-13');

			expect(result.htmlContent).toContain('https://build-os.com/projects/my-project');
			expect(result.htmlContent).toContain('https://build-os.com/tasks/task-123');
		});
