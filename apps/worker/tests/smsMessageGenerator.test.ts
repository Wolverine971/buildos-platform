...
import { SMSMessageGenerator, type EventContext } from '../src/lib/services/smsMessageGenerator';
...
describe('SMSMessageGenerator', () => {
	...
	describe('generateEventReminder (template fallback)', () => {
		it('should generate template message under 160 characters for simple meeting', async () => {
			...
			const result = await generator.generateEventReminder(event, 15, 'test-user-123');
		...
		it('should include event title in the message', async () => {
			...
			const result = await generator.generateEventReminder(event, 30, 'test-user-456');
		...
		it('should handle meetings with links', async () => {
			...
			const result = await generator.generateEventReminder(event, 10, 'test-user-789');
		...
		it('should handle long event titles gracefully', async () => {
			const event: EventContext = {
				eventId: 'test-long',
				title: 'Quarterly Business Review and Strategic Planning Session for Q4 2025 with Executive Team',
			...
			const result = await generator.generateEventReminder(event, 60, 'test-user-long');
		...
		it('should not contain markdown or emojis', async () => {
			...
			const result = await generator.generateEventReminder(event, 15, 'test-user-clean');
		...
		it('should handle events with location', async () => {
			...
			const result = await generator.generateEventReminder(event, 30, 'test-user-location');
		...
		it('should always fall back to template on LLM error', async () => {
			...
			const result = await generator.generateEventReminder(event, 5, 'test-user-fallback');
