...
import {
  ...
  formatSender,
  getGmailConfig,
} from "../src/lib/services/gmail-transporter";
...
async function testEmailConfiguration() {
  ...
  if (!gmailConfig) {
    ...
    console.log(
      "  - GMAIL_ALIAS (optional): Your sender alias (e.g., noreply@build-os.com)",
    );
  ...
  console.log(
    `  - Sender Alias: ${gmailConfig.alias || "(not set - will use Gmail account)"}`,
  );
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.message.includes("Invalid login")) {
        ...
        console.log(
          '  2. Go to Gmail Settings > Accounts > "Send mail as" to add/verify aliases',
        );
        console.log(
          "  3. The alias must be a real email address that you control",
        );
      }
    }
