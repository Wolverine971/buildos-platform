...
async function checkQueuedJobs() {
	...
	const { data: jobs, error } = await supabase
		.from('queue_jobs')
		.select('id, queue_job_id, user_id, job_type, status, scheduled_for, created_at, metadata')
	...
	if (!jobs || jobs.length === 0) {
		...
		if (futureJobs && futureJobs.length > 0) {
			console.log(`📅 Found ${futureJobs.length} jobs scheduled for the future:\n`);
	...
	for (const job of jobs) {
		...
		const hoursOffset = ((scheduledDate.getTime() - now.getTime()) / (1000 * 60 * 60)).toFixed(
			1
		);
