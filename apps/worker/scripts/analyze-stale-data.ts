...
async function analyzeStaleData() {
	console.log('🔍 Analyzing stale and failed data across all job-related tables\n');
	...
	const { data: queueStats } = await supabase.from('queue_jobs').select('status, job_type');

	if (queueStats) {
		const statusMap: Record<string, Record<string, number>> = {};
		queueStats.forEach((job: any) => {
			if (!statusMap[job.status]) statusMap[job.status] = {};
			if (!statusMap[job.status][job.job_type]) statusMap[job.status][job.job_type] = 0;
	...
	const { data: briefStats } = await supabase.from('daily_briefs').select('generation_status');
	...
	const { data: smsStats } = await supabase.from('sms_messages').select('status');
	...
	console.log(`  • Project Briefs (failed/pending >30d): ~${oldProjectBriefCount || 0}`);
	console.log(`  • Scheduled SMS (pending/failed >30d): ~${oldScheduledSmsCount || 0}`);
