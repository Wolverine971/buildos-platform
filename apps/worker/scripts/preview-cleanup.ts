...
async function previewCleanup() {
	...
	if (jobError) {
		...
		Object.entries(byType).forEach(([type, jobs]) => {
			...
			jobs.slice(0, 3).forEach((job: any) => {
				...
				console.log(`     - ${job.queue_job_id} (${age}d old, attempts: ${job.attempts})`);
	...
	const { data: oldFailedBriefs, error: briefError } = await supabase
		.from('daily_briefs')
		.select('id, user_id, brief_date, generation_status, created_at, generation_error')
	...
	const { data: stuckBriefs, error: stuckError } = await supabase
		.from('daily_briefs')
		.select('id, user_id, brief_date, generation_status, created_at, generation_started_at')
		.eq('generation_status', 'processing')
		.lt('created_at', sevenDaysAgo)
		.order('created_at', { ascending: true });

	const allBriefsToDelete = [...(oldFailedBriefs || []), ...(stuckBriefs || [])];
	...
	if (briefError || stuckError) {
		...
		allBriefsToDelete.slice(0, 10).forEach((brief: any) => {
			...
			console.log(`     - Brief ${brief.id.substring(0, 8)}... (${brief.brief_date})`);
	...
	const { data: oldProjectBriefs, error: projectBriefError } = await supabase
		...
		.select('id, project_id, brief_date, generation_status, created_at, generation_error')
		.in('generation_status', ['failed', 'pending'])
		.lt('created_at', thirtyDaysAgo)
		.order('created_at', { ascending: true });
	...
	if (projectBriefError) {
		...
		console.log(`   Found ${oldProjectBriefs.length} project briefs to delete:\n`);
	...
	console.log('='.repeat(70));
	const totalToDelete =
		(oldFailedJobs?.length || 0) + allBriefsToDelete.length + (oldProjectBriefs?.length || 0);
	...
	console.log("  ✅ Only deleting records >30 days old (>7d for stuck 'processing')");
