...
async function checkStaleJobs() {
	console.log('🔍 Checking for stale queue jobs (scheduled >24 hours ago)...\n');
	...
	const { data: activeJobs } = await supabase
		.from('queue_jobs')
		.select('id, queue_job_id, job_type, status, scheduled_for, created_at, attempts')
		.in('status', ['pending', 'processing', 'retrying'])
	...
	if (activeJobs && activeJobs.length > 0) {
		activeJobs.forEach((job: any) => {
			...
			console.log(`    Scheduled for: ${job.scheduled_for} (${scheduledAge}h ago)`);
	...
	const { data: oldFailedJobs } = await supabase
		.from('queue_jobs')
		.select('id, job_type, status, scheduled_for, attempts, max_attempts, error_message')
		.eq('status', 'failed')
		.in('job_type', ['generate_daily_brief', 'generate_brief_email'])
