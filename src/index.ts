...
console.log(`   → USE_WEBHOOK_EMAIL: "${process.env.USE_WEBHOOK_EMAIL}" (type: ${typeof process.env.USE_WEBHOOK_EMAIL})`);
...
app.post('/queue/brief', async (req, res) => {
  try {
    ...
    if (forceRegenerate) {
      const targetBriefDate = requestedBriefDate || 
      ...
      if (count > 0) {
        console.log(`🚫 Force regenerate: Cancelled ${count} existing brief job(s) for ${targetBriefDate}`);
      }
    }
