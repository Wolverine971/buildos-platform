...
export class EmailService {
	...
	constructor(supabase: SupabaseClient) {
		...
		if (this.gmailConfig) {
			...
			console.warn('⚠️ Gmail not configured. Emails will be logged but not sent.');
		}
	}
	...
	async sendEmail(data: EmailData): Promise<void> {
		...
		let simulated = false;
		const htmlBody = data.useTemplate === false
			? data.body
		...
		try {
			...
			if (this.transporter && this.gmailConfig) {
				const mailOptions = {
					...
					cc: data.cc?.join(', '),
					bcc: data.bcc?.join(', '),
					replyTo: data.replyTo || this.gmailConfig.alias || this.gmailConfig.email
				};

				const info = await this.transporter.sendMail(mailOptions);
				messageId = info.messageId;
				console.log(`✅ Email sent successfully to ${data.to} (Message ID: ${messageId})`);
				...
				console.log('📧 Email (simulated):', { 
					to: data.to, 
					subject: data.subject,
					from: this.gmailConfig ? formatSender(this.gmailConfig) : 'noreply@build-os.com'
				});
				simulated = true;
	...
	private async handlePostSendSuccess(params: {
		...
		const { emailId, recipientId, status, messageId, metadata, simulated } = params;
