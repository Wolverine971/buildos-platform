...
const VALID_EMAIL_RECIPIENT_STATUSES: readonly EmailRecipientStatus[] = ['pending', 'sent', 'delivered', 'failed', 'bounced'] as const;
...
export class DailyBriefEmailSender {
	...
	constructor(private supabase: SupabaseClient) {
		...
		console.log(`   → Raw USE_WEBHOOK_EMAIL value: "${process.env.USE_WEBHOOK_EMAIL}"`);
		console.log(`   → Type of USE_WEBHOOK_EMAIL: ${typeof process.env.USE_WEBHOOK_EMAIL}`);
		console.log(`   → Strict equality to 'true': ${process.env.USE_WEBHOOK_EMAIL === 'true'}`);
		console.log(`   → Webhook URL present: ${process.env.BUILDOS_WEBHOOK_URL ? 'YES' : 'NO'}`);
		console.log(`   → SMTP Host present: ${process.env.SMTP_HOST ? 'YES' : 'NO'}`);
		...
		if (this.useWebhook) {
			try {
				...
				console.log('📨 Email sender initialized: Using WEBHOOK service for BuildOS');
				console.log('   → Webhook URL configured:', process.env.BUILDOS_WEBHOOK_URL ? 'Yes' : 'No');
				if (process.env.BUILDOS_WEBHOOK_URL) {
					console.log(`   → Webhook URL domain: ${new URL(process.env.BUILDOS_WEBHOOK_URL).hostname}`);
				}
			} catch (error) {
			...
			console.log(`   → Reason: USE_WEBHOOK_EMAIL="${process.env.USE_WEBHOOK_EMAIL}" !== 'true'`);
		}
	}
	 ...
	 */
	formatBriefForEmail(brief: DailyBriefResult, briefDate: string): { htmlContent: string; plainText: string } {
...
`.trim();
   ...
   → Method: ${this.useWebhook ? 'WEBHOOK' : 'DIRECT SMTP'}`)
			...
			const subject = `Daily Brief - ${new Date(briefDate).toLocaleDateString('en-US', {
				weekday: 'short',
				month: 'short',
				day: 'numeric'
			})}`;
