<!-- src/lib/components/history/LinkBraindumpToProject.svelte -->
<script lang="ts">
	...
	// Link braindump to selected project
	async function linkToProject() {
		if (!selectedProjectId || isLinking) return;

		isLinking = true;
		try {
			const response = await fetch(`/api/braindumps/${braindumpId}/link`, {
				method: 'PATCH',
			...
			if (result.success) {
				...
				toastService.success(`Linked to ${selectedProject?.name || 'project'}`);
				...
				throw new Error(result.error || 'Failed to link braindump');
			}
		} catch (error) {
			console.error('Failed to link braindump:', error);
			toastService.error('Failed to link to project');
		} finally {
			isLinking = false;
		}
	}
