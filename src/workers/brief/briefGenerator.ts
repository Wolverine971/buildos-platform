...
export type TaskCalendarEvent = Database['public']['Tables']['task_calendar_events']['Row'];
...
function getDayBoundsInTimezone(date: Date | string, timezone: string): { start: Date; end: Date } {
...
export async function generateDailyBrief(
  ...
  console.log(`📝 Processing daily brief for user ${userId} on ${briefDate} (timezone: ${userTimezone})`);
  ...
  const briefMetadata = {
    ...
    ...((typeof existingBrief?.metadata === 'object' && existingBrief?.metadata !== null) ? existingBrief.metadata : {})
  };
  ...
  try {
    ...
    await updateProgress(dailyBrief.id, { step: 'generating_project_briefs', progress: 20 }, jobId);
    ...
    for (let i = 0; i < projects.length; i++) {
      ...
      await updateProgress(dailyBrief.id, { 
        step: `processing_project_${project.name}`, 
        progress: Math.round(progress) 
      }, jobId);

      try {
        const projectBrief = await generateProjectBrief(project, briefDateInUserTz, userTimezone);
        projectBriefs.push(projectBrief);
        console.log(`📄 Generated brief for project: ${project.name}`);
      } catch (error) {
        console.warn(`⚠️ Failed to generate brief for project ${project.name}:`, error);
    ...
    const analysisProjects: DailyBriefAnalysisProject[] = buildDailyBriefAnalysisProjects(projectBriefs);
    ...
    if (analysisProjects.length > 0) {
      ...
      try {
        ...
        console.error('Failed to generate LLM analysis for daily brief:', analysisError);
      }
    }
...
async function getUserProjectsWithData(userId: string, options?: BriefJobData['options']): Promise<ProjectWithRelations[]> {
  ...
  if (options?.excludeProjects?.length) {
    projectsQuery = projectsQuery.not('id', 'in', `(${options.excludeProjects.join(',')})`);
  }
  ...
  const [tasksResponse, notesResponse, phasesResponse, phaseTasksResponse, calendarEventsResponse] = await Promise.all([
    supabase
      .from('tasks')
      .select('*')
  ...
  const tasksWithCalendarEvents: TaskWithCalendarEvent[] = tasks.map(task => ({
    ...task,
    task_calendar_events: calendarEvents.filter(event => event.task_id === task.id)
  }));
  ...
  return projects.map(project => ({
    ...
    phaseTasks: phaseTasks.filter(pt => 
      phases.some(phase => phase.id === pt.phase_id && phase.project_id === project.id)
    )
  }));
}

async function generateProjectBrief(project: ProjectWithRelations, briefDate: string, timezone: string) {
  ...
  if (currentPhase) {
    ...
    upcomingTasks = getUpcomingPhaseTasksFromToday(project.tasks, project.phaseTasks, currentPhase.id, briefDate, timezone);
  } else {
  ...
  const currentPhaseTasks = currentPhase ? getCurrentPhaseTasks(project.tasks, project.phaseTasks, currentPhase.id) : [];
  ...
  const analysisContext = {
    ...
    current_phase: currentPhase
      ...
      : null,
    todays_tasks: todaysTasks.map(task => mapTaskForAnalysis(task, project.id, timezone)),
    next_seven_days_tasks: nextSevenDaysTasks.map(task => mapTaskForAnalysis(task, project.id, timezone)),
    overdue_tasks: overdueTasks.map(task => mapTaskForAnalysis(task, project.id, timezone)),
    recently_completed_tasks: recentlyCompletedTasks.map(task => mapTaskForAnalysis(task, project.id, timezone, true)),
    recent_notes: recentNotes.map(note => mapNoteForAnalysis(note, project.id, timezone))
  };
...
function getCurrentPhase(phases: Phase[], briefDate: string, timezone: string) {
  ...
  return phases.find(phase => {
    ...
    return phase.start_date <= todayInUserTz && phase.end_date >= todayInUserTz;
  }) || null;
}
...
function getOverdueTasks(tasks: TaskWithCalendarEvent[], briefDate: string, timezone: string) {
  ...
  return tasks.filter((task: TaskWithCalendarEvent) => {
...
function getUpcomingPhaseTasksFromToday(tasks: TaskWithCalendarEvent[], phaseTasks: PhaseTask[], phaseId: string, briefDate: string, timezone: string) {
  const todayBounds = getDayBoundsInTimezone(briefDate, timezone);
...
function getNext10UpcomingTasks(tasks: TaskWithCalendarEvent[], briefDate: string, timezone: string) {
  const todayBounds = getDayBoundsInTimezone(briefDate, timezone);
...
function getRecentlyCompletedTasks(tasks: TaskWithCalendarEvent[], briefDate: string, timezone: string) {
  const todayBounds = getDayBoundsInTimezone(briefDate, timezone);
  ...
  return tasks
    .filter((task: TaskWithCalendarEvent) => {
      ...
      return updatedDate >= twentyFourHoursAgo && updatedDate <= todayBounds.end;
    })
    .sort((a: TaskWithCalendarEvent, b: TaskWithCalendarEvent) => {
...
function getCurrentPhaseTasks(tasks: TaskWithCalendarEvent[], phaseTasks: PhaseTask[], phaseId: string): TaskWithCalendarEvent[] {
  const phaseTaskIds = phaseTasks
    .filter(pt => pt.phase_id === phaseId)
    .map(pt => pt.task_id);
...
function formatTaskWithDetails(task: TaskWithCalendarEvent, projectId: string, timezone: string, showCompletedDate: boolean = false): string {
...
function formatProjectBrief(data: {
  ...
  const { project, currentPhase, todaysTasks, overdueTasks, upcomingTasks, recentlyCompletedTasks, recentNotes, currentPhaseTasks, phaseTasks, timezone } = data;
  ...
  if (todaysTasks.length === 0 && upcomingTasks.length === 0 && recentNotes.length === 0 && recentlyCompletedTasks.length === 0 && overdueTasks.length === 0) {
...
function mapTaskForAnalysis(
  ...
  return {
    ...
    start_date_formatted: task.start_date ? formatDateInTimezone(task.start_date, timezone) : null,
    completed_at: showCompletedDate && task.updated_at ? task.updated_at : null,
    completed_at_formatted:
      showCompletedDate && task.updated_at ? formatDateInTimezone(task.updated_at, timezone, true) : null,
    link: `/projects/${projectId}/tasks/${task.id}`,
    has_calendar_event: Boolean(task.task_calendar_events && task.task_calendar_events.length > 0)
  };
}
...
function generateMainBrief(projectBriefs: any[], briefDate: string, timezone: string) {
  ...
  const projectsWithTodaysTasks = projectBriefs.filter(brief => 
    brief.metadata?.todays_task_count > 0
  ).length;
  const totalTodaysTasks = projectBriefs.reduce((sum, brief) => 
    sum + (brief.metadata?.todays_task_count || 0), 0
  );
  const totalOverdueTasks = projectBriefs.reduce((sum, brief) => 
    sum + (brief.metadata?.overdue_task_count || 0), 0
  );
  const totalUpcomingTasks = projectBriefs.reduce((sum, brief) => 
    sum + (brief.metadata?.upcoming_task_count || 0), 0
  );
  const totalNextSevenDaysTasks = projectBriefs.reduce((sum, brief) => 
    sum + (brief.metadata?.next_seven_days_task_count || 0), 0
  );
  const totalRecentlyCompleted = projectBriefs.reduce((sum, brief) => 
    sum + (brief.metadata?.recently_completed_count || 0), 0
  );
...
function buildDailyBriefAnalysisProjects(projectBriefs: any[]): DailyBriefAnalysisProject[] {
  return projectBriefs.map((brief: any) => {
    ...
    return {
      ...
      stats: {
        todays_task_count: stats.todays_task_count || (context.todays_tasks ? context.todays_tasks.length : 0) || 0,
        next_seven_days_task_count: stats.next_seven_days_task_count || (context.next_seven_days_tasks ? context.next_seven_days_tasks.length : 0) || 0,
        overdue_task_count: stats.overdue_task_count || (context.overdue_tasks ? context.overdue_tasks.length : 0) || 0,
        recently_completed_count: stats.recently_completed_count || (context.recently_completed_tasks ? context.recently_completed_tasks.length : 0) || 0
      },
...
function getTaskStatusIcon(task: TaskWithCalendarEvent): string {
  ...
  if (task.start_date || (task.task_calendar_events && task.task_calendar_events.length > 0)) {
...
function formatDateInTimezone(timestamp: string, timezone: string, includeTime: boolean = false): string {
...
function extractPriorityActions(content: string): string[] {
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes('Starting Today') || 
...
async function updateProgress(briefId: string, progress: { step: string; progress: number }, jobId?: string) {
