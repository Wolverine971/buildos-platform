-- supabase/migrations/20260501000001_add_p_limit_to_project_summaries_v1.sql
-- Add an optional `p_limit` to `get_onto_project_summaries_v1` so callers that
-- only need the recent N projects (dashboard, workspace navigation) can avoid
-- pulling rollups for every accessible project.
--
-- Backwards-compatible: `p_limit` defaults to NULL, which preserves the current
-- "return all accessible projects" behavior. Existing in-database callers
-- (`build_fastchat_project_intelligence`, `load_fastchat_context`) keep working
-- because they call the function with a single positional arg.
...
GRANT EXECUTE ON FUNCTION get_onto_project_summaries_v1(uuid, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION get_onto_project_summaries_v1(uuid, integer) TO service_role;

-- Drop the old single-arg signature to avoid ambiguity in PostgREST/pgrst
-- function resolution. Existing SQL callers that use a single positional arg
-- transparently resolve to the new function thanks to the DEFAULT.
DROP FUNCTION IF EXISTS get_onto_project_summaries_v1(uuid);
