...
-- Libri phase 3G: atomically claim one OCR image with its paid-call reservation,
-- then persist the OCR chunk and settle usage under the same lease fence.

SET lock_timeout = '5s';
SET statement_timeout = '60s';

CREATE POLICY images_libri_worker_update_ocr
	ON libri.images
	FOR UPDATE
	TO libri_worker
	USING (true)
...
CREATE POLICY source_chunks_libri_worker_select_ocr
	ON libri.source_chunks
	FOR SELECT
	TO libri_worker
	USING (chunk_type = 'ocr');

CREATE POLICY source_chunks_libri_worker_insert_ocr
	ON libri.source_chunks
	FOR INSERT
	TO libri_worker
	WITH CHECK (chunk_type = 'ocr');

CREATE OR REPLACE FUNCTION libri.enforce_ocr_image_worker_write()
RETURNS trigger
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
	...
	IF NEW.ocr_status = 'processing' THEN
		IF OLD.ocr_status NOT IN ('pending', 'failed')
			OR NEW.ocr_version IS DISTINCT FROM OLD.ocr_version THEN
			RAISE EXCEPTION 'invalid OCR image processing transition';
		END IF;

		BEGIN
			expected_ocr_version := (NEW.ocr_metadata->>'expectedOcrVersion')::integer;
		EXCEPTION
			WHEN invalid_text_representation OR numeric_value_out_of_range THEN
				RAISE EXCEPTION 'invalid OCR image processing metadata';
		END;
		...
		JOIN libri.research_runs AS run
			ON run.id = step.run_id AND run.library_id = step.library_id
		WHERE step.id = (NEW.ocr_metadata->>'stepId')::uuid;
		...
		WHERE reservation.id = (NEW.ocr_metadata->>'costReservationId')::uuid;

		expected_metadata := jsonb_build_object(
			'version', 1,
			'state', 'processing',
			'imageContentSha256', OLD.content_sha256,
			'expectedOcrVersion', expected_ocr_version,
			'runId', step_row.run_id::text,
			'stepId', reservation_row.step_id::text,
			'executionGeneration', reservation_row.execution_generation,
			'costReservationId', reservation_row.id::text,
			'provider', reservation_row.provider,
			'model', reservation_row.model,
			'startedAt', to_jsonb(reservation_row.started_at)
		);

		IF step_row.library_id IS DISTINCT FROM OLD.library_id
			...
			OR step_row.run_status <> 'running'
			OR step_row.cancel_requested_at IS NOT NULL
			OR (step_row.deadline_at IS NOT NULL AND step_row.deadline_at <= clock_timestamp())
			OR reservation_row.library_id IS DISTINCT FROM OLD.library_id
			...
			OR reservation_row.reservation_key IS DISTINCT FROM
				'ocr:image:' || OLD.id::text || ':version:' || expected_ocr_version::text
			OR OLD.ocr_version + 1 <> expected_ocr_version
			OR step_row.payload IS DISTINCT FROM jsonb_build_object(
				'version', 1,
				'kind', 'ocr_image',
				'imageId', OLD.id::text,
				'expectedOcrVersion', expected_ocr_version,
				'maxOutputChars', (step_row.payload->>'maxOutputChars')::integer
			)
			OR NEW.ocr_metadata IS DISTINCT FROM expected_metadata THEN
			RAISE EXCEPTION 'invalid or stale OCR image processing ownership';
		END IF;
		RETURN NEW;
	END IF;

	IF NEW.ocr_status = 'complete' THEN
		IF OLD.ocr_status <> 'processing'
			OR NEW.ocr_version <> OLD.ocr_version + 1 THEN
			RAISE EXCEPTION 'invalid OCR image completion transition';
		END IF;

		SELECT reservation.* INTO reservation_row
		FROM libri.provider_cost_reservations AS reservation
		WHERE reservation.id = (NEW.ocr_metadata->>'costReservationId')::uuid;
		SELECT chunk.id, chunk.chunk_type, chunk.image_id, chunk.content_sha256
		INTO chunk_row
		FROM libri.source_chunks AS chunk
		WHERE chunk.library_id = OLD.library_id
			AND chunk.id = (NEW.ocr_metadata->>'sourceChunkId')::uuid;

		IF reservation_row.status <> 'settled'
			OR reservation_row.library_id IS DISTINCT FROM OLD.library_id
			OR reservation_row.step_id::text IS DISTINCT FROM NEW.ocr_metadata->>'stepId'
			OR reservation_row.execution_generation::text IS DISTINCT FROM
				NEW.ocr_metadata->>'executionGeneration'
			OR chunk_row.chunk_type <> 'ocr'
			OR chunk_row.image_id IS DISTINCT FROM OLD.id
			OR chunk_row.content_sha256 IS DISTINCT FROM NEW.ocr_metadata->>'contentSha256'
			OR NEW.ocr_version::text IS DISTINCT FROM NEW.ocr_metadata->>'ocrVersion'
			OR OLD.content_sha256 IS DISTINCT FROM NEW.ocr_metadata->>'imageContentSha256' THEN
			RAISE EXCEPTION 'invalid OCR image completion ownership';
		END IF;
		RETURN NEW;
	END IF;

	RAISE EXCEPTION 'libri_worker may only claim or complete OCR images';
END;
$function$;

CREATE TRIGGER images_enforce_ocr_worker_write
	BEFORE UPDATE ON libri.images
	FOR EACH ROW EXECUTE FUNCTION libri.enforce_ocr_image_worker_write();
...
BEGIN
	...
	IF NEW.chunk_type <> 'ocr'
		...
		OR NEW.content_sha256 IS DISTINCT FROM encode(
			extensions.digest(convert_to(NEW.content, 'UTF8'), 'sha256'),
			'hex'
		)
...
CREATE TRIGGER source_chunks_enforce_ocr_worker_write
	BEFORE INSERT ON libri.source_chunks
	FOR EACH ROW EXECUTE FUNCTION libri.enforce_ocr_source_chunk_worker_write();

CREATE OR REPLACE FUNCTION libri.authorize_ocr_provider_call(
	p_queue_row_id uuid,
...
BEGIN
	IF p_queue_row_id IS NULL OR p_processing_token IS NULL OR p_step_id IS NULL
		...
		OR p_lease_token IS NULL OR p_reservation_id IS NULL OR p_image_id IS NULL THEN
		RAISE EXCEPTION 'invalid OCR provider authorization input';
	END IF;
	...
	FOR UPDATE;
	IF NOT FOUND THEN
		RETURN QUERY SELECT false, 'stale'::text, NULL::integer, NULL::text, NULL::text;
		RETURN;
	END IF;
	...
	EXCEPTION
		WHEN invalid_text_representation OR numeric_value_out_of_range THEN
			RETURN QUERY SELECT false, 'invalid_payload'::text, NULL::integer, NULL::text, NULL::text;
			RETURN;
	END;
	...
	IF NOT FOUND
		...
		OR reservation_row.reservation_key IS DISTINCT FROM
			'ocr:image:' || p_image_id::text || ':version:' || expected_ocr_version::text THEN
		RETURN QUERY SELECT false, 'stale'::text, NULL::integer, NULL::text, NULL::text;
		RETURN;
	END IF;
	...
	FOR UPDATE;
	IF NOT FOUND THEN
		RETURN QUERY SELECT false, 'image_unavailable'::text, NULL::integer,
			reservation_row.provider, reservation_row.model;
		RETURN;
	END IF;

	IF reservation_row.status = 'started' THEN
		RETURN QUERY SELECT false, 'started'::text, payload_max_output_chars,
			reservation_row.provider, reservation_row.model;
		RETURN;
	END IF;
	IF reservation_row.status <> 'reserved' THEN
		RETURN QUERY SELECT false, reservation_row.status, payload_max_output_chars,
			reservation_row.provider, reservation_row.model;
		RETURN;
	END IF;
	IF image_row.bucket_id <> 'libri-assets'
		OR image_row.mime_type NOT IN ('image/jpeg', 'image/png', 'image/webp')
		OR image_row.byte_size <= 0 OR image_row.byte_size > 26214400
		OR image_row.content_sha256 !~ '^[0-9a-f]{64}$'
		OR image_row.ocr_status NOT IN ('pending', 'failed')
		OR image_row.ocr_version + 1 <> expected_ocr_version THEN
		RETURN QUERY SELECT false, 'image_unavailable'::text, payload_max_output_chars,
			reservation_row.provider, reservation_row.model;
		RETURN;
	END IF;
	...
	RETURNING reservation.started_at INTO started_at_value;

	UPDATE libri.images AS image
	SET
		ocr_status = 'processing',
		ocr_metadata = jsonb_build_object(
			'version', 1,
			'state', 'processing',
			'imageContentSha256', image_row.content_sha256,
			'expectedOcrVersion', expected_ocr_version,
			...
			'provider', reservation_row.provider,
			'model', reservation_row.model,
			'startedAt', to_jsonb(started_at_value)
		)
	WHERE image.id = image_row.id;

	RETURN QUERY SELECT true, 'started'::text, payload_max_output_chars,
		reservation_row.provider, reservation_row.model;
END;
$function$;
...
BEGIN
	...
	content_hash := encode(
		extensions.digest(convert_to(normalized_text, 'UTF8'), 'sha256'),
		'hex'
	);
...
REVOKE ALL ON TABLE libri.images, libri.source_chunks FROM libri_worker;

GRANT SELECT (
	library_id, id, book_id, chapter_id, source_id, bucket_id, mime_type,
	byte_size, content_sha256, page_label, ocr_status, ocr_version, ocr_metadata
) ON TABLE libri.images TO libri_worker;
GRANT UPDATE (ocr_status, ocr_version, ocr_metadata)
	ON TABLE libri.images TO libri_worker;

GRANT SELECT (
	id, library_id, source_id, book_id, chapter_id, image_id, chunk_type,
	page_label, language, content, content_sha256, idempotency_key, metadata
) ON TABLE libri.source_chunks TO libri_worker;
GRANT INSERT (
	library_id, source_id, book_id, chapter_id, image_id, chunk_type,
	page_label, language, content, content_sha256, idempotency_key, metadata
) ON TABLE libri.source_chunks TO libri_worker;

REVOKE ALL ON FUNCTION libri.enforce_ocr_image_worker_write()
	FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION libri.enforce_ocr_source_chunk_worker_write()
	FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION libri.authorize_ocr_provider_call
	FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION libri.persist_and_settle_ocr_result
	FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION libri.enforce_ocr_image_worker_write()
	TO libri_worker, service_role;
GRANT EXECUTE ON FUNCTION libri.enforce_ocr_source_chunk_worker_write()
	TO libri_worker, service_role;
GRANT EXECUTE ON FUNCTION libri.authorize_ocr_provider_call
	TO libri_worker, service_role;
GRANT EXECUTE ON FUNCTION libri.persist_and_settle_ocr_result
	TO libri_worker, service_role;
