...
-- =====================================================
-- Bug: emit_notification_event() was notifying ALL subscribed users
-- instead of only the target_user_id when provided.
--
-- This caused 121 deliveries per event instead of 2-4.
-- =====================================================
...
-- Recreate with proper target_user_id filtering
...
DECLARE
  ...
  v_prefs RECORD;
  v_push_sub RECORD;  -- ✅ Added: Missing variable for push subscription loop
  v_delivery_id UUID;
  v_queue_job_id TEXT;
...
BEGIN
  ...
  -- ✅ FIX: Filter by target_user_id when provided
  FOR v_subscription IN
    SELECT * FROM notification_subscriptions
    WHERE event_type = p_event_type
      AND is_active = true
      AND (p_target_user_id IS NULL OR user_id = p_target_user_id)  -- ✅ FIXED
  LOOP
    ...
    IF v_prefs.sms_enabled THEN
      ...
      BEGIN
        ...
        IF FOUND THEN
          ...
          CASE p_event_type
            WHEN 'brief.completed' THEN
              v_should_send_sms := COALESCE(v_sms_prefs.daily_brief_sms, false);
            ELSE
              v_should_send_sms := false;
          END CASE;

          IF v_should_send_sms THEN
            INSERT INTO notification_deliveries (
              event_id,
              subscription_id,
              recipient_user_id,
...
-- VERIFICATION
-- =====================================================
COMMENT ON FUNCTION emit_notification_event IS 'Creates notification event and deliveries. FIXED: Now filters subscriptions by target_user_id when provided to prevent broadcast bug.';
