-- supabase/migrations/20260715030000_project_full_v2_entity_summaries.sql
-- Keep the v2 initial project payload focused on render-ready summaries.
--
-- Document props can contain legacy body_markdown copies and accounted for up
-- to 94% of the document slice on measured production projects. The initial
-- route only needs tree labels, states, timestamps, and a content-presence bit;
-- full document bodies and props are fetched by the document endpoint on demand.
-- Goals, plans, milestones, and risks follow the same rule: the page receives
-- the list fields it renders, while each edit modal fetches its full entity.
...
-- Used by the document tree's explicit refresh path. SECURITY INVOKER keeps
-- the existing onto_documents RLS policy as the authorization boundary.
CREATE OR REPLACE FUNCTION public.get_project_document_tree_metadata(
	p_project_id uuid
)
RETURNS TABLE (
	id uuid,
...
GRANT EXECUTE ON FUNCTION public.get_project_document_tree_metadata(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_project_document_tree_metadata(uuid) TO service_role;
