-- supabase/migrations/20260429000002_add_fastchat_project_intelligence.sql
-- Add bounded prewarm project intelligence to FastChat context.

CREATE INDEX IF NOT EXISTS idx_onto_tasks_project_due_active
  ON public.onto_tasks(project_id, due_at)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_onto_milestones_project_due_active
  ON public.onto_milestones(project_id, due_at)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_onto_goals_project_target_active
  ON public.onto_goals(project_id, target_date)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_onto_events_project_start_active
  ON public.onto_events(project_id, start_at)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_onto_project_logs_project_created_desc
  ON public.onto_project_logs(project_id, created_at DESC);
...
GRANT EXECUTE ON FUNCTION public.build_fastchat_project_intelligence(text, uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.build_fastchat_project_intelligence(text, uuid, uuid) TO service_role;
...
BEGIN
  ...
  IF position('project_intelligence' in v_function_sql) = 0 THEN
