...
-- Add RPC for FastChat V2 initial context loading in a single round trip.
...
GRANT EXECUTE ON FUNCTION load_fastchat_context(text, uuid, uuid, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION load_fastchat_context(text, uuid, uuid, text, uuid) TO service_role;

COMMENT ON FUNCTION load_fastchat_context(text, uuid, uuid, text, uuid) IS
  'Loads FastChat V2 context payload in a single round trip.';
