-- supabase/migrations/20260914165546_agentic_chat_workflow_prompt_snapshot.sql
-- Tasker 83: persist the exact first provider request of the read-only
-- /workflow prototype. That request is a separate bounded two-message prompt
-- (review rules + evidence packet), not the admitted chat prompt with inserted
-- runtime guidance, so v3 rejected every workflow snapshot as
-- agentic_chat_prompt_snapshot_invalid_runtime_augmentation (prepared hit or miss).
--
-- The worker call signature is unchanged. Ordinary and runtime-augmented
-- snapshots keep the 20260817010000 behavior exactly.
...
DECLARE
	...
	v_admitted_message text;
	v_workflow_question text;
	v_workflow_system text;
	v_workflow_user text;
	v_base_system_sha256 text;
BEGIN
	...
	-- Read-only /workflow prototype. Its first provider request replaces the
	-- admitted system prompt, so it can only be recognized from the admitted
	-- command plus a fixed, tool-free, two-message shape bound to this turn's
	-- question. An ordinary-shaped request on the same turn keeps the fences below.
	v_admitted_message := v_turn.request_payload->>'message';
	IF v_admitted_message ~* '^\s*/workflow(\s|$)'
		AND p_model_messages->0 IS DISTINCT FROM v_base_messages->0 THEN
		-- Mirrors the worker: message.replace(/^\s*\/workflow\s*/i, '').trim().
		v_workflow_question := regexp_replace(
			regexp_replace(v_admitted_message, '^\s*/workflow\s*', '', 'i'),
			'\s+$',
			''
		);
		IF jsonb_typeof(p_model_messages) IS DISTINCT FROM 'array'
			OR jsonb_array_length(p_model_messages) <> 2
			OR p_tool_definitions IS DISTINCT FROM '[]'::jsonb
			OR jsonb_typeof(p_model_messages->0->'content') IS DISTINCT FROM 'string'
			OR jsonb_typeof(p_model_messages->1->'content') IS DISTINCT FROM 'string'
			OR v_workflow_question = '' THEN
			RAISE EXCEPTION 'agentic_chat_prompt_snapshot_invalid_workflow_prompt';
		END IF;
		v_workflow_system := p_model_messages->0->>'content';
		v_workflow_user := p_model_messages->1->>'content';
		IF p_model_messages->0 IS DISTINCT FROM jsonb_build_object(
				'role', 'system',
				'content', v_workflow_system
			)
			OR p_model_messages->1 IS DISTINCT FROM jsonb_build_object(
				'role', 'user',
				'content', v_workflow_user
			)
			OR NOT starts_with(
				v_workflow_system,
			...
			OR NOT starts_with(
				v_workflow_user,
				E'USER QUESTION\n' || v_workflow_question || E'\n\n'
			)
			OR p_system_prompt_sha256 IS DISTINCT FROM
				encode(sha256(convert_to(v_workflow_system, 'UTF8')), 'hex') THEN
			RAISE EXCEPTION 'agentic_chat_prompt_snapshot_invalid_workflow_prompt';
		END IF;
		...
		IF p_system_prompt_chars < char_length(v_workflow_system)
			OR p_system_prompt_chars > octet_length(v_workflow_system)
		...
		SELECT COALESCE(sum(char_length(item->>'content')), 0)::integer,
			COALESCE(sum(ceil(char_length(item->>'content') / 4.0)), 0)::integer
		INTO v_base_message_chars, v_base_approx_prompt_tokens
		FROM jsonb_array_elements(v_base_messages) messages(item);
		IF (v_artifact.prepared->'contextUsageSnapshot'->>'estimatedTokens') ~ '^[0-9]{1,9}$' THEN
			v_base_approx_prompt_tokens :=
				(v_artifact.prepared->'contextUsageSnapshot'->>'estimatedTokens')::integer;
		END IF;
		v_base_system_sha256 := encode(
			sha256(convert_to(v_artifact.prepared->>'systemPrompt', 'UTF8')),
			'hex'
		);

		-- v2 keeps every queue, ownership, artifact, and tool-surface fence. It
		-- persists the reconstructed admitted base; the same transaction then
		-- replaces only the evidence with the validated workflow request.
		v_receipt := public.persist_agentic_chat_prompt_snapshot_v2(
			...
			v_base_messages,
			p_tool_definitions,
			v_base_system_sha256,
			p_messages_sha256,
			p_tools_sha256,
			char_length(v_artifact.prepared->>'systemPrompt'),
		...
		SET system_prompt = v_workflow_system,
			model_messages = p_model_messages,
			...
			prompt_sections = jsonb_set(
				...
				jsonb_build_object(
					...
					'messages_sha256', p_messages_sha256,
					'admitted_system_prompt_sha256', v_base_system_sha256,
					'admitted_message_count', jsonb_array_length(v_base_messages)
				),
				true
			)
...
COMMENT ON FUNCTION public.persist_agentic_chat_prompt_snapshot_v3(
	...
	'P5 runtime augmentation plus Tasker 83 workflow prompt: exact first provider request with bounded system-only guidance, or the tool-free read-only /workflow request bound to the admitted question.';
