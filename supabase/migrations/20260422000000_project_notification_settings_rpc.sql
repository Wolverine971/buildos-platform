...
-- Project-scoped notification settings (project default + per-member override)
-- persisted in onto_projects.props.notifications.shared_project_activity.
...
GRANT EXECUTE ON FUNCTION public.get_project_notification_settings(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_project_notification_settings(uuid) TO service_role;
...
COMMENT ON FUNCTION public.get_project_notification_settings(uuid) IS
	'Returns project activity notification settings (project default + current member effective state).';

COMMENT ON FUNCTION public.set_project_notification_settings(uuid, boolean, boolean) IS
	'Updates project activity notification settings. Members can set their own override; admins can change project default.';
