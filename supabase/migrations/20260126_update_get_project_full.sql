-- supabase/migrations/20260126_update_get_project_full.sql
-- Update get_project_full() to remove task-plan enrichment and filter soft-deleted entities
...
DECLARE
  ...
  v_result jsonb;
BEGIN
  -- Verify the project exists and user has access (exclude soft-deleted projects)
  SELECT to_jsonb(p.*)
  INTO v_project
...
COMMENT ON FUNCTION get_project_full(uuid, uuid) IS
  ...
  'Filters soft-deleted entities and returns NULL if project not found or user lacks access.';
