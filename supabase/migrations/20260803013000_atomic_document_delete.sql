-- supabase/migrations/20260803013000_atomic_document_delete.sql
...
CREATE OR REPLACE FUNCTION public.onto_document_delete_atomic(
	p_project_id uuid,
	p_document_id uuid,
	p_permanent boolean,
...
BEGIN
	IF p_project_id IS NULL
		...
		RAISE EXCEPTION USING
			ERRCODE = '22023',
			MESSAGE = 'document_delete_invalid_arguments';
	END IF;
	...
	IF coalesce(auth.role(), '') <> 'service_role'
		...
		RAISE EXCEPTION USING
			ERRCODE = '42501',
			MESSAGE = 'document_delete_access_denied';
	END IF;
	...
	IF NOT FOUND THEN
		RAISE EXCEPTION USING
			ERRCODE = 'P0002',
			MESSAGE = 'document_delete_project_not_found';
	END IF;
	...
	IF v_current_structure_version <> p_expected_structure_version THEN
		RAISE EXCEPTION USING
			ERRCODE = 'P0001',
			MESSAGE = 'document_delete_structure_version_conflict';
	END IF;
	...
	IF NOT FOUND THEN
		RAISE EXCEPTION USING
			ERRCODE = 'P0002',
			MESSAGE = 'document_delete_document_not_found';
	END IF;

	IF v_target_updated_at IS DISTINCT FROM p_expected_updated_at THEN
		RAISE EXCEPTION USING
			ERRCODE = 'P0001',
			MESSAGE = 'document_delete_version_conflict';
	END IF;

	IF p_permanent AND v_target_state <> 'archived' THEN
		RAISE EXCEPTION USING
			ERRCODE = '22023',
			MESSAGE = 'document_delete_permanent_requires_archived';
	END IF;

	IF NOT p_permanent AND v_target_state = 'archived' THEN
		RAISE EXCEPTION USING
			ERRCODE = '22023',
			MESSAGE = 'document_delete_archived_requires_permanent';
	END IF;
	...
	IF p_permanent THEN
		DELETE FROM public.onto_documents AS document
		WHERE document.id = p_document_id
			AND document.project_id = p_project_id
			AND document.deleted_at IS NULL;
	ELSE
		...
		WHERE document.id = p_document_id
			...
			AND document.deleted_at IS NULL;
	END IF;
	GET DIAGNOSTICS v_affected_documents = ROW_COUNT;

	IF v_affected_documents <> 1 THEN
		RAISE EXCEPTION USING
			ERRCODE = 'P0001',
			MESSAGE = 'document_delete_document_mismatch';
	END IF;
...
REVOKE ALL ON FUNCTION public.onto_document_delete_atomic(
	uuid,
	uuid,
	boolean,
	timestamptz,
...
GRANT EXECUTE ON FUNCTION public.onto_document_delete_atomic(
	uuid,
	uuid,
	boolean,
	timestamptz,
...
COMMENT ON FUNCTION public.onto_document_delete_atomic(
	uuid,
	uuid,
	boolean,
	timestamptz,
	...
	'Atomically soft- or permanently deletes one document and applies its canonical tree mutation.';
