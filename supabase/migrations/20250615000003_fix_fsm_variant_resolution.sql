...
comment on function get_allowed_transitions(text, uuid) is
  'Returns allowed transitions for an entity by evaluating its FSM guards, following template inheritance when necessary.';

grant execute on function get_allowed_transitions(text, uuid) to authenticated;

-- ---------------------------------------------------------------------------
-- Backfill projects stuck in invalid states (e.g., variant templates).
-- ---------------------------------------------------------------------------
