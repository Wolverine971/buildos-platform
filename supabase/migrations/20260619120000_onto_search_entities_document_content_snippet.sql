...
--
-- Step 5 / Finding F7 (AGENTIC_CHAT_SEARCH_EVAL_2026-06-19): the document branch of
-- onto_search_entities built its ts_headline snippet from title + description + props::text,
-- NOT from the canonical `content` column. After the 20260617010000 fix, document bodies are
-- matched via search_vector(content) but the SNIPPET source still used props::text, yielding
-- noisy JSON fragments (e.g. `{"body_markdown": ...}`) and never highlighting a body-only term.
--
-- This migration re-creates onto_search_entities with the document snippet sourced from
-- `content` instead of `props::text` (title + description + content), matching the indexed
-- weights. Only the document branch changes; all other entity branches are byte-identical to
-- 20260428000014. Idempotent search_vector/index DDL from that migration is intentionally NOT
-- repeated here. This must land before props.body_markdown is dropped.
