 ...
 */
function transformBriefFailed(payload: BriefFailedEventPayload): NotificationPayload {
	return {
 ...
 */
function transformTaskDueSoon(payload: TaskDueSoonEventPayload): NotificationPayload {
	const hoursText = payload.hours_until_due === 1 ? '1 hour' : `${payload.hours_until_due} hours`;
...
function transformProjectPhaseScheduled(
	...
	const taskText = payload.task_count === 1 ? '1 task' : `${payload.task_count} tasks`;
 ...
 */
function transformCalendarSyncFailed(payload: CalendarSyncFailedEventPayload): NotificationPayload {
	return {
 ...
 */
function transformUserSignup(payload: UserSignupEventPayload): NotificationPayload {
	const methodText = payload.signup_method === 'google_oauth' ? 'Google' : 'Email';
...
export function transformEventPayload(
	...
	try {
		...
		console?.error(`[PayloadTransformer] Error transforming ${eventType}:`, error);
		throw error;
	}
}
