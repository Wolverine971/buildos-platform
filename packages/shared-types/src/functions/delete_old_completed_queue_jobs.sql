-- packages/shared-types/src/functions/delete_old_completed_queue_jobs.sql
-- Source: Migration (20260425000002_queue_job_retention_cleanup.sql)

CREATE OR REPLACE FUNCTION public.delete_old_completed_queue_jobs(p_retention_days integer DEFAULT 30, p_batch_size integer DEFAULT 500, p_job_types text[] DEFAULT NULL::text[])
 RETURNS TABLE(deleted_count integer, oldest_deleted_completed_at timestamp with time zone, newest_deleted_completed_at timestamp with time zone)
 LANGUAGE plpgsql
AS $function$
DECLARE
