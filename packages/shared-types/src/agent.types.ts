...
export interface FocusEntitySummary {
	id: string;
	name: string;
	type: 'task' | 'goal' | 'plan' | 'document' | 'milestone' | 'risk' | 'requirement';
	metadata?: {
...
export function isAgentChatType(type: string): type is AgentChatType {
	return ['general', 'project_create', 'project', 'daily_brief_update'].includes(type);
}
...
export function getNextPrioritizedDimension(detected: string[], covered: string[]): string | null {
