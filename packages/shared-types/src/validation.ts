...
export function isValidBriefGenerationStep(step: string): step is BriefGenerationStep {
...
export function validateDailyBriefMetadata(metadata: unknown): DailyBriefJobMetadata {
	...
	// Required fields
	if (typeof meta.briefDate !== 'string' || !isValidDateString(meta.briefDate)) {
		throw new ValidationError('briefDate', meta.briefDate, 'YYYY-MM-DD date string');
	}
...
export function validateBriefGenerationProgress(progress: unknown): BriefGenerationProgress {
	...
	if (!isValidProgress(prog.progress as number)) {
		throw new ValidationError('progress', prog.progress, 'number between 0-100');
	}
...
export function validatePhaseGenerationMetadata(metadata: unknown): PhaseGenerationJobMetadata {
	if (!metadata || typeof metadata !== 'object') {
		throw new ValidationError('metadata', metadata, 'object');
	}
	...
	if (meta.includeExistingTasks !== undefined && typeof meta.includeExistingTasks !== 'boolean') {
		throw new ValidationError('includeExistingTasks', meta.includeExistingTasks, 'boolean');
	}
...
export function validateCalendarSyncMetadata(metadata: unknown): CalendarSyncJobMetadata {
	if (!metadata || typeof metadata !== 'object') {
		throw new ValidationError('metadata', metadata, 'object');
	}
	...
	if (meta.dateRange !== undefined) {
		...
		if (typeof range.start !== 'string' || typeof range.end !== 'string') {
			throw new ValidationError('dateRange', range, 'object with start and end strings');
		}
	}

	if (meta.lastSyncedAt !== undefined && typeof meta.lastSyncedAt !== 'string') {
...
export function validateEmailJobMetadata(metadata: unknown): EmailJobMetadata {
	...
	if (typeof meta.recipientUserId !== 'string') {
		throw new ValidationError('recipientUserId', meta.recipientUserId, 'string');
	}
	...
	if (!validEmailTypes.includes(meta.emailType as string)) {
		throw new ValidationError('emailType', meta.emailType, validEmailTypes.join('|'));
	}
...
export function validateRecurringTaskMetadata(metadata: unknown): RecurringTaskJobMetadata {
	if (!metadata || typeof metadata !== 'object') {
		throw new ValidationError('metadata', metadata, 'object');
	}
	...
	if (meta.taskIds !== undefined) {
		if (!Array.isArray(meta.taskIds) || !meta.taskIds.every((id) => typeof id === 'string')) {
	...
	if (meta.updatedCount !== undefined && typeof meta.updatedCount !== 'number') {
...
export function validateCleanupJobMetadata(metadata: unknown): CleanupJobMetadata {
	if (!metadata || typeof metadata !== 'object') {
		throw new ValidationError('metadata', metadata, 'object');
	}
	...
	if (meta.deletedCount !== undefined && typeof meta.deletedCount !== 'number') {
		throw new ValidationError('deletedCount', meta.deletedCount, 'number');
	}

	if (meta.entities !== undefined) {
		if (!Array.isArray(meta.entities) || !meta.entities.every((e) => typeof e === 'string')) {
...
export function validateSendSMSMetadata(metadata: unknown): SendSMSJobMetadata {
	...
	if (meta.priority !== undefined && !validPriorities.includes(meta.priority as string)) {
		throw new ValidationError('priority', meta.priority, validPriorities.join('|'));
	}

	if (meta.template_key !== undefined && typeof meta.template_key !== 'string') {
		throw new ValidationError('template_key', meta.template_key, 'string');
	}

	if (meta.scheduled_for !== undefined && typeof meta.scheduled_for !== 'string') {
...
export function validateNotificationMetadata(metadata: unknown): NotificationJobMetadata {
	if (!metadata || typeof metadata !== 'object') {
		throw new ValidationError('metadata', metadata, 'object');
	}
...
export function validateVoiceNoteTranscriptionMetadata(
	metadata: unknown
): VoiceNoteTranscriptionJobMetadata {
	if (!metadata || typeof metadata !== 'object') {
		throw new ValidationError('metadata', metadata, 'object');
	...
	if (typeof meta.voiceNoteId !== 'string' || !isValidUUID(meta.voiceNoteId)) {
		throw new ValidationError('voiceNoteId', meta.voiceNoteId, 'valid UUID');
	}
