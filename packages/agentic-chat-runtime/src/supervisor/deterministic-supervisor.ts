// packages/agentic-chat-runtime/src/supervisor/deterministic-supervisor.ts
import type {
	TurnDigest,
	TurnSupervisor,
...
class DeterministicTurnSupervisor implements TurnSupervisor {
	...
	observe(observation: TurnSupervisorObservation): TurnSupervisorDecision[] {
		...
		// Keep construction transport-neutral and side-effect free. The explicit
		// turn-start observation owns the semantic epoch; the first observation is
		// a compatibility fallback for hosts that predate that boundary.
		this.startedAt ??= at;
		this.applyObservation(observation, at);
		return this.decide(observation, at);
	}

	getDigest(at = Date.now()): TurnDigest {
		return this.buildDigest(at);
	}
	...
	private buildDigest(at: number): TurnDigest {
		const elapsedMs = Math.max(0, at - (this.startedAt ?? at));
