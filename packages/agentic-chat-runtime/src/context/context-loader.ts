...
import type { MasterPromptContext } from './prompt-context';
import { isValidIanaTimezone } from '@buildos/shared-agent-ops/dates/civil-date';
import {
	ensureActorId,
...
/** A host supplies logging; clients and user identity remain per-call inputs. */
export type FastChatContextLoaderPorts = {
 ...
 * Project RPC denial never falls back to ordinary table reads. This factory does
 * not grant access or make a service-role client safe for arbitrary user input.
 */
export function createFastChatContextLoader({ logger }: FastChatContextLoaderPorts) {
	...
	function buildGlobalContextFromRpc(payload: FastChatContextRpcResponse): GlobalContextData {
		...
		const milestones = asArray<MilestoneRow & { project_id?: string }>(
			payload.milestones
		).filter(
	...
	function buildProjectContextFromRpc(
		...
		return buildProjectContextData({
			...
			projectIntelligence: normalizeProjectIntelligenceFromPayload(
	...
	async function loadProjectStartHereDocument(
		...
		return documentRow
			? mapStartHereDocument(documentRow as ProjectStartHereDocumentRow)
			: null;
	}
	...
	async function loadGlobalContextData(
		...
		const projectDatesById = new Map<
			string,
	...
	async function loadProjectContextData(
		...
		const [
			...
			supabase
				...
				.gte(
					...
					new Date(
						Date.now() -
							PROJECT_INTELLIGENCE_RECENT_CHANGES_MAX_LOOKBACK_DAYS * DAY_IN_MS
					).toISOString()
				)
		...
		if (membersRes.error)
			logger.warn('Failed to load project members for fast context', {
				error: membersRes.error
			});
		if (membersRes.error) {
			reportContextLoadError(onError, 'query.project.members', membersRes.error, {
				projectId
			});
		}
		...
		const fetches = entries.map(
			async ([kind, ids]): Promise<readonly [string, Record<string, unknown>[]]> => {
				...
				if (error) {
					...
					reportContextLoadError(
						onError,
						`query.project.linked_entities.${kind}`,
						error,
						{
							projectId,
							focusEntityId,
	...
	async function loadEntityContextData(params: {
		...
		const projectContext = await loadProjectContextData(
			supabase,
			projectId,
			eventWindow,
			onError
	...
	async function loadDailyBriefContextData(params: {
		...
		const mentionedEntitiesFromTable: DailyBriefMentionedEntity[] = (
			entitiesRes.data ?? []
		).map((row) => {
			...
			if (rpcPayload) {
				if (rpcContextType === 'global') {
					...
					reportContextLoadError(
						...
						new Error(
							'FastChat global RPC payload did not include project intelligence'
						),
