...
export class SMSMetricsService {
	 ...
	 * Record SMS sent
	 */
	async recordSent(userId: string, smsMessageId: string, twilioSid?: string): Promise<void> {
		try {
			const today = format(new Date(), 'yyyy-MM-dd');
	...
	async recordDelivered(
		...
		try {
			...
			console.log(`[SMSMetrics] Recorded delivery for user ${userId} (${deliveryTimeMs}ms)`);
		} catch (error) {
	 ...
	 * Record SMS failed
	 */
	async recordFailed(userId: string, smsMessageId: string, errorMessage?: string): Promise<void> {
		try {
			const today = format(new Date(), 'yyyy-MM-dd');
	 ...
	 */
	async recordCancelled(userId: string, smsMessageId: string, reason?: string): Promise<void> {
		try {
			const today = format(new Date(), 'yyyy-MM-dd');
	...
	async recordLLMGeneration(
		...
		try {
			...
			console.log(`[SMSMetrics] Recorded ${generatedVia} generation for user ${userId}`);
		} catch (error) {
	 ...
	 */
	async getDailyMetrics(startDate: string, endDate?: string): Promise<DailyMetricsSummary[]> {
		try {
			const { data, error } = await (this.supabase.rpc as any)('get_sms_daily_metrics', {
	...
	async getUserMetrics(userId: string, days: number = 30): Promise<UserMetrics[]> {
		try {
			const { data, error } = await (this.supabase.rpc as any)('get_user_sms_metrics', {
				p_user_id: userId,
				p_days: days
			});
	 ...
	 *
	 * Refreshes the sms_metrics_daily materialized view with latest data.
	 * This should be called hourly by the scheduler to keep dashboard metrics current.
	 */
	async refreshMaterializedView(): Promise<void> {
		try {
			const { error } = await (this.supabase.rpc as any)('refresh_sms_metrics_daily');
	...
	async calculateLLMSuccessRate(userId: string, days: number = 7): Promise<number> {
		try {
			const { data, error } = await this.supabase
				...
				.gte(
					'metric_date',
					format(new Date(Date.now() - days * 24 * 60 * 60 * 1000), 'yyyy-MM-dd')
				)
...
export const smsMetricsService = {
	...
	recordScheduled: (...args: Parameters<SMSMetricsService['recordScheduled']>) =>
	...
	recordDelivered: (...args: Parameters<SMSMetricsService['recordDelivered']>) =>
	...
	recordCancelled: (...args: Parameters<SMSMetricsService['recordCancelled']>) =>
		smsMetricsService.instance.recordCancelled(...args),
	recordLLMGeneration: (...args: Parameters<SMSMetricsService['recordLLMGeneration']>) =>
	...
	recordQuietHoursSkip: (...args: Parameters<SMSMetricsService['recordQuietHoursSkip']>) =>
		smsMetricsService.instance.recordQuietHoursSkip(...args),
	recordDailyLimitHit: (...args: Parameters<SMSMetricsService['recordDailyLimitHit']>) =>
		smsMetricsService.instance.recordDailyLimitHit(...args),
	getDailyMetrics: (...args: Parameters<SMSMetricsService['getDailyMetrics']>) =>
	...
	getTodayMetrics: (...args: Parameters<SMSMetricsService['getTodayMetrics']>) =>
		smsMetricsService.instance.getTodayMetrics(...args),
	refreshMaterializedView: (...args: Parameters<SMSMetricsService['refreshMaterializedView']>) =>
		smsMetricsService.instance.refreshMaterializedView(...args),
	calculateDeliveryRate: (...args: Parameters<SMSMetricsService['calculateDeliveryRate']>) =>
		smsMetricsService.instance.calculateDeliveryRate(...args),
	calculateLLMSuccessRate: (...args: Parameters<SMSMetricsService['calculateLLMSuccessRate']>) =>
		smsMetricsService.instance.calculateLLMSuccessRate(...args),
	getAvgLLMCostPerUser: (...args: Parameters<SMSMetricsService['getAvgLLMCostPerUser']>) =>
