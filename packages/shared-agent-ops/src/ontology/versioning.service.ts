// packages/shared-agent-ops/src/ontology/versioning.service.ts
 ...
 * write. The `update_onto_documents_updated_at` trigger ignores outline-only
 * writes, so this does not bump the document's audit/recency timestamp. Read
 * tools recompute live, so a transient failure here is harmless.
 */
async function persistDocumentOutline(
	supabase: Supabase,
	documentId: string,
	content: string | null
): Promise<void> {
	try {
		...
		console.warn('[versioning] Failed to persist document outline:', error);
	}
}
...
export async function createOrMergeDocumentVersion(
	...
	// Refresh the derived outline cache whenever a version is created or merged
	// (i.e. whenever the document actually changed). Awaited but error-swallowed.
	await persistDocumentOutline(supabase, documentId, snapshot.content);
