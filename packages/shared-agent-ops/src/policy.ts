// packages/shared-agent-ops/src/policy.ts
//
// Scope/op policy for BuildOS agent operations — scope_mode + allowed_ops
// normalization, op classification (read/write), and caller-policy helpers.
//
// Moved here from apps/web/src/lib/server/agent-call/agent-call-policy.ts so both
// the web agent-call gateway AND the worker Agent Run runner share one policy path.
// Pure logic: depends only on @buildos/shared-types.
import {
