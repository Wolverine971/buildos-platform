// packages/shared-agent-ops/src/email/gmail-read-oauth.service.ts
// Server-only Gmail read OAuth: authorization, verified callback exchange,
// credential rotation and connection lifecycle. Hosts inject how private
// configuration resolves; no SvelteKit dependency.
import {
	CodeChallengeMethod,
	OAuth2Client,
	type Credentials,
...
import {
	...
	type GmailTokenKeyResolver
} from './gmail-token-crypto';
import type { GmailConnectionSummary, GmailConnectionsPayload } from './gmail-integration.types';
...
const LOG_PREFIX = '[GmailReadOAuthService]';
...
/** Resolves a private configuration value for the host runtime. */
export type GmailReadEnvReader = (name: string) => string | undefined;

export const readGmailReadEnvFromProcess: GmailReadEnvReader = (name) => {
	const value = process.env[name];
	return typeof value === 'string' && value.trim().length > 0 ? value.trim() : undefined;
};

export type GmailReadOAuthServiceOptions = {
	clientId?: string;
	clientSecret?: string;
	createOAuthClient?: (redirectUri?: string) => GmailOAuthClient;
	now?: () => Date;
	randomToken?: () => string;
	/** How private configuration (client id/secret, encryption keys) resolves. */
...
export class GmailReadOAuthService {
	...
	private readonly readEnv: GmailReadEnvReader;
	private readonly resolveTokenKey: GmailTokenKeyResolver;
	...
	constructor(
		...
		this.readEnv = options.readEnv ?? readGmailReadEnvFromProcess;
		this.resolveTokenKey = (version) =>
			this.readEnv(`PRIVATE_GMAIL_TOKEN_ENCRYPTION_KEY_V${version}`);
		this.clientId = options.clientId ?? this.readEnv('PRIVATE_GMAIL_READ_CLIENT_ID') ?? '';
		this.clientSecret =
			options.clientSecret ?? this.readEnv('PRIVATE_GMAIL_READ_CLIENT_SECRET') ?? '';
	...
	isConfigured(): boolean {
		const encryptionKey = this.readEnv('PRIVATE_GMAIL_TOKEN_ENCRYPTION_KEY_V1');
		const existingGoogleClientId = this.readEnv('PRIVATE_GOOGLE_CLIENT_ID');
		return Boolean(
			this.clientId &&
				this.clientSecret &&
				encryptionKey &&
		...
		try {
			return await new GmailAccountReadPort(this.admin as unknown as TypedSupabaseClient, {
				available: isGmailReadRuntimeConfigured({
					...
					tokenEncryptionKey: this.readEnv('PRIVATE_GMAIL_TOKEN_ENCRYPTION_KEY_V1'),
					conflictingGoogleClientId: this.readEnv('PRIVATE_GOOGLE_CLIENT_ID')
				})
			...
			console.error(`${LOG_PREFIX} Shared Gmail account read failed`, { error, userId });
	...
	async exchangeAuthorizationCode(params: {
		...
		const accessTokenCiphertext = encryptGmailToken(
			tokens.access_token,
			tokenContext,
			this.resolveTokenKey
		);
		const refreshTokenCiphertext = encryptGmailToken(
			refreshToken,
			tokenContext,
			this.resolveTokenKey
		);
	...
	async getAuthorizedReadAccessToken(
		...
		const { error: rotateError } = await this.admin.rpc('rotate_gmail_read_credentials', {
			p_user_id: userId,
			p_connection_id: connectionId,
			p_access_token_ciphertext: encryptGmailToken(
				refreshed.access_token,
				tokenContext,
				this.resolveTokenKey
			),
			p_refresh_token_ciphertext: encryptGmailToken(
				rotatedRefreshToken,
				tokenContext,
				this.resolveTokenKey
			),
