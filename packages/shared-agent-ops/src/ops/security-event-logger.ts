// packages/shared-agent-ops/src/ops/security-event-logger.ts
//
// Worker-safe security event logger (R3 dependency-inversion cut).
//
// The web build creates an admin Supabase client by default; the package cannot
// (no `$lib/supabase/admin`, no `$env`). So at the package boundary the caller
// MUST pass `options.supabase`. The web shim at `$lib/server/security-event-logger`
// re-exports everything here and defaults `supabase` to `createAdminSupabaseClient()`
// so the existing web importers keep their auto-admin behavior.
...
async function persistSecurityEvent(
	...
	try {
		const admin = options.supabase as AdminSupabaseClient | undefined;
		if (!admin) {
			console.warn('[SecurityEvent] Skipped write: no supabase client supplied', {
