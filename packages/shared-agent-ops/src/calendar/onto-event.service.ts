// packages/shared-agent-ops/src/calendar/onto-event.service.ts
// Ontology `onto_events` row CRUD, shared by web and the worker. Pure Supabase
// writes with no host runtime dependency — moved out of apps/web so the shared
// calendar write service can create and update events on either host.
