...
describe('ensureToolCompatibleModels', () => {
	...
	it('routes new OpenRouter models only through compatible lanes', () => {
		...
		// Tasker 108: GPT-6 Luna replaced Gemini 3.7 Flash in every automatic lane.
		expect(OPENROUTER_V2_TEXT_MODELS).not.toContain(GEMINI_37_FLASH_MODEL);
		...
		expect(OPENROUTER_V2_TEXT_MODELS[1]).toBe(GPT_6_LUNA_MODEL);
		...
		expect(OPENROUTER_V2_JSON_MODELS).not.toContain(GEMINI_37_FLASH_MODEL);
		expect(OPENROUTER_V2_JSON_MODELS).not.toContain(QWEN_37_PLUS_EXPERIMENT_MODEL);
		expect(OPENROUTER_V2_JSON_MODELS[1]).toBe(GPT_6_LUNA_MODEL);
		...
		expect(OPENROUTER_V2_TOOL_MODELS).not.toContain(GEMINI_37_FLASH_MODEL);
		expect(OPENROUTER_V2_TOOL_MODELS).not.toContain(QWEN_37_PLUS_EXPERIMENT_MODEL);
		expect(OPENROUTER_V2_TOOL_MODELS[1]).toBe(GPT_6_LUNA_MODEL);
		...
		expect(OPENROUTER_V2_MULTIMODAL_MODELS[0]).toBe(GPT_6_LUNA_MODEL);
	...
	it('uses stronger specialist models only for quality and maximum profiles', () => {
		...
		expect(TEXT_PROFILE_MODELS.quality[0]).toBe(GPT_6_LUNA_MODEL);
		...
		expect(JSON_PROFILE_MODELS.powerful[0]).toBe(GPT_6_LUNA_MODEL);
